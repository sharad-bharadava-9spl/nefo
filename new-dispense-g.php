...
EOD;
	...
	// KONSTANT CODE UPDATE END
			...
			header("Location: new-dispense-g.php?user_id=$userid");
			exit();
  	   ...
  	   foreach($_POST['sales'] as $sale) {
					...
					// code update start by konstant for dispense validation on 28-02-2022
						if($sale['grams'] > 0 && ($sale['euro'] == 0 || $sale['euro'] == '')){
							...
							header("Location: new-dispense-g.php?user_id=$user_id");
							exit();
						}
					...
					if($sale['grams'] > 0){
						...
						if ($_SESSION['domain'] == 'greengoat' || $_SESSION['domain'] == 'greengoat2' || $_SESSION['domain'] == 'demo') {
							...
							$saleEuro =	 round($saleEuro, 2);
							$applied_discounts = (100 - $totDiscountInput) / 100;
							$tempPrice = $saleEuro * $applied_discounts;
	                        $eurdisc = $eurdiscount;
	                        $newSaleEuro = $tempPrice - $eurdiscount;
							$newSaleEuro =	 number_format($newSaleEuro, 2, '.', '');
							$saleGrams = number_format($saleGrams, 2, '.', '');
							$saleUnits = number_format($saleUnits, 2, '.', '');
							
							
/*		echo 	$eurcalcTOT."-"	.$newSaleEuro."-".$gramsTOT."-".$saleGrams; 
		
		if(abs($eurcalcTOT - $newSaleEuro) > 0.00001) {		
			echo "<br>4 error";
		}
...
function load1(){
    ...
    $.ajax({
      type:"post",
      url:"getflowers-g.php?i="+dayJSID+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
...
function load2(){
    ...
    $.ajax({
      type:"post",
      url:"getextracts-g.php?i="+dayJSID+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
      success:function(data)
      {
			$("#spinner2").remove();
...
function load(cat,type){ 
    ...
    $.ajax({
      type:"post",
      url:"getothers-g.php?i="+dayJSID+"&cat="+cat+"&type="+type+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
      success:function(data)
      {
			$("#spinner"+cat).remove();
...
<!-- Shopping cart JS -->
<script type="text/javascript" src="scripts/cart-g.js?t=<?php echo time();?>"></script>
