<?php 
	...
	if ($prodFijo != 0 || $prodFijo != '') {
		...
		if ($flower['medDiscount'] > 0) {
        	...
        	$discountType = 2;
			$price = round(($flower['salesPrice']) * $medicalDiscountCalc,2);
		} else {
			$dispDiscount1 = number_format((($medicalDiscount * 100) / $flower['salesPrice']),2);
        	$discountType = 2;
			$price = round(($flower['salesPrice']) - $medicalDiscount,2);
		}
            ...
            if(isset($rowDU['discount_percentage']) && $rowDU['discount_percentage'] > 0){
                ...
                if(isset($rowDU['discount_price']) && $rowDU['discount_price'] > 0){
                    ...
                    $discountType = 4;
                    $price = round(($flower['salesPrice']) - $rowDU['discount_price'],2);
                }
            }
	}
	 else {
		
		$price = round($flower['salesPrice'],2);
		...
		$catCurrentDiscount = (100 - $catDiscount) / 100;
            ...
            if ($prodFijoHD != 0 || $prodFijoHD != '') {
                ...
                if ($prodFijoHD1 > $dispDiscount1){
                    ...
                    //$price = round($flower['salesPrice'] * $prodDiscountHD,2);
...
<script>
   ...
   function compute2() {
          ...
          var roundedtotal = total.toFixed(2);
	  	    ...
	  	    // KONSTANT CODE UPDATE BEGIN
	        var volDiscount_amount = $('#volume_%d_discount_amount').val().delay(700);
	...
	if ($prodFijo != 0 || $prodFijo != '') {
		...
		if ($flower['medDiscount'] > 0) {
            ...
            if(isset($rowDU['discount_percentage']) && $rowDU['discount_percentage'] > 0){
                ...
                $discountType = 4;
                $price = round($flower['salesPrice'] - $dispDiscountP,2);
	} else {
