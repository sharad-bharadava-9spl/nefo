...
/*	ini_set("log_errors", TRUE);  
	...
	if (isset($_POST['mydata'])) {
		...
		$_SESSION['success_url'] = "expenses.php";
		...
		if ($_FILES[$image_fieldname]['error'] != 0) {
			...
			header("Location: new-expense.php?expenseid=$expenseid");
			exit();
		}
	 ...
	 <table class="default" id="mainTable">
	  <thead>
	   <tr style='cursor: pointer;'>
	    <th style='position: relative;'><a href="#" id="xllink"  style='position: absolute; top: 0; left: 10px; margin-top: -66px;'><img src="images/excel-new.png" /></a><?php echo $lang['pur-date']; ?></th>
...
/*	if (file_exists($file)) {
	...
	if ($invoice == 1) {
  	  ...
  	  <tr>
  	   ...
  	   <td class='clickableRow left' href='expense.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense.php?expenseid=%d'>%s</td>
  	   <td style='text-align: right;' class='clickableRow' href='expense.php?expenseid=%d'>%0.2f</td>
  	   <td class='clickableRow left' href='expense.php?expenseid=%d'>%s</td>
	  ...
	  $expense['expenseid'], $formattedDate, $expense['expenseid'], $formattedTime, $expense['expenseid'], $expenseCat, $expense['expenseid'], $expense['expense'], $expense['expenseid'], $expense['shop'], $expense['expenseid'], $member, $expense['expenseid'], $expense['amount'], $expense['expenseid'], $currency, $expense['expenseid'], $source, $recClass, $expense['expenseid'], $receipt, $invClass, $expense['expenseid'], $invoice, $invScan, $expense['expenseid'], $expense['expenseid']
	  );
  		} else {
			$expense_row =	sprintf("
  	  <tr>
...
<script type="text/javascript">
	/*$("#xllink").click(function(){
	    $("#load").show();
	    window.location.href = "expenses.php?action=xls"; 
	    setTimeout(function () {
	        $("#load").hide();
	    }, 2000);     
	 });*/
	...
	$("#xllink").click(function(){
	    ...
	    var url = 'expenses-report.php?fromDate='+fromDate+'&untilDate='+untilDate+'&filter='+filter;
