...
if(isset($_POST["title"]))
{
   ...
   $access_code = md5($_POST['title'].$_POST['start']); 
   $invite_email = '';
   if(!empty($_POST['invite_email'])){
        $invite_email = implode(",", $_POST['invite_email']);
    }
   // end event date on recurring events
    if(!empty($_POST['add_end_recurr'])){
        $end_date = $_POST['add_end_recurr'];
    }else{
        $end_date = $_POST['end'];
    }
    try {
        $query = "
            INSERT INTO events 
            (user_id,title, start_event, end_event,location,description, recurring_time, event_cale_id, usergroups, invite_email, access) 
            VALUES (:user_id,:title, :start_event, :end_event, :location, :description, :recurring_time, :event_cale_id, :usergroups, :invite_email, :access) ";
...
$calender_url = $siteroot."calendar/calendar-event-view.php?user_id=".$user_id."&event_id=".$last_id."&access=".$access_code."&domain=".$domain;
        ...
        if (!empty($addresses)) {
           // $file_to_attach = 'images/event_images/';
            foreach ($addresses as $key) {
                ...
                $mail->isHTML(true);
                $mail->Body = '<p> Invitation from '.$_SESSION['first_name'].'</p><p> Title : '.$_POST['title'].'  </p><p> Location : '.$_POST['location'].'  </p> <p>Description : '.$_POST['description'].' </p><p>Event Details: <a href='.$visitor_calendar_url.'>Visit Event</a></p>';
                $mail->send();
