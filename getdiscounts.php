...
while ($user = $results->fetch()) {
  ...
  if (($day + 1000) < $totrows) {
	  ...
	  $sofar = $day + 1000;
		...
		$sales_row .= <<<EOD
	<tr id='loadMore' style='border-bottom: 0;'><td class='centered' colspan='12'>($sofar / $totrows {$lang['global-members']} {$lang['loaded']})<br /><a href='#' onclick='event.preventDefault(); loadMoreDays()' style='font-size: 12px;'>{$lang['load-more']}</a></td></tr>
EOD;
} else {
		$sales_row .= <<<EOD
	<tr id='loadMore' style='border-bottom: 0;'><td class='centered' colspan='12'>($totrows / $totrows {$lang['global-members']} {$lang['loaded']})</td></tr>
EOD;
}
