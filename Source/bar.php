...
EOD;
	...
	if (isset($_POST['cardid'])) {
		...
		if ($cardid == '') {
			...
			if ($userGroup < 4 && $_SESSION['workertracking'] == 1) {
				...
				if (!$data) {
					...
					if ($data2) {
						...
						echo "<br /><center><a href='?signout&user_id=$user_id' class='cta'>{$lang['global-yes']}</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='bar-new-sale-2.php?user_id=$user_id' class='cta'>{$lang['global-no']}</a></center>";
						...
						echo "<br /><center><a href='?signin&user_id=$user_id' class='cta'>{$lang['global-yes']}</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='bar-new-sale-2.php?user_id=$user_id' class='cta'>{$lang['global-no']}</a></center>";
			...
			// On success: redirect.
			header("Location: bar-new-sale-2.php?user_id={$user_id}");
			exit();
    ...
    $.ajax({
      ...
      success:function(data)
      {
        if( data == 'false' ) {
	        ...
	        window.location.replace("bar.php?notregistered");
		    
	    } else {
	        window.location.replace("bar-new-sale-2.php?user_id="+data);
	    }
      }
    });
}, 3000);
...
EOD;
	...
	pageStart("CCS", NULL, $testinput, "pindex", "loggedIn", $lang['bar'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
