<?php
	...
	$selectUsers = "SELECT u.user_id, u.memberno, u.first_name, u.last_name, u.registeredSince, u.dni, u.gender, u.day, u.month, u.year, u.doorAccess, u.paidUntil, u.userGroup, u.form1, u.form2, u.credit, u.usageType, u.creditEligible, u.dniscan, u.dniext1, u.starCat, u.oldNumber, u.exento FROM users u, donations d WHERE u.user_id = d.userid AND u.memberno <> '0' AND u.userGroup < 6 AND DATE(d.donationTime) = DATE(NOW()) GROUP BY u.user_id ORDER by u.memberno ASC";
		try
		{
...
<br />
	...
	$donatedQuery = "SELECT SUM(amount) FROM donations WHERE DATE(donationTime) = DATE(NOW()) AND userid = $user_id";
		try
		{
	...
	$dispensedQuery = "SELECT SUM(amount) FROM sales WHERE DATE(saletime) = DATE(NOW()) AND userid = $user_id";
		try
		{
	...
	$barQuery = "SELECT SUM(amount) FROM b_sales WHERE DATE(saletime) = DATE(NOW()) AND userid = $user_id";
		try
		{
