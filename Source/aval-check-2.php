...
?>
	...
	if (isset($_GET['twoavals'])) {
		...
		$user_row =	sprintf("
	  	  <tr>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
	  	   <td class='clickableRow centered' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d&twoavals'>%s</td>
		  </tr>",
		...
		$user_row =	sprintf("
	  	  <tr>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d'>%s</td>
	  	   <td class='clickableRow centered' href='aval-details-2.php?user_id=%d'>%s</td>
	  	   <td class='clickableRow' href='aval-details-2.php?user_id=%d'>%s</td>
		  </tr>",
		  $user['user_id'], $user['memberno'], $user['user_id'], $user['first_name'], $user['user_id'], $user['last_name'], $user['user_id'], date("d-m-y",strtotime($user['registeredSince'])), $user['user_id'], $user['usageType'], $user['user_id'], $membertill
		  );
    ...
    $.ajax({
      ...
      success:function(data)
      {
        if( data == 'false' ) {
	        ...
	        window.location.replace("aval-check-2.php?aval=$aval&notregistered");
		    
	    } else {
		    
	        window.location.replace("aval-details-2.php?user_id="+data);
...
EOD;
	...
	$selectUsers = "SELECT user_id, memberno, first_name, last_name, registeredSince, gender, day, month, year, paidUntil, userGroup, usageType, exento FROM users WHERE memberno <> '0' AND userGroup < 6 AND user_id <> $aval ORDER by memberno ASC LIMIT $offset, $no_of_records_per_page";
		try
		{
...
EOD;
		...
		pageStart($lang['aval'], NULL, $validationScript, "aval1", "dispensepre memberlist", $lang['member-newmembercaps'] . " - " . $lang['avalC'] . "#2", $_SESSION['successMessage'], $_SESSION['errorMessage']);
		

	echo "<form onsubmit='oneClick.disabled = true; return true;' id='registerForm' action='aval-details-2.php' method='POST'>";
...
<center>
<br />
<a href="?pageno=1<?php echo $sortparam . '&aval=' . $aval; ?>" class='pagination <?php if ($pageno == 1 || (!isset($_GET['pageno']))) { echo 'disabled'; } ?>'>&laquo;</a>
<a href="<?php if($pageno <= 1){ echo '#'; } else { echo "?pageno=".($pageno - 1); } echo $sortparam . '&aval=' . $aval; ?>" class='pagination <?php if($pageno <= 1){ echo 'disabled'; } ?>'>Prev</a>
<a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo "?pageno=".($pageno + 1); } echo $sortparam . '&aval=' . $aval; ?>" class='pagination <?php if ($total_pages == $pageno){ echo 'disabled'; } ?>'>Next</a>
<a href="?pageno=<?php echo $total_pages; echo $sortparam . '&aval=' . $aval; ?>" class='pagination <?php if ($total_pages == $pageno){ echo 'disabled'; } ?>'>&raquo;</a>
</center>
<!-- Pagination code END -->
