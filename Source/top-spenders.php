...
EOD;
	...
	pageStart($lang['title-dispensary'], NULL, $deleteDonationScript, "topspenders", "product admin", $lang['topspenderscaps'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
echo "<span class='winnerboxheader'>$currMonth</span>";
		...
		while ($topUser = $result->fetch()) {
			...
			if ($i == 1) {
				...
				if (!file_exists($winnerImg)) {
					
					$winnerImg = "<img class='winnerimg' src='images/silhouette-new-big.png' />";
					
				} else {
					
			 		$winnerImg = "<img class='winnerimg' src='images/_$domain/members/$user_id.$photoExt' />";
...
<div class='winnerstats'>$amount &euro; /  $quantity gr</div>
...
<div class='notwinnerstats'>$amount &euro; /  $quantity gr</div>
</div>
...
echo "<span class='winnerboxheader'>$currMonth</span>";
		...
		// Look up this months sales
		$selectTopUsers = "SELECT u.user_id, u.first_name, u.memberno, u.photoExt, SUM(d.amount), SUM(d.quantity), SUM(d.quantity) from salesdetails d JOIN sales s ON s.saleid = d.saleid JOIN users u ON s.userid = u.user_id WHERE MONTH(s.saletime) = MONTH(NOW()) AND YEAR(s.saletime) = YEAR(NOW()) AND d.category = 1 GROUP BY u.user_id ORDER BY SUM(d.amount) DESC LIMIT $numberOfMembers";
		try
		{
			$result = $pdo3->prepare("$selectTopUsers");
			$result->execute();
...
<?php
		...
		$selectCats = "SELECT id, name, description from categories WHERE id > 2 ORDER by id ASC";
		try
		{
...
echo "<span class='winnerboxheader'>$currMonth</span>";
		...
		// Look up this months sales
		$selectTopUsers = "SELECT u.user_id, u.first_name, u.memberno, u.photoExt, SUM(d.amount), SUM(s.units) from salesdetails d JOIN sales s ON s.saleid = d.saleid JOIN users u ON s.userid = u.user_id WHERE MONTH(s.saletime) = MONTH(NOW()) AND YEAR(s.saletime) = YEAR(NOW()) AND d.category = $categoryid GROUP BY u.user_id ORDER BY SUM(d.amount) DESC LIMIT $numberOfMembers";
		try
		{
			$result = $pdo3->prepare("$selectTopUsers");
			$result->execute();
...
<a href='profile.php?user_id=$user_id'><span class='firsttext'>#$memberno</span><span class='nametext2'>$first_name</span></a><br />
<div class='winnerstats'>$amount &euro; /  $quantity u</div>
...
<div class='notwinner'>
<a href='profile.php?user_id=$user_id'><span class='usergrouptext2'>$i</span> &nbsp;<span class='firsttext'>#$memberno</span><span class='nametext2'>$first_name</span></a>
<div class='notwinnerstats'>$amount &euro; /  $quantity u</div>
</div>
