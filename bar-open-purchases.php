...
EOD;
	...
	echo "<center><a href='bar-closed-purchases.php' class='cta1'>{$lang['closed-purchases']}</a>";
	echo "<a href='bar-new-purchase.php' class='cta1'>{$lang['newpurchase']}</a>";
?>

<br><img src="images/excel-new.png" style="cursor: pointer;" onclick="loadExcel();" value="Export to Excel" />

<?php echo "</center><br /><h5 class='title2'>{$lang['open-purchases']}</h3><br />";
        

	// Query to look up categories
	$selectCats = "SELECT id, name, description, icon from b_categories ORDER by sortorder ASC, id ASC";
	try
	{
		$resultCats = $pdo3->prepare("$selectCats");
		$resultCats->execute();
...
<?php
	...
	$purchase_row =	sprintf("
  	  <tr>
  	   ...
  	   <td class='clickableRow right' href='bar-purchase.php?open&purchaseid=%d'>%0.0f g</td>
  	   <td class='clickableRow right' href='bar-purchase.php?open&purchaseid=%d'>%0.0f g</td>
  	   <td style='text-align: center' href='bar-purchase.php?open&purchaseid=%d'><a href='uTil/bar-menuchange.php?purchaseid=%d&menu=%s'>%s</a></td>
	   <td>$barCodeImg</td>
...
<script type="text/javascript">
	 function loadExcel(){
 			$("#load").show();
       		window.location.href = 'bar-open-purchases-report.php';
       		    setTimeout(function () {
			        $("#load").hide();
			    }, 5000);   
       }
