<?php
	...
	if (isset($_POST['realClosing'])) {
		...
		$selectProduct = "SELECT category FROM purchases WHERE purchaseid = $toProduct";
		try
		{
		...
		if ($category == 1) {
			...
			$selectProduct = "SELECT g.name, p.purchaseid FROM extract g, purchases p WHERE p.category = 2 AND p.purchaseid = $toProduct AND p.productid = g.extractid";
			
		} else {
			
			$selectProduct = "SELECT g.name, p.purchaseid FROM products g, purchases p WHERE p.purchaseid = $toProduct AND p.productid = g.productid";
	    ...
	    // Query to look up product names:
		$selectProduct = "SELECT category FROM purchases WHERE purchaseid = $purchaseid";
		try
		{
			$result = $pdo3->prepare("$selectProduct");
			$result->execute();
		...
		if ($category == 1) {
			...
			$selectProduct = "SELECT g.name, p.purchaseid FROM extract g, purchases p WHERE p.category = 2 AND p.purchaseid = $purchaseid AND p.productid = g.extractid";
			
		} else {
			
			$selectProduct = "SELECT g.name, p.purchaseid FROM products g, purchases p WHERE p.purchaseid = $purchaseid AND p.productid = g.productid";
    ...
    $(document).ready(function() {
     ...
     $('#otherProduct').change(function(){
        ...
        if(this.checked){
            $('#otherProductHolder').parent().parent('tr').fadeIn('slow');
            $('#otherProductHolder').fadeIn('slow');
        }
        else{
            $('#otherProductHolder').parent().parent('tr').fadeOut('slow');
...
<form id="registerForm" action="" method="POST">
 ...
 <table class="purchasetable">
  <tr>
   ...
   <input type="number" lang="nb" class="fourDigit purchaseNumber" id="realQuantity" name="realQuantity" value="<?php echo number_format($realQuantity,2,'.',''); ?>" readonly /></td>
  </tr>
  <tr>
   <td>+ <?php echo $lang['closeproduct-reloads']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber green" id="reloads" name="reloads" value="<?php echo number_format($reloads,2,'.',''); ?>" readonly /></td>
  </tr>
  <tr>
   <td>+ <?php echo $lang['global-added']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber green" id="additions" name="additions" value="<?php echo number_format($nonReloads,2,'.',''); ?>" readonly /></td>
  </tr>
  <tr>
   <td>- <?php echo $lang['closeproduct-takeouts']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber red" id="removals" name="removals" value="<?php echo number_format($permRemovals,2,'.',''); ?>" readonly /></td>
  </tr>
  <tr>
   <td>- <?php echo $lang['closeproduct-totaldispensed']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber red" id="quantity" name="quantity" value="<?php echo number_format($quantity,2,'.',''); ?>" /></td>
  </tr>
  <tr>
   <td>- <?php echo $lang['stash']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber red" id="quantity" name="quantity" value="<?php echo number_format($inStash,2,'.',''); ?>" /></td>
  </tr>
  <tr>
   <td>= <?php echo $lang['closeproduct-estclosing']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber" id="estClosing" name="estClosing" value="<?php echo number_format($estClosing,2,'.',''); ?>" readonly /></td>
  </tr>
  <tr>
   <td><?php echo $lang['product-remaining']; ?>
   <input type="number" lang="nb" class="fourDigit purchaseNumber" id="realClosing" name="realClosing" step="0.01" /></td>
  </tr>
  <tr>
   <td><?php echo $lang['add-to-other-product']; ?>?
   <input type="checkbox" name="otherProduct" id="otherProduct" style="width: 12px;" value="1" /></td>
  </tr>
  <tr  style='display: none;'>
   <td colspan="2"><span id='otherProductHolder' style='display: none;'><span class="fakelabel">&nbsp;</span><select class="fakeInput defaultinput" name="toProduct" >
...
?>
 ...
 <center><textarea name="closingComment" class='defaultinput' placeholder="<?php echo $lang['global-comment']; ?>?"></textarea></center>
 <br />
 <button name='oneClick' class='oneClick cta4' type="submit"><?php echo $lang['closeproduct-closeproduct']; ?></button>
</form>
	</div>
</div>
<center>
