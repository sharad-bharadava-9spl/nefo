<?php
	...
	if ($_POST['almacen'] == 'complete') {
		...
		if($movementTypeid == 12 && $movementTypeid2 == 5){
			$stashMovementType = 1;
		}

		if($movementTypeid == 2 && $movementTypeid2 == 6){
			$stashMovementType = 2;
		}
		...
		// Query to add new purchase movement - 6 arguments
		  ...
		  $movementtime, '1', $purchaseid, $quantity, $movementTypeid, $comment, $_SESSION['user_id'], $category, $stashMovementType);
		try
		{
		...
		catch (PDOException $e)
		{
		  ...
		  // Query to add new purchase movement - 6 arguments
		  $query = sprintf("INSERT INTO productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, user_id, category, stashMovementType) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%d', '%d');",
		  $movementtime, '2', $purchaseid, $quantity, $movementTypeid2, $comment, $_SESSION['user_id'], $category, $stashMovementType);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
	...
	if ($_POST['productRemove'] == 'complete') {
		...
		if ($otherProduct == 1) {
			...
			// Query to add new purchase movement - 6 arguments, add
			  $query = sprintf("INSERT INTO productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, user_id, category, stashMovementType) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%d', '%d');",
			  $movementtime, '1', $toProduct, $quantity, '22', $lang['added-from-other-product'] . ': ' . $fromName . '.', $_SESSION['user_id'], $toCategory, $stashMovementType);
			try
			{
				$result = $pdo3->prepare("$query")->execute();
			}
			catch (PDOException $e)
			{
			  ...
			  $query = sprintf("INSERT INTO productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, user_id, category, stashMovementType) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%d', '%d');",
			  $movementtime, '2', $purchaseid, $quantity, $movementTypeid, $lang['added-to-other-product'] . ': ' . $toName . '.', $_SESSION['user_id'], $category, $stashMovementType);
			try
			{
				$result = $pdo3->prepare("$query")->execute();
			}
			catch (PDOException $e)
			{
			  ...
			  // Query to add new purchase movement - 6 arguments
			  $query = sprintf("INSERT INTO productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, user_id, category, stashMovementType) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%d', '%d');",
			  $movementtime, '2', $purchaseid, $quantity, $movementTypeid, $comment, $_SESSION['user_id'], $category, $stashMovementType);
			try
			{
				$result = $pdo3->prepare("$query")->execute();
			}
...
<div class="actionbox-np2 mainbox-no-width-cls">
 ...
 <div class="boxcontent">
	<form id="registerForm1" action="" method="POST">
			...
			<div>
				<select name='provider' class="defaultinput" id='provider'>
			...
			?>
				...
				<span id="providerBox">
				<span style='display: inline-block; width: 80px;' class="smallgreen"><?php echo $lang['price']; ?> /g:</span> <input type="number" lang="nb" name="priceg" id="priceg" class="fourDigit defaultinput" step="0.01" value="<?php echo $purchasePrice; ?>" /><?php echo $_SESSION['currencyoperator'] ?><br />
				<span style='display: inline-block; width: 80px;' class="smallgreen"><?php echo $lang['price']; ?>:</span> <input type="number" lang="nb" name="price" id="price" class="fourDigit defaultinput" step="0.01" /><?php echo $_SESSION['currencyoperator'] ?><br />
				<span style='display: inline-block; width: 80px;' class="smallgreen"><?php echo $lang['paid']; ?>:<br /></span> <input type="number" lang="nb" name="paid" id="paid" class="fourDigit defaultinput" step="0.01" /><?php echo $_SESSION['currencyoperator'] ?><br />
				</div>
			<br />
				</span>
			...
			<button class='oneClickc cta4' name='oneClick' type="submit" id="addButton" style="margin-top: 97px;"><?php echo $lang['global-add']; ?></button>
	</form>
 </div>
</div>
...
<div class="actionbox-np2 mainbox-no-width-cls">
	...
	<div class="boxcontent">
		<form id="registerForm2" action="" method="POST">
			...
			<center><textarea name="comment" class="defaultinput" placeholder="<?php echo $lang['global-comment']; ?>?"></textarea><br /></center>
			
				
			<div class="fakeboxholder">	
			 <label class="control">
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $lang['add-to-other-product']; ?>?
			  <input type="checkbox" name="otherProduct" id="otherProduct" value="1" >
			...
			<input type='hidden' name='purchaseid' value='<?php echo $purchaseid; ?>' />
			<input type='hidden' name='category' value='<?php echo $category; ?>' />
			<button class='oneClick cta4' name='oneClick' type="submit" style="margin-top: 76px;"><?php echo $lang['addremove-remove']; ?></button>
		</form>
	</div>
</div>
...
<div class="main_box_title"><span>Almacen a almacen</span></div>
<div class="boxcontent">
	<form id="registerForm1" action="" method="POST">
			<span class="smallgreen"><?php echo $lang['global-source']; ?>:</span>
			...
			<br />
			<span class="smallgreen"><?php echo $lang['addremove-destination']; ?>:</span>
			<select class="fakeInput defaultinput" name="movementTypeid2">
				<option value=""><?php echo $lang['addremove-pleaseselect']; ?>:</option>
			<?php
					// Query to look up reasons for removing:
					if ($_SESSION['lang'] == 'es') {
