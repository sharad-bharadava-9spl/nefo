...
<?php
	...
	$objPHPExcel->getActiveSheet()
	            ->setCellValue('J1',$lang['pur-inmenu']);
	$objPHPExcel->getActiveSheet()->getStyle('J1')->getFont()->setBold(true); 
	$objPHPExcel->getActiveSheet()
	            ->setCellValue('K1',$lang['pur-closedat']);
	$objPHPExcel->getActiveSheet()->getStyle('K1')->getFont()->setBold(true); 
	$objPHPExcel->getActiveSheet()
	            ->setCellValue('L1','Cerrado en');
	$objPHPExcel->getActiveSheet()->getStyle('L1')->getFont()->setBold(true);  
?>
<!-- KONSTANT CODE UPDATE END -->
<center>
 <a href="bar-new-purchase.php" class="cta1"><?php echo $lang['newpurchase']; ?></a> 
 <br><br><br><br>

	 <table class="default" id="mainTable">
	  <thead>
	   <tr style='cursor: pointer;'>
	    <th style='position: relative;'><a href="#" id="xllink" ><img src="images/excel-new.png" style='position: absolute; top: 0; left: 10px; margin-top: -66px;' value="Export to Excel"/></a><?php echo $lang['pur-date']; ?></th>
	...
	$purchase_row =	sprintf("
  	  <tr>
  	   ...
  	   <td class='clickableRow right' href='bar-purchase.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> {$_SESSION['currencyoperator']} /u</span></td>
  	   <td class='clickableRow right' href='bar-purchase.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> {$_SESSION['currencyoperator']} /u</span></td>
  	   <td class='clickableRow right' href='bar-purchase.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> {$_SESSION['currencyoperator']} /u</span></td>
  	   <td class='clickableRow right' href='bar-purchase.php?purchaseid=%d'>%0.0f u</td>
	  ...
	  echo $purchase_row;
		    ...
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('B'.$startIndex, $purchaseCategory);
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('C'.$startIndex, $name);
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('D'.$startIndex, $purchase['purchaseQuantity'].' u'); 
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('E'.$startIndex, $purchase['purchasePrice'].' '.$_SESSION['currencyoperator'].' / u'); 
		   $objPHPExcel->getActiveSheet()
		                ->setCellValue('F'.$startIndex, $purchase['salesPrice'] - $purchase['purchasePrice'].' '.$_SESSION['currencyoperator'].' / u'); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('G'.$startIndex, $purchase['salesPrice']); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('H'.$startIndex, $estStock); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('I'.$startIndex, $inMenu); 
	        $objPHPExcel->getActiveSheet()
	       				 ->setCellValue('J'.$startIndex, $inMenu); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('K'.$startIndex, ''); 
	       $objPHPExcel->getActiveSheet()
	        			->setCellValue('L'.$startIndex, $purchase['adminComment']); 
	    

	   $startIndex++; 
}
	  $startIndex2 = $openRows + 2;
	  		// Closed products  		
	  ...
	  echo $purchase_row;
	  	  	  	// KONSTANT CODE UPDATE BEGIN
	  		$objPHPExcel->getActiveSheet()
		                ->setCellValue('A'.$startIndex2, $formattedDate);
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('B'.$startIndex2, $purchaseCategory);
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('C'.$startIndex2, $name);
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('D'.$startIndex2, $purchase['purchaseQuantity'].' u'); 
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('E'.$startIndex2, $purchase['purchasePrice'].' '.$_SESSION['currencyoperator'] .'/ u'); 
		   $objPHPExcel->getActiveSheet()
		                ->setCellValue('F'.$startIndex2, $purchase['salesPrice'] - $purchase['purchasePrice'].' '.$_SESSION['currencyoperator'].' / u'); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('G'.$startIndex2, $purchase['salesPrice']); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('H'.$startIndex2, $inMenu); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('I'.$startIndex2, $closedAt); 
	        $objPHPExcel->getActiveSheet()
	       				 ->setCellValue('J'.$startIndex2, $inMenu); 
	        $objPHPExcel->getActiveSheet()
	        			->setCellValue('K'.$startIndex2, date("d-m-Y", strtotime($purchase['closingDate']))); 
	       $objPHPExcel->getActiveSheet()
	        			->setCellValue('L'.$startIndex2, $purchase['closingComment']); 
 ...
 if(isset($_GET['action'])){
    ...
    header('Content-Disposition: attachment;filename=bar-purchases.xlsx');
    header("Content-Type: application/download");
    //header('Cache-Control: max-age = 0');
    $objWriter->save('php://output');
    header("location:bar-purchases.php");  // KONSTANT CODE UPDATE END 
    die;
}	
?>
...
<script type="text/javascript">
	$("#xllink").click(function(){
	    $("#load").show();
	    window.location.href = "bar-purchases.php?action=xls"; 
	    setTimeout(function () {
	        $("#load").hide();
	    }, 2000);     
	 });
