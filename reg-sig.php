<?php
	...
	if (isset($_POST['searchfield'])) {
		...
		$selectUsers = "SELECT u.user_id, u.memberno, u.first_name, u.last_name, u.registeredSince, u.paidUntil, u.userGroup, ug.groupName, u.credit, u.oldNumber, u.exento FROM users u, usergroups ug WHERE u.userGroup = ug.userGroup AND (u.first_name LIKE ('%$phrase%') OR u.last_name LIKE ('%$phrase%') OR u.oldNumber LIKE ('%$phrase%') OR u.memberno LIKE ('%$phrase%') OR u.cardid LIKE ('%$phrase%')) ORDER by u.memberno ASC";
...
<?php } ?>
	...
	echo sprintf("
  	  <tr>
  	   <td class='clickableRow' href='reg-sig-2.php?user_id=%d'>%s</td>
  	   <td class='clickableRow' href='reg-sig-2.php?user_id=%d'>%s</td>
  	   <td class='clickableRow' href='reg-sig-2.php?user_id=%d'>%s</td>
  	   <td class='clickableRow' href='reg-sig-2.php?user_id=%d'>%s</td>",
...
if ($_SESSION['creditOrDirect'] == 1) {
	...
	echo sprintf("
  	   <td class='clickableRow right' href='reg-sig-2.php?user_id=%d'>%0.1f ".$_SESSION['currencyoperator']."</td>",
...
if ($_SESSION['membershipFees'] == 1) {
	...
	echo sprintf("<td class='clickableRow %s' href='reg-sig-2.php?user_id=%d'>%s</td>",
...
/*	echo sprintf("
	...
	echo sprintf("
	   <td style='text-align: center;' class='clickableRow %s' href='reg-sig-2.php?user_id=%d'>%s</td>
  	   <td style='text-align: center;' class='clickableRow %s' href='reg-sig-2.php?user_id=%d'>%s</td>
  	   <td style='text-align: center; color: red;' class='clickableRow' href='reg-sig-2.php?user_id=%d'>%s</td>",
	  $form1colour, $user['user_id'], $form1, $dnicolour, $user['user_id'], $dniScan, $user['user_id'], $oldNumber);
	  
	  
	echo sprintf("
  	   <td style='text-align: center;' class='clickableRow' href='reg-sig-2.php?user_id=%d&openComment'>%s</td>
	  </tr>",
