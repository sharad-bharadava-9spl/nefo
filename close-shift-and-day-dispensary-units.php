<?php
	...
	$checkOpening = "SELECT dis2Closed, dis2ClosedBy, dayClosedNo FROM opening WHERE openingid = $dayopeningid";
		try
		{
	...
	if ($dis2Closed == '2' && (!isset($_GET['redo']))) {
		pageStart($lang['close-shift-and-day'], NULL, $validationScript, "pcloseday", "step2 dev-align-center", $lang['closeday-dis-one'] . " - ERROR", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
EOD;
	exit();
		...
		if ($dayClosedNo > 0) {
			...
			$query = "DELETE from closingdetails WHERE closingid = $dayClosedNo AND categoryType = 1";
		try
		{
		...
		if ($shiftClosedNo > 0) {
			...
			$query = "DELETE from shiftclosedetails WHERE closingid = $shiftClosedNo AND categoryType = 1";
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
...
EOD;
	...
	pageStart($lang['close-shift-and-day'], NULL, $confirmLeave, "pcloseday", "dev-align-center", $lang['closeday-dis-one'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
?>
	...
	<form onsubmit='oneClick.disabled = true; return true;' id="skipWeightForm" action="close-shift-and-day-dispensary-units-1.php" method="POST">

<?php

	echo "<br /><button name='oneClick' type='submit' class='buttonCTA cta1' style='width: auto;'   formnovalidate='formnovalidate'>{$lang['dont-count']}</button>";
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND ((closedAt IS NULL) OR (closingDate BETWEEN '$openingtime' AND '$closingtime'))";
		...
		if ($catType == 0) {
			...
			echo "<h5 class='title'>$catName</h3><div class='productboxwrap'>";
		...
		while ($flower = $resultFlower->fetch()) {
				...
				if ($flower['closedAt'] != NULL) {
					...
					$flower_row = sprintf("
		<div class='actionbox-np2 fixed-width-box'><br>
	<div class='boxcontent'>
				<div class='mainboxheader'>%s (%s)</div><br>
					<div class='dev_box'>
	<input type='number' lang='nb' name='daycloseProduct[%d][weight]' class='fourDigit defaultinput' placeholder='g' step='0.01' value='0' disabled required style='color: red;' /><br />
  	   ...
  	   <input type='hidden' name='daycloseProduct[%d][inMenu]' value='%d' /></div></div></div>",
...
?>
	...
	<form onsubmit='oneClick.disabled = true; return true;' id="registerForm" action="close-shift-and-day-dispensary-units-1.php" method="POST">
...
<?php
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND ((closedAt IS NULL) OR (closingDate BETWEEN '$openingtime' AND '$closingtime'))";
		...
		if ($catType == 0) {
			...
			echo "<h5 class='title'>$catName<div class='fakeboxholder' style='float: right;'> <label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}<input type='checkbox' style='width: 10px;' id='auto$category' /> <div class='fakebox'></div><label></div></h5><div class='productboxwrap'>";
	...
	<div class='actionbox-np2 fixed-width-box'><br>
			...
			<div class='boxcontent'>
				<div class='mainboxheader'>%s (%s)</div><br>
					<div class='dev_box'>
	    <input type='number' lang='nb' name='daycloseProduct[%d][weight]' class='fourDigit defaultinput prequired$category' placeholder='u' step='0.01' id='required%d' required /><br />
...
?>
	 ...
	 <input type='hidden' name='closingConfirm' value='yes' />
 	 <br /><button name='oneClick' type="submit" class="cta1 customButton"><?php echo $lang['global-nextstep']; ?> &raquo;</button>
