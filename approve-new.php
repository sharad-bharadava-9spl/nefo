<?php
	...
	try
	{
		$result = $pdo3->prepare("SELECT u.user_id, u.memberno, u.registeredSince, u.first_name, u.last_name, u.email, u.day, u.month, u.year, u.nationality, u.gender, u.dni, u.street, u.streetnumber, u.flat, u.postcode, u.city, u.country, u.telephone, u.mconsumption, u.usageType, u.signupsource, u.cardid, u.photoid, u.docid, u.doorAccess, u.friend, u.friend2, u.paidUntil, u.adminComment, ug.userGroup, ug.groupName, ug.groupDesc, u.form1, u.form2, datediff(curdate(), u.registeredSince) AS daysMember, u.paymentWarning, u.paymentWarningDate, u.credit, u.banComment, u.creditEligible, u.dniscan, u.discount, u.discountBar, u.photoext, u.dniext1, u.dniext2, u.workStation, u.bajaDate, u.starCat, u.interview, u.exento, u.fptemplate1, u.usergroup2, u.sigext, u.instagram, u.coupon FROM users u, usergroups ug WHERE u.userGroup = ug.userGroup AND u.user_id = '{$user_id}'");
		$result->execute();
	}
...
<center>
 <span class="secondbuttons">
  <a href="dab-approve.php?user_id=<?php echo $user_id;?>" class="cta">Approve</a>
  <a href="dab-modify.php?user_id=<?php echo $user_id;?>" class="cta">Request missing info</a>
  <a href="dab-reject.php?user_id=<?php echo $user_id;?>" class="cta" style="background-color: red;">Reject</a>
  <a href="uTil/delete-user.php?user_id=<?php echo $user_id;?>" class="cta" style="background-color: red;">Delete</a>
 </span>
</center>

<div class="overview">
 <span class="profilepicholder" style='position: relative;'><a href="new-picture.php?user_id=<?php echo $user_id; ?>"><img class="profilepic" src="<?php echo $memberPhoto; ?>" /></a>
 <?php echo $deletePhoto; ?>
...
<?php 
	...
	if ($userGroup == 11) {
		...
		echo "<span style='color: red;'>$groupName</span>";
		
	} else if ($userGroup == 5 && $_SESSION['membershipFees'] == 1 && $exento == 0) {  // show Member w/ expiry
...
/*	if (!file_exists($file)) {
	...
	try{
		$fileinfo =  getFileInfo($google_bucket, $google_root_folder.'images/_' . $_SESSION['domain'] . '/ID/' . $user_id . '-front.' . $dniext1);
...
/*	if (!file_exists($file3)) {
	...
	try{
		$fileinfo3 =  getFileInfo($google_bucket, $google_root_folder.'images/_' . $_SESSION['domain'] . '/sigs/' . $user_id . ".$sigext");
		
	}catch(Exception $e){
		echo "<br /><a href='new-signature.php?user_id=$user_id&mconsumption=$mconsumption&usageType=$usageType'><img src='images/exclamation-22.png' class='warningIcon' style='margin-bottom: -4px; margin-left: 7px; margin-right: 6px;' /><span class='yellow'>" . $lang['signature-missing'] . "</span></a>";
	}
	if ($coupon != '') {
    	echo "<br /><img src='images/exclamation-22.png' class='warningIcon' style='margin-bottom: -4px; margin-left: 7px; margin-right: 6px;' /><span class='yellow'>Coupon code used! Code: <strong>$coupon</strong></span>";
	}
...
echo "</table></div>";
	...
	if ($adminComment != '') {
		echo "<br /><br /><strong>Member comment:</strong><br />$adminComment";
	}
...
<?php
	...
	if ($friend > 0 && $friend2 > 0) {
		...
		$object_exist1 = object_exist($google_bucket, $google_root_folder.$fileF1);
					
 		echo "<div style='width: 50%; float: left;'><a href='aval-details.php?user_id=$friend&chain=true' class='white'>";
		
		if ($object_exist1) {
			echo "<img src='{$google_root}$fileF1' height='75' /><br />";
		} else {
			echo "<img src='{$google_root}images/silhouette.png' height='75' /><br />";
		}
		...
		$object_exist2 = object_exist($google_bucket, $google_root_folder.$fileF2);
					
 		echo "<div style='width: 50%; float: left;'><a href='aval-details.php?user_id=$friend2&chain=true' class='white'>";
		
		if ($object_exist2) {
			echo "<img src='{$google_root}$fileF2' height='75' /><br />";
		} else {
			echo "<img src='{$google_root}images/silhouette.png' height='75' /><br />";
		}
...
<?php	
	/*if (file_exists($file)) {
		echo "<a href='images/_$domain/ID/" . $user_id . "-front." . $dniext1 . "'><img src='images/dni-iconbig.png' style='margin-top: 10px;'/></a> <a href='new-id-scan-front.php?user_id=$user_id'><img src='images/edit-dnibig.png' style='display: inline-block; margin-left: 5px; margin-bottom: 2px;' /></a><a href='uTil/delete-dni-front.php?user_id=$user_id'><img src='images/delete.png' width='15' style='display: inline-block; margin-left: 10px; margin-bottom: 4px;' /></a> <br />";
	} else {
...
/*	if (file_exists($file2)) {
		echo "<a href='images/_$domain/ID/" . $user_id . "-back." . $dniext2 . "'><img src='images/dni-icon-2big.png' style='margin-top: 10px;' /></a> <a href='new-id-scan-back.php?user_id=$user_id'><img src='images/edit-dnibig.png' style='display: inline-block; margin-left: 5px; margin-bottom: 2px;' /></a><a href='uTil/delete-dni-back.php?user_id=$user_id'><img src='images/delete.png' width='15' style='display: inline-block; margin-left: 10px; margin-bottom: 4px;' /></a>  <br />";
	} else {
	...
	try{
		$fileinfo2 =  getFileInfo($google_bucket, $google_root_folder.'images/_' . $_SESSION['domain'] . '/ID/' . $user_id . '-back.' . $dniext2);
		echo "<a href='images/_$domain/ID/" . $user_id . "-back." . $dniext2 . "'><img src='images/dni-icon-2big.png' style='margin-top: 10px;' /></a> <a href='new-id-scan-back.php?user_id=$user_id'><img src='images/edit-dnibig.png' style='display: inline-block; margin-left: 5px; margin-bottom: 2px;' /></a><a href='uTil/delete-dni-back.php?user_id=$user_id'><img src='images/delete.png' width='15' style='display: inline-block; margin-left: 10px; margin-bottom: 4px;' /></a>  <br />";
		
	}catch(Exception $e){
		echo "<img src='images/dni-icon-na-2big.png' style='margin-top: 10px;' /> <a href='new-id-scan-back.php?user_id=$user_id'> <img src='images/edit-dnibig.png' style='display: inline-block; margin-left: 7px; margin-bottom: 2px;' /></a> <br />";
	}
	$object_exist = object_exist($google_bucket, $google_root_folder.$file3);

	if ($object_exist) {
		echo "<a href='{$google_root}images/_$domain/sigs/" . $user_id . ".$sigext'><img src='images/sig-iconbig.png' style='margin-top: 10px;' /></a> <a href='new-signature.php?user_id=$user_id&mconsumption=$mconsumption&usageType=$usageType'><img src='images/edit-dnibig.png' style='display: inline-block; margin-left: 5px; margin-bottom: 2px;' /></a><a href='uTil/delete-sig.php?user_id=$user_id'><img src='images/delete.png' width='15' style='display: inline-block; margin-left: 10px; margin-bottom: 4px;' /></a> <br />";
	} else {
...
Instagram: <?php echo $instagram; ?><br /><br />
...
// Separate methodologies and row displays (linkage) for donations vs sales. Donations next:
		...
		<td class='clickableRow right' href='donation-management.php?userid={$userid}'><strong>{$amount} <span class='smallerfont'>{$_SESSION['currencyoperator']}</span></strong></td>
		<td class='clickable}Row right' href='donation-management.php?userid={$userid}'>{$credit} {$_SESSION['currencyoperator']}</td>
		<td class='clickableRow right' href='donation-management.php?userid={$userid}'>{$newcredit} {$_SESSION['currencyoperator']}</td>
...
<a href="?pageno=<?php echo $total_pages; echo $sortparam . '&user_id=' . $user_id; ?>" class='pagination <?php if ($total_pages == $pageno){ echo 'disabled'; } ?>'>&raquo;</a><br />
<a href="?view=all&user_id=<?php echo $user_id; ?>" class='pagination'>TODO</a>
