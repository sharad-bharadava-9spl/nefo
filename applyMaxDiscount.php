...
function checkMaxDisocunt($user_id, $purchaseid, $categoryId, $salesPrice, $qty=0, $amount=0, $offsetSec = 0){
		...
		$selectPurchaseMedicalDiscount = "SELECT medDiscount FROM purchases WHERE purchaseid = $purchaseid";
		try
		{
		...
		$rowGUD = $result->fetch();
			...
			$discountArr[2]['discount'] = $userDisocunt;
			$discountArr[2]['discountType'] =1;

		// Look up category dicount for user

		$selectUserCategoryDiscount = "SELECT discount FROM catdiscounts WHERE user_id = $user_id AND categoryid = $categoryId";
		try
		{
		...
		$selectUserPurchaseDiscount = "SELECT discount, fijo FROM inddiscounts WHERE user_id = $user_id AND purchaseid = $purchaseid";
		try
		{
		...
		$rowGUGD = $result->fetch();
	        ...
	        $usergroup_discount_price = $rowGUGD['discount_price'];
	        $usergroup_discount_percentage = $rowGUGD['discount_percentage'];
	        $usergroup_dicount_fixed =  number_format((($usergroup_discount_price * 100) / $salesPrice),2);
	        //usergroup_dicount
	        $discountArr[6]['discount'] = $rowGUGD['discount_percentage'] == '' ? 0 : $rowGUGD['discount_percentage'];
	     ...
	     if($usergroup_discount_id){
	       // Look Up category discount for usergroup
		     $selectUgCategoryDiscount = "SELECT discount FROM catdiscounts WHERE usergroup_discount_id = $usergroup_discount_id AND categoryid = $categoryId";
			try
			{
			...
			$selectUgPurchaseDiscount = "SELECT discount, fijo FROM inddiscounts WHERE usergroup_discount_id = $usergroup_discount_id AND purchaseid = $purchaseid";
			try
			{
		...
		$hHDiscountEveryday = "SELECT * FROM global_happy_hour_discounts where discount_date = 'Every day' and time_from <= '".$getTime."' and time_to > '".$getTime."'";
		    ...
		    while ($hHDiscount = $result->fetch()) {
		        ...
		        if(isset($hHDiscount['discount']) && $hHDiscount['discount'] > 0){
		            ...
		            $discountArr[11]['discount'] = $hHDiscount['discount'];
		...
		$hHDiscount = "SELECT * FROM global_happy_hour_discounts where discount_date = '$getDay' and time_from <= '".$getTime."' and time_to > '".$getTime."'";
	        ...
	        while ($hHDiscount = $result->fetch()) {
	            ...
	            if(isset($hHDiscount['discount']) && $hHDiscount['discount'] > 0){
	                ...
	                $discountArr[12]['discount'] = $hHDiscount['discount'];
	    ...
	    if($happy_hour_id){
	    	...
	    	$selectCategoryHHDiscount = "SELECT discount FROM catdiscounts WHERE categoryid = $categoryId and happy_hour_id = ".$happy_hour_id;
	            try
	            {
	        ...
	        $selectPurchaseHHDiscount = "SELECT discount, fijo FROM inddiscounts WHERE purchaseid = $purchaseid and happy_hour_id = $happy_hour_id";
	            try
	            {
...
echo checkMaxDisocunt($user_id, $purchaseid, $categoryId, $salesPrice);*/