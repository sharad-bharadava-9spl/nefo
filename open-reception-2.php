<?php
	...
	if ($_SESSION['firstOpening'] == 'true') {
		...
		// Grab opening ID
		$openingLookup = "SELECT openingid FROM recopening ORDER BY openingtime DESC LIMIT 1";
		try
		{
		...
		if (!$data) {
			...
			$query = sprintf("INSERT INTO recopening (openingtime, oneCent, twoCent, fiveCent, tenCent, twentyCent, fiftyCent, oneEuro, twoEuro, fiveEuro, tenEuro, twentyEuro, fiftyEuro, hundredEuro, coinsTot, notesTot, tillBalance, bankBalance, tillDelta, tillComment) VALUES ('%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s');",
			  $openingtime, $openCount['oneCent'], $openCount['twoCent'], $openCount['fiveCent'], $openCount['tenCent'], $openCount['twentyCent'], $openCount['fiftyCent'], $openCount['oneEuro'], $openCount['twoEuro'], $openCount['fiveEuro'], $openCount['tenEuro'], $openCount['twentyEuro'], $openCount['fiftyEuro'], $openCount['hundredEuro'], $openCount['coinsTot'], $openCount['notesTot'], $openCount['tillTot'], $bankBalance, $tillDelta, $openCount['tillComment']);
		try
		{
		...
		catch (PDOException $e)
		{
			...
			$query = sprintf("UPDATE recopening SET openingtime = '%s', oneCent = '%d', twoCent = '%d', fiveCent = '%d', tenCent = '%d', twentyCent = '%d', fiftyCent = '%d', oneEuro = '%d', twoEuro = '%d', fiveEuro = '%d', tenEuro = '%d', twentyEuro = '%d', fiftyEuro = '%d', hundredEuro = '%d', coinsTot = '%f', notesTot = '%f', tillBalance = '%f', bankBalance = '%f', tillDelta = '%f', tillComment = '%s' WHERE openingid = '%d';",
			  $openingtime, $openCount['oneCent'], $openCount['twoCent'], $openCount['fiveCent'], $openCount['tenCent'], $openCount['twentyCent'], $openCount['fiftyCent'], $openCount['oneEuro'], $openCount['twoEuro'], $openCount['fiveEuro'], $openCount['tenEuro'], $openCount['twentyEuro'], $openCount['fiftyEuro'], $openCount['hundredEuro'], $openCount['coinsTot'], $openCount['notesTot'], $openCount['tillTot'], $bankBalance, $tillDelta, $openCount['tillComment'], $openingid);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
		...
		if ($_SESSION['noCompare'] != 'true') {
			...
			$updateClosing = sprintf("UPDATE recopening SET firstDayOpen = 2, openingtime = '%s', openedby = '%d' WHERE openingid = '%d';",
				$timeNow, $_SESSION['user_id'], $openingid
				);
