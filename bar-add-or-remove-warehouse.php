<?php
	...
	if ($_POST['productAdd'] == 'complete') {
		...
		// Query to add new purchase movement - 6 arguments
		  $query = sprintf("INSERT INTO b_productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, provider, price, paid, priceg, stashMovementType) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%f', '%f', '%f', '%d');",
		  $movementtime, '1', $purchaseid, $quantity, $movementTypeid, $comment, $provider, $price, $paid, $priceg, $stashMovementType);
		try
		{
...
<?php 
if(isset($_GET['internal'])){
	$availStash = "Internal Stash (".$inStashInt." u)";
}else if(isset($_GET['external'])){
	$availStash = "External Stash (".$inStashExt." u)";
}
  ?><br>
  <div id="productoverview"><?php echo $availStash; ?></div>
  <br>
...
<?php
		...
		if ($_SESSION['lang'] == 'es') {
			$movementType = "SELECT movementTypeid, movementNamees FROM productmovementtypes WHERE type = 2 AND (movementTypeid < 16 OR movementTypeid > 27) AND movementTypeid <> 13 AND movementTypeid <> 14 AND movementTypeid !=9  AND movementTypeid NOT IN(5,6) ORDER BY movementNamees ASC";
		try
		{
			$result = $pdo3->prepare("$movementType");
			$result->execute();
		...
		while ($user = $result->fetch()) {
			...
			$movementType = "SELECT movementTypeid, movementNameen FROM productmovementtypes WHERE type = 2 AND (movementTypeid < 16 OR movementTypeid > 27) AND movementTypeid <> 13 AND movementTypeid <> 14 AND movementTypeid !=9 AND movementTypeid NOT IN(5,6) ORDER BY movementNameen ASC";
		try
		{
			$result = $pdo3->prepare("$movementType");
			$result->execute();
...
?>
  ...
  <br />
 <textarea name="comment" class="defaultinput" placeholder="<?php echo $lang['global-comment']; ?>?"></textarea>
 <input type='hidden' name='productRemove' value='complete' />
  <input type='hidden' name='purchaseid' value='<?php echo $purchaseid; ?>' /><br>
