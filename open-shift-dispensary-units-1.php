<?php
	...
	if ($_POST['step3'] == 'complete') {
		...
		if ($dayOpenedNo > 0) {
			...
			$query = sprintf("UPDATE shiftopen SET openingtime = '%s' WHERE openingid = '%d'",
		...
		catch (PDOException $e)
		{
				...
				$updateClosing = sprintf("UPDATE shiftclose SET dis2Opened = 2, dis2OpenedAt = '%s' WHERE closingid = '%d';",
					$timeNow, $closingid
					);
		try
		{
		...
		catch (PDOException $e)
		{
				...
				$timeNow = date('Y-m-d H:i:s');
				$updateClosing = sprintf("UPDATE shiftclose SET dis2Opened = 2, shiftOpenedNo = '%d', dis2OpenedAt = '%s' WHERE closingid = '%d';",
					$openingid,
					$timeNow, $closingid
					);
		try
...
EOD;
	...
	pageStart($lang['title-openday'], NULL, $confirmLeave, "pcloseday", "step1 dev-align-center", $lang['openday-dis2-two'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
	...
	foreach($catArray as $value) {
		...
		// Query to look up yesterday's closing balance
			...
			$closingLookup = "SELECT d.weight FROM shiftclosedetails d, shiftclose c WHERE category = $category AND purchaseid = $purchaseid and d.closingid = c.closingid ORDER BY c.closingtime DESC LIMIT 1";
		try
		{
...
<script>
   ...
   function compute() {
		...
		<table class='purchasetable'>
		 ...
		 <tr>
		  <td>{$lang['countnow']}
		  	<input type='number' lang='nb' name='confirmedOpen[%d][weight]' id='fullWeight%d' class='fourDigit purchaseNumber' value='%0.02f' step='0.01' /></td>
		 </tr>
		 <tr>
		...
		<br />
		 ...
		 <textarea class='defaultinput' name='confirmedOpen[%d][prodopencomment]'></textarea>
		 </div>
	...
	// End loop for each product
		...
		echo "<input type='hidden' name='step3' value='complete' />";
 		echo "<br /><br /><button type='submit' name='oneClick' class='customButton cta1'>{$lang['global-confirm']}</button>";
		echo "</form>";
		echo "</div>";
		## FORM INPUT END ##
