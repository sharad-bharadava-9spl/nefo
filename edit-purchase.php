<?php
		...
		catch (PDOException $e)
		{
                ...
                for($i = 0; $i < count($_POST['volume_unit']); $i++) {
                    if(!empty($_POST['volume_unit'][$i])){
                        ...
                        $addDiscountB = sprintf("INSERT INTO volume_discounts (purchaseid, units, amount) VALUES ('%d', '%f', '%f');",
	...
	if ($_SESSION['domain'] == 'dabulance') {
		...
		$updatePurchase = sprintf("UPDATE purchases SET purchasePrice = '%f', salesPrice = '%f', salesPrice2 = '%f', purchaseQuantity = '%f', realQuantity = '%f', adminComment = '%s', closingComment = '%s', inMenu = '%d', growType = '%d', tupperWeight = '%f', provider = '%d', medDiscount = '%d', barCode = '%s', paid = '%f' WHERE purchaseid = $purchaseid;",
			$purchasePrice,
			$salesPrice,
			$salesPrice2,
			$purchaseQuantity,
...
EOD;
	...
	if ($_SESSION['domain'] == 'dabulance') {
		...
		$purchaseDetails = "SELECT category, productid, purchaseDate, purchasePrice, salesPrice, salesPrice2, purchaseQuantity, realQuantity, adminComment, estClosing, closingComment, closedAt, inMenu, growType, tupperWeight, provider, medDiscount, barCode, paid FROM purchases WHERE purchaseid = $purchaseid";
	} else {
		// Query to look for purchase
	...
	pageStart($lang['title-editpurchase'], NULL, $validationScript, "ppurchase", "admin dev-align-center", $lang['admin-editpurchase'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
	...
	$(document).ready(function() {
        ...
        function addMoreDiscount(){
            if(btncount < 11){
                ...
                $("#volumeDiv").after('<tr><td>'+ unitName +'<input type="number" lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" name="volume_unit[]" /> </td><td>'+ unitPrice + '<input type="number" lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" name="volume_unit_price[]" /><?php echo  $_SESSION['currencyoperator'] ?></td></tr><br>');
            }else{
...
<div class="actionbox-np2" style='height: 308px;'>
...
<div class="actionbox-np2" style='height: 308px;'>
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="left"><?php echo $lang['add-pergram']; ?><input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" id='purchaseppg' name='purchaseppg' placeholder="<?php echo $_SESSION['currencyoperator'] ?>" value="<?php echo $purchasePrice; ?>" /></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?> <input type='text' lang="nb" id="purchasePrice" name="purchasePrice" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" value="<?php echo $purchasePriceTotal; ?>" /></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['paid']; ?>: <input type='text' lang="nb" id="paidNow" name="paidNow" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" value="<?php echo $paidNow; ?>" /></td>
   </tr>
  </table>
</div>
</div>
...
<div class="actionbox-np2" style='height: 308px;'>
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="left"><?php echo $lang['add-pergram']; ?> <input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id='salesppg' name='salesppg' value="<?php echo $salesPrice; ?>" /></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?> <input type='text' lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id="salesPrice" name="salesPrice" value="<?php echo $salesPriceTotal; ?>"/></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?>
    <input type='text' lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id="salesPrice" name="salesPrice" value="<?php echo $salesPriceTotal; ?>"/></td>
   </tr>
   <tr>
  ...
  <!-- // KONSTANT CODE UPDATE BEGIN -->
<div class="actionbox-np2" style='min-height: 268px;'>
 <div class='mainboxheader'>
...
<?php 
    ...
    <tr>
        <td class="left"><?php if ($category < 3 || $type == 1) { echo $lang['number-grams']; } else { echo $lang['units-grams']; } ?>
        <input type='number' lang='nb' class='fourDigit defaultinput-no-margin-smallborder floatright' name='volume_unit[]' value="<?php echo $rs['units'];?>" /></td>
  
        <td><?php echo $lang['add-total']; ?>
        <input type="number" lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" name="volume_unit_price[]" value="<?php echo $rs['amount'];?>" /> <?php echo $_SESSION['currencyoperator'] ?></td>
    </tr>
        
<?php } ?>      
   <tr id="volumeDiv">
        <td class="left"><?php if ($category < 3 || $type == 1) { echo $lang['number-grams']; } else { echo $lang['units-grams']; } ?>
    <input type='number' lang='nb' class='fourDigit defaultinput-no-margin-smallborder floatright' name='volume_unit[]' /></td>
	
    <td><?php echo $lang['add-total']; ?>
    <input type="number" lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" name="volume_unit_price[]" /> <?php echo $_SESSION['currencyoperator'] ?></td>
   <br>
   </tr>
...
<div class="actionbox-np2" style='height: 268px;'>
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="biggerFont"><?php echo $lang['global-quantity']; ?> <input type='text' lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" id="purchaseQuantity" name="purchaseQuantity" placeholder="u" value='<?php echo $purchaseQuantity; ?>'></td>
    <td><?php echo $lang['provider']; ?> <select name='provider' class='defaultinput-no-margin-smallborder floatright'>
      <option value='<?php echo $provider; ?>'><?php echo $providerName; ?></option>
<?php
	$PRquery = "SELECT id, name FROM providers WHERE id <> $provider";
...
<div class="actionbox-np2">
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="left"><?php echo $lang['add-perunit']; ?><input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" id='purchaseppg' name='purchaseppg' placeholder="<?php echo $_SESSION['currencyoperator'] ?>" value="<?php echo $purchasePrice; ?>" /></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?> <input type='text' lang="nb" id="purchasePrice" name="purchasePrice" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" value="<?php echo $purchasePriceTotal; ?>" /></td>
   </tr>
...
<?php
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="left">Retail <input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id='salesppg' name='salesppg' value="<?php echo $salesPrice; ?>" /></td>
   </tr>
   <tr>
    <td class="left">Wholesale <input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id='salesppg2' name='salesppg2' value="<?php echo $salesPrice2; ?>" /></td>
   </tr>
<?php
	} else { ?>
 ...
 <div class='boxcontent'>
  <table class='np-table'>
   <tr>
    <td class="left"><?php echo $lang['add-perunit']; ?> <input type='text' lang='nb' class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id='salesppg' name='salesppg' value="<?php echo $salesPrice; ?>" /></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?> <input type='text' lang="nb" class="fourDigit defaultinput-no-margin-smallborder floatright" placeholder="<?php echo $_SESSION['currencyoperator'] ?>" id="salesPrice" name="salesPrice" value="<?php echo $salesPriceTotal; ?>" /></td>
   </tr>
...
<?php } ?>      
   ...
   <br>
           ...
           <button type="button" class="cta1" onclick="addMoreDiscount()" id="addMoreDiscountButton"><?php echo $lang['add-more']; ?></button> 
