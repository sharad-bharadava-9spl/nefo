<?php
	...
	if (isset($_GET['reopen'])) {
		...
		pageStart($lang['title-product'], NULL, $deleteUserScript, "ppurchase", "admin dev-align-center", "Product", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
EOD;
	...
	if ($_SESSION['domain'] == 'dabulance') {
		...
		$purchaseDetails = "SELECT category, productid, purchaseDate, purchasePrice, salesPrice, salesPrice2, purchaseQuantity, realQuantity, adminComment, estClosing, closingComment, closedAt, inMenu, growType, closingDate, photoExt, tupperWeight, medDiscount FROM purchases WHERE purchaseid = $purchaseid";
	} else {
		// Query to look for purchase
	...
	$closeDiff = $closedAt - $estClosing;
		...
		$selectProducts = "SELECT name, breed2 from products WHERE productid = $productid";
		try
		{
...
EOD;
		pageStart($lang['purchase'], NULL, $deleteUserScript, "ppurchase", "admin ppage dev-align-center", $lang['purchase'], $_SESSION['successMessage'], $_SESSION['errorMessage']); 
	...
	$selectUnStashedInt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 1 AND (movementTypeid = 12 OR movementTypeid = 17)";
		...
		$_SESSION['internalStash'] = $inStashInt;
	...
	$selectUnStashedExt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 1 AND (movementTypeid = 2 OR movementTypeid = 19)";
		...
		$_SESSION['externalStash'] = $inStashExt;
	...
	$selectRemovedFromDisplay = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 1 AND (movementTypeid = 3)";
		try
		{
	...
	//$imgname = "images/_$domain/purchases/$purchaseid.$photoExt";
		...
		$object_exist =  object_exist($google_bucket, $google_root_folder."images/_$domain/purchases/$purchaseid.$photoExt");
	if($object_exist){	
	  		$imgname = $google_root."images/_$domain/purchases/$purchaseid.$photoExt";

	}else{
		// echo 'Caught exception: ',  $e->getMessage(), "\n";
		if ($category == 1) {
			$imgname = $google_root."images/purchase-flower.png";
		} else if ($category == 2) {
			$imgname = $google_root."images/purchase-extract.png";
		} else {
			$imgname = $google_root."images/purchase-cannabis.png";
		}
...
<?php if ($category == '1' || $type == 1) { ?>
  ...
  <?php } ?>
  <tr>
   <td><?php echo $lang['purchase-date']; ?>:</td>
...
<?php
	if ($closedAt == NULL) {
		// KONSTANT CODE UPDATE BEGIN
		echo "<a href='add-remove-options.php?purchaseid=$purchaseid' class='cta1'><span class='ctatext'>{$lang['pur-addremove']}</span></a>";
		// KONSTANT CODE UPDATE END
...
<?php } ?>
<br />
<div class="actionbox-np2" style='height: 681px;'>
...
<?php
   ...
   <tr>
    <td class='biggerFont left'>{$lang['pur-closedat']}:<span class='purchaseNumber'>$closedAt g</span><br />$closingDate</td>
		...
		echo "
   <tr>
    <td colspan='2' style='padding-top: 8px; font-size: 16px;'>{$lang['pur-inmenu']}: <span class='$menuClass'><strong>$inMenu</strong></span></td>
   </tr>";
	}
?>
   <tr>
   ...
   <tr>
    <td class="biggerFont left"><?php echo $lang['member-discount']; ?> <img src="images/medical-15.png" /><span class="purchaseNumber"><?php echo $medDiscount; ?> %</span></td>
   </tr>
  </table>
 </div>
 </div>
...
<div class="actionbox-np2">
 ...
 <div class='boxcontent'>
  <table class='purchasetable'>
   <tr>
    <td class="left"><?php echo $lang['add-pergram']; ?><span class='purchaseNumber'><?php echo number_format($purchasePrice,2); ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?><span class='purchaseNumber'><?php echo $purchasePriceTotal; ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
   </tr>
  </table>
 </div>
 </div>
...
<div class="actionbox-np2">
 ...
 <div class='boxcontent'>
  <table class='purchasetable'>
   <tr>
    <td class="left"><?php echo $lang['add-pergram']; ?><span class='purchaseNumber'><?php echo number_format($salesPrice,2); ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?><span class='purchaseNumber'><?php echo $salesPriceTotal; ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
   </tr>
  </table>
 </div>
 </div>
 <br />
<!-- // KONSTANT CODE UPDATE BEGIN -->
<div class='actionbox-np2' style='width: 360px; min-height: 159px;'>
	    ...
	    <?php 
	        ...
	        <tr>
	         ...
	         <tr>   
	            ...
	            <span class="bigNumber purchaseNumber"><?php echo $rs['amount']; ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
	        </tr>
	    <?php } ?>      
	    </table>
	</div>
...
-->
  ...
  <br />
  	...
  	<div id="tabs-1">
		<?php 
			if (isset($_GET['summary'])) {
				...
				$selectMovements = "SELECT type, SUM(quantity), movementTypeid, user_id FROM productmovements WHERE purchaseid = $purchaseid AND stashMovementType = 0 GROUP BY movementTypeid ORDER by type DESC, movementTypeid ASC";
				
			} else {
				
				// Query to look up movements
				$selectMovements = "SELECT movementid, movementtime, type, purchaseid, quantity, movementTypeid, comment, price, user_id FROM productmovements WHERE purchaseid = $purchaseid AND stashMovementType = 0 ORDER by movementtime ASC";
			}
		  ...
		  <table class='defaultnobg'>
			  <tbody>
		  	  ...
		  	  <tr class='green'>
		  	   ...
		  	   <td style='text-align: right;'><?php echo number_format($purchasePrice,2); ?> <?php echo $_SESSION['currencyoperator'] ?> / g</td>
		  	   <td></td>
		  	   <td></td>
			  </tr>
			...
			$movementTypeid = $movement['movementTypeid'];
			$movementOperatorID = $movement['user_id'];
			$movementOperator = getUser($movementOperatorID);
			
			if ($movementTypeid == 1) {
				$editOrNot = "<a href='edit-movement.php?movementid=$movementid'><img src='images/edit.png' height='15' title='Edit' /></a>&nbsp;&nbsp;";
				$ppg = round($price / $quantity,2) . " ".$_SESSION['currencyoperator']." / g";
			} else {
				$editOrNot = "";
				$ppg = '';
			}
			...
			$movement_row =	sprintf("
			  ...
			  $rowclass, $formattedDate, $movementOperator, $movementName, $quantity, $ppg, $commentRead, $movementid, $purchaseid
			  );
			  echo $movement_row;
		  }
		?>
	...
	<div id="tabs-2">
			<?php 
				// Query to look up internal stash movements
				 $selectinternalMovements = "SELECT movementid, movementtime, type, purchaseid, quantity, movementTypeid, comment, price, user_id FROM productmovements WHERE purchaseid = ".$_GET['purchaseid']." AND stashMovementType = 1 ORDER by movementtime ASC";
				try
				{
					$result = $pdo3->prepare("$selectinternalMovements");
					$result->execute();
				}
				catch (PDOException $e)
				{
			...
			$movementOperator = getUser($movementOperatorID);
						...
						// Look up movement name
							if ($_SESSION['lang'] == 'es') {
								...
								$row = $result2->fetch();
										if($movementTypeid == 5){
											$movementName = 'Add to internal stash';
										}
										else if($movementTypeid == 12){
											$movementName = 'Remove from internal stash';
										}
										else{
											$movementName = $row['movementNameen'];
										}
	...
	<div id="tabs-3">
			<?php 
				// Query to look up extrenal stash movements
				$selectexternalMovements = "SELECT movementid, movementtime, type, purchaseid, quantity, movementTypeid, comment, price, user_id FROM productmovements WHERE purchaseid = ".$_GET['purchaseid']." AND stashMovementType = 2 ORDER by movementtime ASC";
				try
				{
					$result = $pdo3->prepare("$selectexternalMovements");
					$result->execute();
				}
				catch (PDOException $e)
				{
...
<div class="actionbox-np2">
 <div class='mainboxheader'>
  <center><?php echo $lang['global-dispenses']; ?></center>
 </div>
 <div class='boxcontent'>
...
<?php 
	...
	$selectSales = "SELECT s.saleid, s.saletime, s.userid, d.quantity, d.amount, s.operatorid FROM sales s, salesdetails d WHERE s.saleid = d.saleid AND d.purchaseid = $purchaseid ORDER BY s.saletime DESC";
		try
		{
			$result = $pdo3->prepare("$selectSales");
			$result->execute();
		...
		$sale_row =	sprintf("
	  	  <tr>
	  	   ...
	  	   <td style='text-align: right;' class='clickableRow' href='dispense.php?saleid=%d'>%0.2f <span class='smallerfont'>{$_SESSION['currencyoperator']}</span></td>
		  </tr>",
		  $sale['saleid'], $formattedDate, $sale['saleid'], $member, $sale['saleid'], $operator, $sale['saleid'], $quantity, $sale['saleid'], $amount
		  );
		  echo $sale_row;
	}
...
<!--  Shift details for product -->
<?php	// Also look up closings!
	 $selectOpenings = "SELECT 'Cerrar dia' AS type, 'CloseDay' AS typeshort, a.closingid AS openingid, a.closingtime AS shiftStart, a.closedby AS openedby, b.weight, b.weightDelta FROM closing a, closingdetails b WHERE a.closingid = b.closingid AND  b.purchaseid = ".$purchaseid." UNION ALL SELECT 'Abrir dia' AS type, 'OpenDay' AS typeshort, c.openingid, c.openingtime AS shiftStart, c.openedby, d.weight, d.weightDelta  FROM opening c, openingdetails d WHERE c.openingid = d.openingid AND d.purchaseid = ".$purchaseid." UNION ALL SELECT 'Cerrar turno' AS type, 'CloseShift' AS typeshort, e.closingid AS openingid, e.shiftEnd AS shiftStart, e.closedby AS openedby, f.weight, f.weightDelta FROM shiftclose e, shiftclosedetails f WHERE e.closingid = f.closingid AND f.purchaseid =".$purchaseid." UNION ALL SELECT 'Comenzar turno' AS type, 'StartShift' AS typeshort, g.openingid, g.openingtime AS shiftStart, g.openedby, h.weight, h.weightDelta FROM shiftopen g, shiftopendetails h ORDER by shiftStart DESC";  
		try
		{
...
<div class="actionbox-np2">
 <div class='mainboxheader'>
  <center><?php echo $lang['shifts']; ?></center>
 </div>
 <div class='boxcontent'>
	...
	<?php		
// exit(); // exit due to slowness bug, generating infinite lines of opening/closing
	...
	while ($opening = $results->fetch()) {
		...
		$prodStock = $opening['weight'];
		$stockDelta = $opening['weightDelta'];
	...
	if ($typeshort == 'CloseDay') {
		...
		if ($_SESSION['openAndClose'] > 2) {
			...
			$checkFirst = "SELECT a.dayClosed FROM opening a, openingdetails b WHERE a.openingtime < '$shiftStartOrig' AND a.openingid = b.openingid AND b.purchaseid = ".$purchaseid." ORDER BY a.openingtime DESC LIMIT 1";
		try
		{
		...
		$selectUsers = "SELECT COUNT(a.firstDayOpen) FROM opening a, openingdetails b WHERE a.openingid = b.openingid AND b.purchaseid = ".$purchaseid;

		$rowCount = $pdo3->query("$selectUsers")->fetchColumn();
		
		 $checkFirst = "SELECT a.firstDayOpen FROM opening a, openingdetails b WHERE a.openingid = b.openingid AND b.purchaseid = ".$purchaseid;
		try
		{
			$result = $pdo3->prepare("$checkFirst");
			$result->execute();
		...
		if ($rowCount == 1) {
			...
			// Not first opening
			$checkFirst = "SELECT a.dayOpened FROM closing a, closingdetails b WHERE a.closingtime < '$shiftStartOrig'  AND a.closingid = b.closingid AND b.purchaseid = ".$purchaseid." ORDER BY a.closingtime DESC LIMIT 1";
		try
		{
			$result = $pdo3->prepare("$checkFirst");
			$result->execute();
	...
	if ($shifttype == 2) {
		...
		$expense_row =	sprintf("
	  	  <tr>
	  	   ...
	  	   <td class='clickableRow %s' href='shift.php?type=%d&id=%d' style='text-align: right; border-bottom: 4px solid #507c3d;'>%0.2f <span class='smallerfont'>g.</span></td>
		  </tr>
",
		  $shifttype, $openingid, $type, $shifttype, $openingid, $shiftStart, $shifttype, $openingid, $user, $shifttype, $openingid, $prodStock, $tillColour, $shifttype, $openingid, $stockDelta
		  );
		...
		$expense_row =	sprintf("
	  	  <tr>
	  	   ...
	  	   <td class='clickableRow %s' href='shift.php?type=%d&id=%d' style='text-align: right;'>%0.2f <span class='smallerfont'>g.</span></td>
		  </tr>",
		  $shifttype, $openingid, $type, $shifttype, $openingid, $shiftStart, $shifttype, $openingid, $user, $shifttype, $openingid, $prodStock, $tillColour, $shifttype, $openingid, $stockDelta
		  );
  ...
  <table class="purchasetable">
   <tr>
    <td class="biggerFont left"><?php echo $lang['add-amountpurchased']; ?>
          <span class="purchaseNumber"><?php echo $purchaseQuantity; 6?> u</span></td>
   </tr>
   <tr>
    <td class="biggerFont left"><?php echo $lang['closeproduct-reloads']; ?>
          <span class="purchaseNumber"><?php echo number_format($tota6lReloads,0); ?> u</span></td>
   </tr>
   <tr style="border-bottom: 1px solid #eee;">
    <td class="biggerFont left"><?php echo $lang['closeday-added']; ?>
          <span class="purchaseNumber"><?php echo number_format($shak6eAdded,0); ?> u</span></td>
   </tr>
   <tr style="border-bottom: 2px solid #eee;">
    <td class="biggerFont left"><?php echo $lang['add-totalpurchased']; ?>
          <span class="purchaseNumber"><?php echo number_format($tota6lPurchased,0); ?> u</span></td>
   </tr>
    <tr>
    <td class="biggerFont left"><?php echo $lang['member-discount']; ?> <img src="images/medical-15.png" />
          <span class="purchaseNumber"><?php echo $medDiscount; ?> %</span></td>
   </tr>
...
<?php
	if ($closedAt != NULL) {
		...
		echo "
   <tr>
    <td class='biggerFont'>{$lang['pur-closedat']}
    <span class='purchaseNumber $closeClass'>$closedAt g</span><br />$closingDate</td>
   </tr>";
	} else {
		echo "
   <tr>
    <td colspan='2' style='padding-top: 8px; font-size: 16px;'>{$lang['pur-inmenu']}: <span class='purchaseNumber $menuClass'><strong>$inMenu</strong></span></td>
   </tr>";
	}
?>
  </table>
  ...
  <table class="purchasetable">
   <tr>
    <td class="left"><?php echo $lang['add-perunit']; ?>
    	<span class="bigNumber purchaseNumber"><?php echo number_format($purchasePrice,2); ?> <?php echo $_SESSION['currencyoperator'] ?></span>
	</td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['add-total']; ?>
    	<span class="bigNumber purchaseNumber"><?php echo $purchasePriceTotal; ?> <?php echo $_SESSION['currencyoperator'] ?></span>
	</td>
   </tr>
  </table>
 </div>
...
<?php
	if ($_SESSION['domain'] == 'dabulance') { ?>
		  <div class="mainboxheader">Sales Price</div>
		  <table class="purchasetable">
		   <tr>
		    <td class="left">Retail
		    <span class="bigNumber purchaseNumber"><?php echo number_format($salesPrice,2); ?> <?php echo $_SESSION['currencyoperator'] ?></span>
			</td>
		   </tr>
		   <tr>
		    <td class="left">Wholesale
		    <span class="bigNumber purchaseNumber"><?php echo number_format($salesPrice2,2); ?> <?php echo $_SESSION['currencyoperator'] ?></span>
			</td>
		   </tr>
		  </table>
<?php
		  ...
		  <table class="purchasetable">
		   ...
		   <tr>
		    <td class="left"><?php echo $lang['add-total']; ?>
		    <span class="bigNumber purchaseNumber"><?php echo $salesPriceTotal; ?> <?php echo $_SESSION['currencyoperator'] ?></span></td>
		   </tr>
		  </table><?php
	}
...
<br />
 ...
 <div class='actionbox-np2'>
  <div class="mainboxheader"><center><?php echo $lang['admin-statistics']; ?> total</center></div><br />
   <div class='boxcontent'>
  <form action='' method='POST'>
<?php
	if (isset($_POST['fromDate'])) {
...
<br /><br />
  ...
  <table class='defaultpad'>
   <tr>
		  	<td class="left"><?php echo $lang['closeday-dispensed']; ?></td>
		    <td class="bigNumber right"><?php echo number_format($sales,2); ?> u</td>
			<td class="bigNumber right"><?php echo number_format($soldPercentage,1); ?> %</td>
		</td>
   </tr>
   <tr style="border-bottom: 1px solid #bbb;">
	   	<td class="left"><?php echo $lang['gifted']; ?></td>
	    <td class="bigNumber right"><?php echo number_format($gifts,2); ?> u</td>
	    <td class="bigNumber right"><?php echo number_format($giftedPercentage,1); ?> %</td>
   </tr>
   <tr>
	    <td class="left"><strong><?php echo $lang['add-total']; ?></strong></td>
	    <td class="bigNumber right"><strong><?php echo number_format($gifts + $sales,2); ?> u</strong></td>
	    <td class="bigNumber right"><?php echo number_format($totalPercentage,1); ?> %</td>
   </tr>
   <tr>
    <td colspan="3">&nbsp;</td>
...
<?php if ($_SESSION['realWeight'] == 1) { ?>
   ...
   <tr>
	    <td class="left"><?php echo $lang['closeday-dispensed']; ?></td>
	    <td class="bigNumber right"><?php echo number_format($realSales,2); ?> u</td>
	    <td class="bigNumber right"><?php echo number_format($soldPercentageReal,1); ?> %</td>
   </tr>
   <tr style="border-bottom: 1px solid #bbb;">
	   	<td class="left"><?php echo $lang['gifted']; ?></td>
	    <td class="bigNumber right"><?php echo number_format($realgifts,2); ?> u</td>
	    <td class="bigNumber right"><?php echo number_format($giftedPercentageReal,1); ?> %</td>
   </tr>
   <tr>
	   	<td class="left"><strong><?php echo $lang['add-total']; ?></strong></td>
	    <td class="bigNumber right"><strong><?php echo number_format($realSales + $realgifts,2); ?> u</strong></td>
	    <td class="bigNumber right"><?php echo number_format($totalPercentageReal,1); ?> %</td>
   </tr>
   <tr>
    <td colspan="3">&nbsp;</td>
...
<?php } 
   ...
   <tr>
    <td colspan="3"><h5><strong>{$lang['closeday-takeouts']}</strong></h5></td>
   </tr>
EOD;
		foreach ($takeoutData as $movement) {
...
?>
 ...
 <br />
		 ...
		 <div class='actionbox-np2'>
		  <div class="mainboxheader"><?php echo $lang['pur-eststock']; ?> <a href="stock.php" class="smallerFont"></a></div>
		  <div class="boxcontent">
		  <table class="purchasetable">
		   <tr>
		    <td class="left"><?php echo $lang['global-stock']; ?>
		    <span class="bigNumber purchaseNumber"><?php echo number_format($estStock,0); ?> u</span></td>
		   </tr>
		   <tr>
		    <td class="left"><?php echo $lang['pur-displayjar']; ?>
		    <span class="bigNumber purchaseNumber"><?php echo number_format($inDisplay,0); ?> u</span></td>
		   </tr>
		   <tr>
		    <td class="left"><?php echo $lang['intstash']; ?>
		    <span class="bigNumber purchaseNumber"><?php echo number_format($inStashInt,0); ?> u</span></td>
		   </tr>
		   <tr style="border-bottom: 1px solid yellow;">
		    <td class="left"><?php echo $lang['extstash']; ?>
		    <span class="bigNumber purchaseNumber"><?php echo number_format($inStashExt,0); ?> u</span></td>
		   </tr>
		   <tr>
		    <td class="left"><strong><?php echo $lang['global-total']; ?>:</strong>
		    <span class="bigNumber purchaseNumber"><strong><?php echo number_format($totalProduct,0); ?> u</strong></span></td>
		   </tr>
		  </table>
		</div>
		</div>
...
<div class="actionbox-np2">
	<div class="boxcontent">
  <div class="mainboxheader">Stock real&nbsp;&nbsp;<img src="images/questionmark.png" id="helpmark2" /><div id='helpBox2' class='helpBox2'><strong><?php echo $lang['est-stock-info']; ?></strong></div>
		                <script>
		                  	$('#helpmark2').on({
						 		'mouseover' : function() {
								 	$('#helpBox2').css('display', 'block');
  ...
  <table class="purchasetable">
   <tr>
    <td class="left"><?php echo $lang['global-stock']; ?>
    <span class="bigNumber purchaseNumber"><?php echo number_format($realStock,0); ?> u</span></td>
   </tr>
   <tr>
    <td class="left"><?php echo $lang['pur-displayjar']; ?>
    <span class="bigNumber purchaseNumber"><?php echo number_format($inDisplay,0); ?> u</span></td>
   ...
   <tr>
    <td class="left"><strong><?php echo $lang['global-total']; ?>:</strong>
    <span class="bigNumber purchaseNumber"><strong><?php echo number_format($totalRealProduct,0); ?> u</strong></span></td>
   </tr>
  </table>
      </div>
  </div>
...
<div class='actionbox-np2'>
 ...
 <div class='boxcontent'>
  <div id="tabs">
  <center>
   <a href="?purchaseid=<?php echo $purchaseid; ?>&summary" class='topLink usergrouptext2'><?php echo $lang['summary']; ?></a>
<!--
   <a href="?purchaseid=<?php echo $purchaseid; ?>&week" class='topLink'>Weekly</a>
   <a href="?purchaseid=<?php echo $purchaseid; ?>&month" class='topLink'>Monthly</a>
-->
   <a href="?purchaseid=<?php echo $purchaseid; ?>&all" class='topLink usergrouptext2'><?php echo $lang['all']; ?></a>
  </center><br>
  	<ul>
		<li><a href="#tabs-1"><?php echo $lang['dispensary-movements']; ?></a></li>
		<li><a href="#tabs-2"><?php echo $lang['internal-stash-movements'] ?></a></li>
   		...
   		<div id="tabs-1">
			<?php 
				if (isset($_GET['summary'])) {
					...
					// Query to look up movements
					$selectMovements = "SELECT type, SUM(quantity), movementTypeid, user_id FROM productmovements WHERE purchaseid = $purchaseid GROUP BY movementTypeid ORDER by type DESC, movementTypeid ASC";
					
				} else {
					
					// Query to look up movements
					$selectMovements = "SELECT movementid, movementtime, type, purchaseid, quantity, movementTypeid, comment, price, user_id FROM productmovements WHERE purchaseid = $purchaseid ORDER by movementtime ASC";
				}
			...
			?>
				...
				$movement_row =	sprintf("
				  ...
				  $rowclass, $formattedDate, $movementOperator, $movementName, $quantity, $commentRead, $movementid, $purchaseid
				  );
				  echo $movement_row;
			  }
			?>
	...
	if ($shifttype == 2) {
		...
		$expense_row =	sprintf("
	  	  <tr>
	  	   ...
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: left; border-bottom: 4px solid #507c3d;'>%s</td>
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: left; border-bottom: 4px solid #507c3d;'>%s</td>
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: right; border-bottom: 4px solid #507c3d;'>%0.2f <span class='smallerfont'>u.</span></td>
	  	   <td class='clickableRow %s' href='shift.php?type=%d&id=%d' style='text-align: right; border-bottom: 4px solid #507c3d;'>%0.2f <span class='smallerfont'>u.</span></td>
		  </tr>
",
		  $shifttype, $openingid, $type, $shifttype, $openingid, $shiftStart, $shifttype, $openingid, $user, $shifttype, $openingid, $prodStock, $tillColour, $shifttype, $openingid, $stockDelta
		  );
		...
		$expense_row =	sprintf("
	  	  <tr>
	  	   ...
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: left;'>%s</td>
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: left;'>%s</td>
	  	   <td class='clickableRow' href='shift.php?type=%d&id=%d' style='text-align: right;'>%0.2f <span class='smallerfont'>u.</span></td>
	  	   <td class='clickableRow %s' href='shift.php?type=%d&id=%d' style='text-align: right;'>%0.2f <span class='smallerfont'>u.</span></td>
		  </tr>",
		  $shifttype, $openingid, $type, $shifttype, $openingid, $shiftStart, $shifttype, $openingid, $user, $shifttype, $openingid, $prodStock, $tillColour, $shifttype, $openingid, $stockDelta
		  );
