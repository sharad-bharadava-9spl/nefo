...
* -------------------------------
*/
$dd_savePath=str_replace('\\','/',dirname(__FILE__))."/images/_$domain/sigs";
$savePath="images/_$domain/sigs/";
$prependSig=$nextMemberNo;
/**
* -------------------------------
		...
		switch($action) {
			case "sig":
				...
				if($result==true) {
					...
					$uploaded = $savePath.$finalName;
					$fileContent = file_get_contents($uploaded);
                	$cloudPath = $google_root_folder.$uploaded;
                	$isSucceed = uploadFile($google_bucket, $fileContent, $cloudPath);
                	if($isSucceed == true){
                		unlink($uploaded);
