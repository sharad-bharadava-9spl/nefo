...
if ($_SESSION['realWeight'] == 0) {
	...
	if (isset($_POST['dispense'])) {
		...
		if ($pmtType == 3) {
		   ...
		   $query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity, discount, direct, discounteur, puesto) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%f', '%d', '%f', '%d');", 
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $gramsTOT, $totDiscountInput, $pmtType, $eurdiscount, $_SESSION['ordertype']);   
			...
			// Query to add new sale to Sales table - 6 arguments
		   $query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, expiry, realQuantity, discount, direct, discounteur, puesto) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $paidUntil, $gramsTOT, $totDiscountInput, $pmtType, $eurdiscount, $_SESSION['ordertype']);  
			try
			{
				$result = $pdo3->prepare("$query")->execute();
			}
...
<?php
	...
	if (isset($_POST['dispense'])) {
		...
		if ($pmtType == 3) {
			...
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, discount, direct, discounteur, puesto) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $realQuantity, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $totDiscountInput, $pmtType, $eurdiscount, $_SESSION['ordertype']);
			...
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, expiry, discount, direct, discounteur, puesto) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $realQuantity, $unitsTOT, $adminComment, $saletime, '1', $paidUntil, $totDiscountInput, $pmtType, $eurdiscount, $_SESSION['ordertype']);
	...
	foreach($_POST['sales'] as $sale) {
		...
		if ($gramsTot > 0) {
			...
			if ($grams > 0 && $grams2 > 0) {
	    		...
	    		// Query to add new sale to salesdetails table - ? arguments
			  	$query = sprintf("INSERT INTO salesdetails (saleid, category, productid, quantity, amount, purchaseid, realQuantity,  discountType, discountPercentage) VALUES ('%d', '%d', '%d', '%f', '%f', '%d', '%f');",
			  	$saleid, $category, $productid, $grams2, '0', $purchaseid, $grams2, 7, '');
