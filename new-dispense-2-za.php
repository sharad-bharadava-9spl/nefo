...
EOD;
    ...
    if ($_SESSION['domain'] == 'thealibi' || $_SESSION['domain'] == 'irena' || $_SESSION['domain'] == 'demo') {
    		$isGreengoat = 1;
    		$isChecked = 'checked';
	...
	if ($_SESSION['domain'] == 'macarena' || $_SESSION['domain'] == 'breakingbuds') {
		...
		{
				$error = 'Error fetching userA1: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	if ($_POST['newDonation'] > 0 && $_POST['gramsTOT'] == 0 && $_POST['unitsTOT'] == 0 ) {
		...
		{
				$error = 'Error fetching userA2: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching userA3: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching userA4: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		try{
			...
			$error = 'Error Fetching dataA5: ' . $e->getMessage();
			echo $error;
			exit();
		}
		...
		foreach($postSales as $postSale){
			...
			try{
				...
				$error = 'Error Fetching dataA6: ' . $e->getMessage();
				echo $error;
				exit();
			}
...
if ($_SESSION['realWeight'] == 0) {
	...
	if (isset($_POST['dispense'])) {
		...
		// code update start by KONSTANT on 19-08-2022 for TASK-15169823
		...
		if ($_POST['newDonation'] > 0) {
			...
			{
					$error = 'Error fetching userA7: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching userA8: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching userA9: ' . $e->getMessage();
					echo $error;
					exit();
			}
		}
		// code update end by KONSTANT on 19-08-2022 for TASK-15169823
	    ...
	    $gramExist = 0;
  	   ...
  	   foreach($_POST['sales'] as $sale) {
					...
					{
							$error = 'Error fetching userA10: ' . $e->getMessage();
							echo $error;
							exit();
					}
					...
					// code update start by konstant for dispense validation on 28-02-2022
						if($sale['grams'] > 0 && ($sale['euro'] == 0 || $sale['euro'] == '')){
							//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (1).";
							...
							{
									$error = 'Error fetching userA11: ' . $e->getMessage();
									echo $error;
									exit();
							}							
					...
					if($sale['grams'] > 0){
						if($sale['volume_discount'] != 0 || $sale['volume_discount_amount'] != 0){
							...
							//	$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (2).";
							...
							{
									$error = 'Error fetching userA12: ' . $e->getMessage();
									echo $error;
									exit();
							}							
							//	header("Location: new-dispense-2.php?user_id=$user_id");
							...
							//	$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (3).";
							...
							{
									$error = 'Error fetching userA13: ' . $e->getMessage();
									echo $error;
									exit();
							}							
							//	header("Location: new-dispense-2.php?user_id=$user_id");
							...
							if($sale['euro'] != $sale_price){
								$_SESSION['errorMessage'] = "4 Something went wrong! Please re-do the dispense.";
								header("Location: new-dispense-2.php?user_id=$user_id");
								exit();
							}
						}*/
						...
						if ($_SESSION['domain'] == 'thealibi' || $_SESSION['domain'] == 'irena' || $_SESSION['domain'] == 'demo') {
							...
							$saleEuro = number_format($saleEuro, 2, '.', '');
							 ...
							 $appliedDisc = (100 - $totDiscountInput) / 100;
							 $tempPrice = $saleEuro * $appliedDisc;
							 $eurdisc = $eurdiscount;
							 $newPrice = $tempPrice - $eurdisc;	

							$saleEuro = number_format($newPrice, 2, '.', '');
							$saleGrams = number_format($saleGrams, 2, '.', '');
							$saleUnits = number_format($saleUnits, 2, '.', '');
							
							$eurcalcTOT = number_format($eurcalcTOT, 2, '.', '');
							$gramsTOT = number_format($gramsTOT, 2, '.', '');
							$unitsTOT = number_format($unitsTOT, 2, '.', '');
...
/*		echo 	$eurcalcTOT."-"	.$saleEuro."-".$gramsTOT."-".$saleGrams; 
		...
		if(abs($eurcalcTOT - $saleEuro) > 0.00001) {
			//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (4).";
							...
							{
									$error = 'Error fetching userA14: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if(abs($gramsTOT - $saleGrams) > 0.00001) {	
			//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (5).";
							...
							{
									$error = 'Error fetching userA15: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if(abs($unitsTOT - $saleUnits) > 0.00001) {		
			//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (6).";
							...
							{
									$error = 'Error fetching userA16: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if($gramExist){
			if($saleGrams == 0){
				//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (7).";
							...
							{
									$error = 'Error fetching userA17: ' . $e->getMessage();
									echo $error;
									exit();
							}							
				//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if($unitExist){
			if($saleUnits == 0){
				//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (8).";
							...
							{
									$error = 'Error fetching userA18: ' . $e->getMessage();
									echo $error;
									exit();
							}							
				//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if($totalCheck == 0){
			//$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (9).";
							...
							{
									$error = 'Error fetching userA19: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			//header("Location: new-dispense-2.php?user_id=$user_id");
		...
		{
				$error = 'Error fetching userA20: ' . $e->getMessage();
				echo $error;
				exit();
		}
		$row = $result->fetch();
		...
		if ($pmtType == 3) {
			...
			{
					$error = 'Error fetching userA21: ' . $e->getMessage();
					echo $error;
					exit();
			}
			
			// Query to add new sale to Sales table - 6 arguments
		   $query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity, discount, direct, discounteur, unpaid) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%f', '%d', '%f', '%d');", 
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $gramsTOT, $totDiscountInput, $pmtType, $eurdiscount, $unpaid);   
			...
			{
					$error = 'Error fetching userA22: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			if($discountType < 5){
				...
				{
						$error = 'Error fetching userA23: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			if($totDiscountInput !='' && !empty($totDiscountInput)){
				...
				{
						$error = 'Error fetching userA24: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($giftDiscount){
				...
				{
						$error = 'Error fetching userA25: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($volumeDiscount){
				...
				{
						$error = 'Error fetching userA26: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($happyhourDiscount){
				...
				{
						$error = 'Error fetching userA27: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			{
					$error = 'Error fetching userA28: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			// Query to add new sale to Sales table - 6 arguments
		   $query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, expiry, realQuantity, discount, direct, discounteur, unpaid) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $paidUntil, $gramsTOT, $totDiscountInput, $pmtType, $eurdiscount, $unpaid);  
			try
			{
				$result = $pdo3->prepare("$query")->execute();
			}
			...
			{
					$error = 'Error fetching userA29: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			if($discountType < 5){
				...
				{
						$error = 'Error fetching userA30: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			if($totDiscountInput !='' && !empty($totDiscountInput)){
				...
				{
						$error = 'Error fetching userA31: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($giftDiscount){
				...
				{
						$error = 'Error fetching userA32: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($volumeDiscount){
				...
				{
						$error = 'Error fetching userA33: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if($happyhourDiscount){
				...
				{
						$error = 'Error fetching userA34: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			{
					$error = 'Error fetching userA35: ' . $e->getMessage();
					echo $error;
					exit();
			}
...
/*		if ($totDiscountInput > 0) {
		...
		if ($gramsTot > 0) {
			...
			if ($grams > 0 && $grams2 > 0) {
				...
				{
						$error = 'Error fetching userA36: ' . $e->getMessage();
						echo $error;
						exit();
				}
				...
				{
						$error = 'Error fetching userA37: ' . $e->getMessage();
						echo $error;
						exit();
				}
				...
				{
						$error = 'Error fetching userA38: ' . $e->getMessage();
						echo $error;
						exit();
				}
		...
		{
				$error = 'Error fetching userA39: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching userA40: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching userA41: ' . $e->getMessage();
				echo $error;
				exit();
		}
		// On success: redirect.
		$donationMessage = "";
		if($_POST['newDonation'] > 0){
			$latest_credit = $newCredit - $_POST['eurcalcTOT'];
		   $donationMessage = $lang['global-added'] . " " . $amount . $lang['donation-addedsuccessfully'] . $latest_credit . " ".$_SESSION['currencyoperator'];
		}
		$_SESSION['successMessage'] = $lang['dispense-added']."<br><br>". $donationMessage;
    ...
    if ((parseFloat(field_1) + parseFloat(field_2) + parseFloat(field_3)) == 0) {
	  ...
	  $('#registerForm').validate({
    	  ...
    	  submitHandler: function(form) {
				   ...
				   if(isGreengoat){
						setTimeout(function(){
						   	if(confirm('Are you sure you want to confirm the dispense?')){
							       form.submit();
						   }else{
...
$(document).on('click keypress keyup blur', function(event) {
   ...
   if (!$(event.target).is("#pmt0")) {
   if (!$(event.target).is("#pmt1")) {
   if (!$(event.target).is("#pmt2")) {
   if (!$(event.target).is("#pmt3")) {
   if (!$(event.target).is("#pmt6")) {
   if (!$(event.target).is("#pmt7")) {
   if (!$(event.target).is("#pmt8")) {
...
<br />  	   
  	   <table id='memberBoxTable'>
  	   	<?php if($domain == 'demo'){ ?>
  	   		<tbody class="show_cart_data mini_cart"></tbody>
  	   <?php } ?>
  	    <tr>
...
<!--  	    <tr>
   	    ...
   	    <tr>
   	    <td></td>
  	     <td><input type="radio" id="pmt0" name="pmtType" value='0' />
          <label for="pmt0"><span class='full' id="pmt0trigger">Unpaid</span></label>
         </td>
   	    <td></td>
        </tr>
   	    <tr>
...
<?php } ?>
          ...
          <label for="pmt3"><span class='full' id="pmt3trigger"><?php echo $lang['global-credit']; ?></span></label><br />
  	     ...
  	     <tr>
  	     <td>
          <input type="radio" id="pmt6" name="pmtType" value='6' />
          <label for="pmt6"><span class='full' id="pmt6trigger">SnapScan</span></label><br />
  	     </td>
  	     <td>
          <input type="radio" id="pmt7" name="pmtType" value='7' />
          <label for="pmt7"><span class='full' id="pmt7trigger">Ozow</span></label><br />
  	     </td>
  	     <td>
          <input type="radio" id="pmt8" name="pmtType" value='8' />
          <label for="pmt8"><span class='full' id="pmt8trigger">EFT</span></label><br />
  	     </td>
	    </tr>
...
function load1(){
    ...
    $.ajax({
      type:"post",
      url:"getflowers-za.php?i="+dayJSID+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
...
function load2(){
    ...
    $.ajax({
      type:"post",
      url:"getextracts-za.php?i="+dayJSID+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
      success:function(data)
      {
			$("#spinner2").remove();
...
function load(cat,type){ 
    ...
    $.ajax({
      type:"post",
      url:"getothers-za.php?i="+dayJSID+"&cat="+cat+"&type="+type+"&uid=$user_id&discount=$discount&usageType=$usageType",
      datatype:"text",
      success:function(data)
      {
			$("#spinner"+cat).remove();
...
<?php
	...
	if (isset($_POST['dispense'])) {
		...
		if ($_POST['newDonation'] > 0) {
			...
			{
					$error = 'Error fetching user00: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user01: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user02: ' . $e->getMessage();
					echo $error;
					exit();
			}
		}
	    ...
	    $sales_details = "";
  	   foreach($_POST['sales'] as $sale) {
				    ...
				    if($grams > 0 || $grams2 > 0){
				    	...
				    	$sales_details .= "Name:".$name."--";
				    	$sales_details .= "Category:".$category."--";
				    	$sales_details .= "Product ID:".$productid."--";
				    	$sales_details .= "Purchase ID:".$purchaseid."--";
				    	$sales_details .= "Grams:".$grams."--";
				    	$sales_details .= "Gifts:".$grams2."--";
				    	$sales_details .= "Real grams:".$realGrams."--";
				    	$sales_details .= "Disc Type:".$salediscType."--";
				    	$sales_details .= "Disc Percentage:".$discPercentage."--";
				    	$sales_details .= "Happy Hour:".$happy_hour_discount."--";
				    	$sales_details .= "Volume Disocunt:".$volume_discount."--";
						$sales_details .= "Volume per disc:".$volume_per_discount."--";
					}
					...
					catch (PDOException $e)
					{
							...
							$error_msg = 'Error fetching user03: ' . $e->getMessage();
							$error_query = "INSERT INTO errors (code, user_id, operator_id, comment) VALUES (15, $user_id, $operatorId, $error_msg)";
							try
							{
								$pdo3->prepare("$error_query")->execute();
							}
							catch (PDOException $e)
							{
									$error = 'Error fetching user: ' . $e->getMessage();
							...
							$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense.";
							header("Location: new-dispense-2.php?user_id=$user_id");
							exit();
					}
					$cat_row = $resultCat->fetch();
...
/*		echo 	$eurcalcTOT."-"	.$saleEuro."-".$gramsTOT."-".$saleGrams; 
		...
		if(abs($eurcalcTOT - $saleEuro) > 0.00001) {
			...
			$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (4).";
						...
						// insert error details
							$abs_comp = abs($eurcalcTOT - $saleEuro);
							$realWeight = $_SESSION['realWeight'];
							$query = "INSERT INTO errors (code, user_id, operator_id, first_comp_value, validation_comp_value, abs_comp_value, last_salesDetails, realweight) VALUES (4, $user_id, $operatorId, $eurcalcTOT, $saleEuro, $abs_comp, '$sales_details', $realWeight)";
							try
							{
								$result = $pdo3->prepare("$query")->execute();
							}
							...
							{
									$error = 'Error fetching user04: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if(abs($gramsTOT - $saleGrams) > 0.00001) {	
			$abs_comp = abs($gramsTOT - $saleGrams);
			$realWeight = $_SESSION['realWeight'];
			$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (5).";
							$query = "INSERT INTO errors (code, user_id, operator_id, first_comp_value, validation_comp_value, abs_comp_value, last_salesDetails, realweight) VALUES (5, $user_id, $operatorId, $gramsTOT, $saleGrams, $abs_comp, '$sales_details',$realWeight)";
							try
							{
								$result = $pdo3->prepare("$query")->execute();
							}
							...
							{
									$error = 'Error fetching user05: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			header("Location: new-dispense-2.php?user_id=$user_id");
		...
		if(abs($unitsTOT - $saleUnits) > 0.00001) {	
			$abs_comp = abs($unitsTOT - $saleUnits);
			$realWeight = $_SESSION['realWeight'];	
			$_SESSION['errorMessage'] = "Something went wrong! Please re-do the dispense. (6).";
							$query = "INSERT INTO errors (code, user_id, operator_id, first_comp_value, validation_comp_value, abs_comp_value, last_salesDetail, realweight) VALUES (6, $user_id, $operatorId, $unitsTOT, $saleUnits, $abs_comp, '$sales_details', $realWeight)";
							try
							{
								$result = $pdo3->prepare("$query")->execute();
							}
							...
							{
									$error = 'Error fetching user06: ' . $e->getMessage();
									echo $error;
									exit();
							}							
			header("Location: new-dispense-2.php?user_id=$user_id");
		...
		{
				$error = 'Error fetching user07: ' . $e->getMessage();
				echo $error;
				exit();
		}
		$row = $result->fetch();
		...
		if ($pmtType == 3) {
			...
			{
					$error = 'Error fetching user08: ' . $e->getMessage();
					echo $error;
					exit();
			}
			
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, discount, direct, discounteur, unpaid) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $realQuantity, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $totDiscountInput, $pmtType, $eurdiscount, $unpaid);
			...
			{
					$error = 'Error fetching user09: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			if ($discountType == '') {
				...
				{
						$error = 'Error fetching user13: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if ($discountType == '') {
				...
				{
						$error = 'Error fetching user14: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			{
					$error = 'Error fetching user15: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, expiry, discount, direct, discounteur, unpaid) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%d', '%f', '%d');",
		  	$saletime, $operatorId, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $realQuantity, $unitsTOT, $adminComment, $saletime, '1', $paidUntil, $totDiscountInput, $pmtType, $eurdiscount, $unpaid);
			...
			{
					$error = 'Error fetching user16: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			if($discountType < 5){
				...
				{
						$error = 'Error fetching user17: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			if($totDiscountInput !='' && !empty($totDiscountInput)){
				...
				{
						$error = 'Error fetching user18: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if ($discountType == '') {
				...
				{
						$error = 'Error fetching user19: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if ($discountType == '') {
				...
				{
						$error = 'Error fetching user20: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
			...
			if ($discountType == '') {
				...
				{
						$error = 'Error fetching user21: ' . $e->getMessage();
						echo $error;
						exit();
				}
			}
...
/*		if ($totDiscountInput > 0) {
		...
		if ($gramsTot > 0) {
			...
			if ($grams > 0 && $grams2 > 0) {
				...
				{
						$error = 'Error fetching user25: ' . $e->getMessage();
						echo $error;
						exit();
				}
		...
		10, $logTime, $user_id, $_SESSION['user_id'], $eurcalcTOT, $creditLookup, $newCreditCalc);
				...
				{
						$error = 'Error fetching user26: ' . $e->getMessage();
						echo $error;
						exit();
				}
				...
				{
						$error = 'Error fetching user26b: ' . $e->getMessage();
						echo $error;
						exit();
				}
				$query = sprintf("DELETE from saveDispense_summary where user_id = '%d'", $user_id);
				...
				{
						$error = 'Error fetching user27: ' . $e->getMessage();
						echo $error;
						exit();
				}
		// On success: redirect.
	...
	{
			$error = 'Error fetching user28: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
...
/*	$cartID = 'savedCart' . $user_id;
	...
	{
			$error = 'Error fetching user29: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
	...
	{
			$error = 'Error fetching user30: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row2 = $result->fetch();
	...
	{
			$error = 'Error fetching user31: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
...
EOD;
	...
	{
			$error = 'Error fetching user32: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
...
<?php
	...
	{
			$error = 'Error fetching user34: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
	...
	{
			$error = 'Error fetching user35: ' . $e->getMessage();
			echo $error;
			exit();
	}
	$row = $result->fetch();
...
<?php
	...
	{
			$error = 'Error fetching user36: ' . $e->getMessage();
			echo $error;
			exit();
	}
	while ($sort = $resultCats->fetch()) {
	...
	{
			$error = 'Error fetching user37: ' . $e->getMessage();
			echo $error;
			exit();
	}
		// Retrieve Menu type and discount for medical users
...
EOD;
	...
	if ($_SESSION['menusortdisp'] == 0) {
		...
		$sorting = "g.name ASC";
			...
			{
					$error = 'Error fetching user38: ' . $e->getMessage();
					echo $error;
					exit();
			}
	...
	$selectFlower = "SELECT g.flowerid, g.name, g.breed2, g.flowertype, g.sativaPercentage, g.description, g.medicaldescription, p.productid, p.purchaseid, p.salesPrice, p.realQuantity, p.growType, p.photoExt, p.medDiscount FROM flower g, purchases p WHERE p.category = 1 AND p.productid = g.flowerid AND p.closedAt IS NULL AND p.inMenu = 1 ORDER BY $sorting";
		...
		{
				$error = 'Error fetching user39: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user40: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	$growDetails = "SELECT growtype FROM growtypes WHERE growtypeid = $growtype";
		...
		{
				$error = 'Error fetching user41: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	$selectPurchaseDiscount = "SELECT discount, fijo FROM inddiscounts WHERE user_id = $user_id AND purchaseid = $purchaseid";
		...
		{
				$error = 'Error fetching user42: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	if ($_SESSION['showStock'] == 1) {
		...
		{
				$error = 'Error fetching user43: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user44: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user45: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user46: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user47: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user48: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user49: ' . $e->getMessage();
				echo $error;
				exit();
		}
...
",
	...
	{
			$error = 'Error fetching user50: ' . $e->getMessage();
			echo $error;
			exit();
	}
...
EOD;
	...
	{
			$error = 'Error fetching user51: ' . $e->getMessage();
			echo $error;
			exit();
	}
	...
	if ($data) {
		...
		{
				$error = 'Error fetching user52: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	$selectPurchaseDiscount = "SELECT discount, fijo FROM inddiscounts WHERE user_id = $user_id AND purchaseid = $purchaseid";
		...
		{
				$error = 'Error fetching user53: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	if ($_SESSION['showStock'] == 1) {
		...
		{
				$error = 'Error fetching user54: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user55: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user56: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user57: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user58: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user59: ' . $e->getMessage();
				echo $error;
				exit();
		}
		...
		{
				$error = 'Error fetching user60: ' . $e->getMessage();
				echo $error;
				exit();
		}
...
",
	...
	if ($menuType == 1) {
		...
		{
				$error = 'Error fetching user61: ' . $e->getMessage();
				echo $error;
				exit();
		}
...
EOD;
	...
	{
			$error = 'Error fetching user62: ' . $e->getMessage();
			echo $error;
			exit();
	}
	...
	$selectCategoryDiscount = "SELECT discount FROM catdiscounts WHERE user_id = $user_id AND categoryid = $categoryid";
		...
		{
				$error = 'Error fetching user63: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	$selectPurchaseDiscount = "SELECT discount, fijo FROM inddiscounts WHERE user_id = $user_id AND purchaseid = $purchaseid";
		...
		{
				$error = 'Error fetching user64: ' . $e->getMessage();
				echo $error;
				exit();
		}
	...
	if ($product['medicaldescription'] != '') {
		...
		if ($_SESSION['showStock'] == 1) {
			...
			{
					$error = 'Error fetching user65: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user66: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user67: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user68: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user69: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user70: ' . $e->getMessage();
					echo $error;
					exit();
			}
			...
			{
					$error = 'Error fetching user71: ' . $e->getMessage();
					echo $error;
					exit();
			}
...
{
		$error = 'Error fetching user72: ' . $e->getMessage();
		echo $error;
		exit();
}
...
<!-- Shopping cart JS -->
<script type="text/javascript" src="scripts/cart-za.js?t=<?php echo time();?>"></script>
...
// for greengoat only
/*var isGreengoat = <?php echo $isGreengoat ?>;
if(isGreengoat){
...
$(document).ready(function() {
	...
	var fullmenu = "<?php echo $fullmenu ?>";
...
<?php	displayFooter(); ?>
<script type="text/javascript">
	// checkout disocunt system
	$("input[name='inDiscount']").change(function(){
		$("#eurdiscount").val('');
	});
	$("#eurdiscount").on('keyup change keypress',function(){
		$("input[name='inDiscount']").prop("checked", false);
	});
