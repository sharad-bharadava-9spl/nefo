...
function getSettings() {
	...
	try
	{
		$result = $pdo3->prepare("SELECT highRollerWeekly, minAge, closingMail, dispensaryGift, barGift, menuType, medicalDiscount, logouttime, logoutredir, dispDonate, dispExpired, dispenseLimit, showAge, showGender, keepNumber, membershipFees, medicalDiscountPercentage, bankPayments, creditOrDirect, visitRegistration, cropOrNot, puestosOrNot, openAndClose, barMenuType, flowerLimit, extractLimit, realWeight, showStock, showOrigPrice, checkoutDiscount, consumptionMin, consumptionMax, showStockBar, showOrigPriceBar, barTouchscreen, trialMode, contract, iPadReaders, cashdro, creditchange, expirychange, exentoset, menusortdisp, menusortbar, dispsig, barsig, openmenu, keypads, moneycount, customws, negcredit, language, nobar, sigtablet, entrysys, entrysysstay, entrysyssecs, dooropener, cuotaincrement, checkoutDiscountBar, chipcost, barfullmenu, fingerprint, consumptionPercentage, pagination, normalNumbers, dooropenfor, workertracking, signupcode, presignup, allowvisitors, flowerLimitPercentage, extractLimitPercentage, fastVisitor, saldoGift, currencyoperator, gramLimit FROM systemsettings");
		$result->execute();
	}
	catch (PDOException $e)
	{
	...
	$row = $result->fetch();
		...
		$_SESSION['dooropenfor'] = $row['dooropenfor'];
		$_SESSION['workertracking'] = $row['workertracking'];
		$_SESSION['fullmenu'] = $row['fullmenu'];
		$_SESSION['barfullmenu'] = $row['barfullmenu'];
		$_SESSION['presignup'] = $row['presignup'];
		$_SESSION['signupcode'] = $row['signupcode'];
		$_SESSION['allowvisitors'] = $row['allowvisitors'];
		$_SESSION['flowerLimitPercentage'] = $row['flowerLimitPercentage'];
		$_SESSION['extractLimitPercentage'] = $row['extractLimitPercentage'];
		$_SESSION['fastVisitor'] = $row['fastVisitor'];
		$_SESSION['saldoGift'] = $row['saldoGift'];
		$_SESSION['currencyoperator'] = $row['currencyoperator'];
		$_SESSION['gramLimit'] = $row['gramLimit'];
...
function isZA() {
	...
	if ($_SESSION['domain'] == 'thealibi' || $_SESSION['domain'] == 'zatestandy') {
...
function isUS() {
	...
	if ($_SESSION['domain'] == 'us' || $_SESSION['domain'] == 'hazyhaus' || $_SESSION['domain'] == 'pakalolos') {
...
function isv6() {
		if ($_SESSION['domain'] != 'tresor' && $_SESSION['domain'] != 'theplug' && $_SESSION['domain'] != 'personal' && $_SESSION['domain'] != 'lasoci') {
