...
?>
<link rel="stylesheet" href="<?php echo $siteroot ?>owlcarousel/owl.carousel.min.css">
<link rel="stylesheet" href="<?php echo $siteroot ?>owlcarousel/owl.theme.default.min.css">
<?php
...
if ($_POST['newDonation'] > 0 && $_POST['gramsTOT'] == 0 && $_POST['unitsTOT'] == 0) {
	$cart_data = json_decode($_POST['save_cart_data'], true);
	...
	$query = sprintf(
		"INSERT INTO donations (userid, donationTime, type, amount, comment, creditBefore, creditAfter, donatedTo, operator) VALUES ('%d', '%s', '%d', '%f', '%s', '%f', '%f', '%d', '%d');",
		$userid,
		$registertime,
		'1',
		$amount,
	...
	$updateUser = sprintf(
		"UPDATE users SET credit = '%f', lastDispense = '%s' WHERE user_id = '%d';",
		$newCredit,
		$registertime,
		$userid
	);
	...
	if ($savedSummary_rows > 0) {
		$query = sprintf(
			"UPDATE saveDispense_summary SET gramsTOT='%f', realgramsTOT='%f', unitsTOT='%f', credit='%f', newcredit='%f', eurcalcTOT='%f' WHERE user_id = '%d'",
			$_POST['gramsTOT'],
			$_POST['realgramsTOT'],
			$_POST['unitsTOT'],
			$newCredit,
		...
		$query = sprintf(
			"INSERT INTO saveDispense_summary (user_id, gramsTOT, realgramsTOT, unitsTOT, credit, newcredit, eurcalcTOT) VALUES ('%d', '%f', '%f', '%f', '%f', '%f', %f);",
			$userid,
			$_POST['gramsTOT'],
			$_POST['realgramsTOT'],
			$_POST['unitsTOT'],
	...
	foreach ($postSales as $postSale) {
		...
		if ($savedData_rows > 0) {
			$query = sprintf(
				"UPDATE savesales_details SET grams='%f', euro='%f', realGrams='%f', grams2='%f' WHERE purchase_id='%d' AND user_id = '%d'",
				$postSale[grams],
				$postSale[euro],
				$postSale[realGrams],
				$postSale[grams2],
			...
			if ($postSale[grams] != 0) {
				// Query to save to savesales_details table
				$query = sprintf(
					"INSERT INTO savesales_details (user_id, purchase_id, grams, euro, realGrams, grams2) VALUES ('%d', '%d', '%f', '%f', '%f', '%f');",
					$userid,
					$postSale[purchaseid],
					$postSale[grams],
					$postSale[euro],
...
if ($_SESSION['realWeight'] == 0) {
	...
	if (isset($_POST['dispense'])) {
		...
		if ($pmtType == 3) {
			...
			$updateUser = sprintf(
				"UPDATE users SET credit = '%f' WHERE user_id = '%d';",
				$newCreditCalc,
				$user_id
			);
			...
			$query = sprintf(
				"INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity, discount, direct, discounteur) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%f', '%d', '%f');",
				$saletime,
				$operatorId,
				$user_id,
				$eurcalcTOT,
			...
			if ($discountType < 5) {
				$disc_query = sprintf(
					"INSERT INTO sales_discount (salesId, discountType, discountPercentage) VALUES ('%d', '%s', '%f');",
					$saleid,
					$discountType,
					$discountPercent
				);
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO f_sales (saletime, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity, discount, direct) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%f', '%d');",
				$saletime,
				$user_id,
				$eurcalcTOT,
				$eurcalcTOT,
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, expiry, realQuantity, discount, direct, discounteur) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%f', '%d', '%f');",
				$saletime,
				$operatorId,
				$user_id,
				$eurcalcTOT,
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO f_sales (saletime, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, expiry, realQuantity, discount, direct) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%f', '%d');",
				$saletime,
				$user_id,
				$eurcalcTOT,
				$eurcalcTOT,
		...
		foreach ($cart_data as $sale) {
			...
			if ($gramsTot > 0) {
				...
				if ($grams > 0 && $grams2 > 0) {
					...
					$query = sprintf(
						"INSERT INTO salesdetails (saleid, category, productid, quantity, amount, purchaseid, realQuantity, discountType, discountPercentage, happyhourDiscount, volumeDiscount) VALUES ('%d', '%d', '%d', '%f', '%f', '%d', '%f', '%d', '%f', '%f', '%f');",
						$saleid,
						$category,
						$productid,
						$grams,
					...
					// Query to add new sale to salesdetails table - ? arguments
					$query = sprintf(
						"INSERT INTO salesdetails (saleid, category, productid, quantity, amount, purchaseid, realQuantity,  discountType, discountPercentage) VALUES ('%d', '%d', '%d', '%f', '%f', '%d', '%f', '%d', '%f');",
						$saleid,
						$category,
						$productid,
						$grams2,
					...
					// Query to add new sale to salesdetails table - ? arguments
					$query = sprintf(
						"INSERT INTO salesdetails (saleid, category, productid, quantity, realQuantity, amount, purchaseid, discountType, discountPercentage, happyhourDiscount, volumeDiscount) VALUES ('%d', '%d', '%d', '%f', '%f', '%f', '%d', '%d', '%f', '%f', '%f');",
						$saleid,
						$category,
						$productid,
						$grams,
		...
		$query = sprintf(
			"INSERT INTO log (logtype, logtime, user_id, operator, amount, oldCredit, newCredit) VALUES ('%d', '%s', '%d', '%d', '%f', '%f', '%f');",
			10,
			$logTime,
			$user_id,
			$_SESSION['user_id'],
...
$(document).on('click keypress keyup blur', function(event) {
   ...
   if (!$(event.target).is("#chipClick")) {
		...
		$("input[name='pmtType']").attr("checked", false);
		setTimeout(function(){
			getItems();
		    computeTot();
		}, 700); 
...
?>
	...
	<!-- <div style='display: inline-block; text-align: left !important; float: left; position: absolute; left: 64px; margin-top: -26px; z-index: 999999 !important;'>
...
<table id='searchtable' class='default bgwhite' style='text-align: left !important; margin-left: 0; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-top: 1px solid #ddd; margin-left: 11px; margin-top: -16px; z-index: 999999 !important;'>
	...
	<div class="top-bar-dis">
		<div>
			<div class='twoboxessmall10'>
				<form id='searchform' autocomplete="off">
					<input type="text" name="searchfield" class='defaultinput' id='searchfield' autofocus placeholder="Search" />
					<div id="button1212">Buscar
						<span class="spn-img-src"> <img src="images/icons8-ser.png" alt="" width="15" height="14"></span>
					</div>
				</form>
			</div>

			<!-- <div class="ship-10">
				<img src="images/newcart.png" width="50" height="50"><span id="cart_count" style="vertical-align: bottom;">0</span>
			</div> -->
		</div>
		<div>
			<!-- width : 63% -->
			<div style='position: absolute;margin-top: -16px;z-index: 999999 !important; margin-left: 12.4%;'>
				<table id='searchtable' class='default bgwhite' style='text-align: left !important; border-left: 1px solid #ddd; border-right: 1px solid #ddd; border-top: 1px solid #ddd;  z-index: 999999 !important;'>
				</table>
			</div>
		</div>
...
EOD;
				...
				<br />
				<input type="hidden" name="save_cart_data" id="save_cart_data">
			...
			<!--new tab-->
			<div id="pdispense">
				<!-- <div class="owl-carousel owl-theme newTab_slider">
				...
				<div id="dispense-tabs" class="newTab" style="display: none;">
					<div class="ulOuter">
					<div class="owl-carousel owl-theme newTab_slider">
							<!-- <span id="slideLeft" class="tb_nav prev"></span>
							<span id="slideRight" class="tb_nav next"></span> -->
							<?php
							...
							while ($sort = $resultCats->fetch()) {
								...
								if (trim($cat_icon) != '') {
									$cat_img = "<img src='images/_$domain/category/" . $cat_icon . "' class='cat_green'/>";
								} else {
									$cat_img = "<img src= 'images/grey-leaf-3x.png'  />";
								}
								...
								if ($categoryid == 1) {
									...
									if ($data) {
										...
										$i = 0;
										echo "<div class='item'><a href='#category1' onclick='load1();' class='index_category1'><h4> $cat_img {$lang['global-flowerscaps']} </h4></a>";
								...
								function load1(){
									...
									var usageType = "<?php echo $usageType ?>";	
								    $.ajax({
								      type:"post",
...
<?php
									...
									if ($data) {
										echo "<div class='item'><a href='#category2' onclick='load2();' class='index_category2'><h4> $cat_img {$lang['global-extractscaps']} </h4></a>";
...
<?php
									...
									if ($data) {
										echo "<div class='item'><a href='#category$categoryid' onclick='load($categoryid,$type);' class='index_category$categoryid'><h4> $cat_img {$name} </h4></a>";
...
<?php
					...
					<!-- <div style="margin: 14px 0;text-align:center">
						<span id="dot1slide" class="dotnewslider"></span>
						<span id="dot2slide" class="dotnewslider"></span>
						<span id="dot3slide" class="dotnewslider"></span>
						<span id="dot4slide" class="dotnewslider"></span>
					</div> -->
					<br>
					<div id="loader_img"></div>
					<?php
					try {
						$resultCats2 = $pdo3->prepare("$selectCats");
						$resultCats2->execute();
					} catch (PDOException $e) {
						$error = 'Error fetching user: ' . $e->getMessage();
						echo $error;
					...
					while ($cat_row = $resultCats2->fetch()) {
						...
						if ($categoryid == 1) {
							...
							$selectFlower1 = "SELECT g.flowerid, g.name, g.breed2, g.flowertype, g.sativaPercentage, g.description, g.medicaldescription, p.productid, p.purchaseid, p.salesPrice, p.realQuantity, p.growType, p.photoExt, p.medDiscount FROM flower g, purchases p WHERE p.category = 1 AND p.productid = g.flowerid AND p.closedAt IS NULL AND p.inMenu = 1 ORDER BY $sorting;";

							try {
								$result1 = $pdo3->prepare("$selectFlower1");
								$result1->execute();
							...
							if ($data1) {
								...
								$i = 0;
								echo "<div id='category1' class='hideUnhide'><span id='menu1'></span></div>";
							}
						} else if ($categoryid == 2) {
							...
							$selectExtract1 = "SELECT h.extractid, h.name, h.extract, h.description, h.medicaldescription, p.productid, p.purchaseid, p.salesPrice, p.realQuantity, p.photoExt, p.medDiscount FROM extract h, purchases p WHERE p.category = 2 AND p.productid = h.extractid AND p.closedAt IS NULL AND p.inMenu = 1 ORDER BY $sorting;";
							try {
								$result2 = $pdo3->prepare("$selectExtract1");
								$result2->execute();
							...
							if ($data2) {
								...
								echo "<div id='category2' class='hideUnhide'><span id='menu2'></span></div>";
							}
						} else {
							...
							// For each cat, look up products
							$selectProduct1 = "SELECT pr.productid, pr.name, pr.description, pr.medicaldescription, p.purchaseid, p.salesPrice, p.realQuantity, p.photoExt, p.medDiscount FROM products pr, purchases p WHERE p.category = $categoryid AND p.productid = pr.productid AND p.closedAt IS NULL AND inMenu = 1 ORDER BY $sorting;";


							try {
								$result3 = $pdo3->prepare("$selectProduct1");
								$result3->execute();
								$data3 = $result3->fetchAll();
							} catch (PDOException $e) {
								$error = 'Error fetching user test: ' . $e->getMessage();
								echo $error;
								exit();
							}

							if ($data3) {
								echo "<div id='category$categoryid' class='hideUnhide'><span id='menu$categoryid'></span></div>";
							}
						}
					}
					?>
...
<?php
	...
	if (isset($_POST['dispense'])) {
		...
		if ($pmtType == 3) {
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, discount, direct, discounteur) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%d', '%f');",
				$saletime,
				$operatorId,
				$user_id,
				$eurcalcTOT,
			...
			// KONSTANT CODE UPDATE END
			$query = sprintf(
				"INSERT INTO f_sales (saletime, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, discount, direct) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%d');",
				$saletime,
				$user_id,
				$eurcalcTOT,
				$eurcalcTOT,
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO sales (saletime, operatorid, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, expiry, discount, direct, discounteur) VALUES ('%s', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%d', '%f');",
				$saletime,
				$operatorId,
				$user_id,
				$eurcalcTOT,
			...
			// Query to add new sale to Sales table - 6 arguments
			$query = sprintf(
				"INSERT INTO f_sales (saletime, userid, amount, amountPaid, quantity, realQuantity, units, adminComment, settled, settledTo, expiry, discount, direct) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%s', '%f', '%d');",
				$saletime,
				$user_id,
				$eurcalcTOT,
				$eurcalcTOT,
	...
	<script>
		...
		function zoomTo(category, purchaseid, type) {
			...
			var timer = setInterval(function() {
				/*var this_id = $("a.index_category" + category).attr('id');
				console.log(this_id);
				const indx_arr = this_id.split("-");
				var index = parseInt(indx_arr[2]) - 1;
				console.log(index);
				$('#dispense-tabs').tabs("option", "active", index);*/
...
<script>
	...
	$(function() {
		$("#dispense-tabs").tabs();
		$("#dispense-tabs .ui-tabs-active a").trigger('click');
		//$('#dispense-tabs').tabs("option", "active", 5);
	});
	
	$('.hideUnhide').hide();

</script>
<script src="<?php echo $siteroot ?>owlcarousel/owl.carousel.min.js"></script>

<script type="text/javascript">
	
	$('.newTab_slider ').owlCarousel({
		//margin:10,
		loop:false,
    	items:6,
		autoWidth:true,
...
<script>
$(".owl-item .item a").first().trigger('click');
$(".owl-item .item a h4").first().addClass('active');


$('.item a h4').on('click',function(){
	$('.item a h4').removeClass('active');
