		...
		$row = $result->fetch();
			...
			for ($i = 1; $i <= $noOfLines; $i++) {
    			...
    			// exclude today's date
					...
					$output .= "<form action='' method='POST'><input type='hidden' name='reportDate' value='$reportDateSQL'><input type='hidden' name='reportDateReadable<br />' value='$reportDateReadable'><button type='submit' class='linkStyle'>$reportDateReadable<br /></button></form>";
...
if ($_SESSION['realWeight'] == 1) {
		...
		$row = $result->fetch();
				...
				$selectDonations = "SELECT COUNT(donationid) from donations WHERE (donatedTo < 2 OR donatedTo = 4) AND DATE(donationTime) = DATE('$reportDate')";
		try
		{
		...
		$row = $result->fetch();
				...
				if ($_SESSION['creditOrDirect'] == 0) {
					...
					$selectSales = "SELECT SUM(amount) from sales WHERE DATE(saletime) = DATE('$reportDate') AND direct < 2";
		try
		{
			$result = $pdo3->prepare("$selectSales");
			$result->execute();
		...
		$row = $result->fetch();
					...
					$selectSales = "SELECT SUM(amount) from b_sales WHERE DATE(saletime) = DATE('$reportDate') AND direct < 2";
		try
		{
			$result = $pdo3->prepare("$selectSales");
			$result->execute();
...
<table style='color: #444; text-align: right; font-family: Tahoma, Verdana, sans-serif; font-size: 14px;'>
 ...
 <tr>
  ...
  <td>{$expr(number_format($salesToday,2))} {$_SESSION['currencyoperator']}</td>
  <td></td>
...
EOD;
 ...
 <tr>
  ...
  <td>{$expr(number_format($prodPrice,2))} {$_SESSION['currencyoperator']}</td>
...
EOD;
 ...
 <tr>
  ...
  <td>{$expr(number_format($flowertotalNoShake,2))} g.</td>
 ...
 <tr>
  ...
  <td>{$expr(number_format($extracttotalWeight,2))} g.</td>
...
EOD;
 ...
 <tr>
  ...
  <td>{$expr(number_format($otherTotals[$category]['otherWeightPrice'],2))} {$_SESSION['currencyoperator']}</td>
EOD;
				if ($_SESSION['realWeight'] == 1) {
...
EOD;
 ...
 <tr style='border-top: 1px solid #888; border-bottom: 2px solid #888;'>
  ...
  <td><strong>{$expr(number_format($euroTotG + $extractWeightPrice + $flowerWeightPrice,2))} {$_SESSION['currencyoperator']}</strong></td>
EOD;
				if ($_SESSION['realWeight'] == 1) {
...
EOD;
 ...
 <tr style='border-top: 1px solid #888; border-bottom: 2px solid #888;'>
  ...
  <td><strong>{$expr(number_format($euroTotU,2))} {$_SESSION['currencyoperator']}</strong></td>
...
EOD;
	  ...
	  echo "</table>";
		...
		$row = $result->fetch();
			...
			// Look up todays donations
			$selectDonations = "SELECT SUM(amount) from donations WHERE (donatedTo < 2 OR donatedTo = 4) AND DATE(donationTime) = DATE('$reportDate')";
		try
		{
			$result = $pdo3->prepare("$selectDonations");
			$result->execute();
...
EOD;
		...
		$customProducts .= " UNION ALL SELECT '$categoryid' AS category, pr.productid AS productid, pr.name AS name, p.purchaseid AS purchaseid, '' AS growtype, p.inMenu AS inMenu, p.closedAt AS closedAt, salesPrice AS gramPrice, p.realQuantity FROM products pr, purchases p WHERE p.category = $categoryid AND p.productid = pr.productid AND (p.closedAt IS NULL OR DATE(p.closingDate) >= DATE('$reportDate')) AND DATE(p.purchaseDate) <= DATE('$reportDate')";
		
		
		// Look up sales in this cat
		$selectSalesOthers = "SELECT SUM(d.amount), SUM(d.quantity), SUM(d.realQuantity) from sales s, salesdetails d WHERE s.saleid = d.saleid AND DATE(s.saletime) = DATE('$reportDate') AND d.category = $categoryid";
		try
		{
			$result = $pdo3->prepare("$selectSalesOthers");
			$result->execute();
...
EOD;
 ...
 <tr>
  <td style='text-align: left;'><strong>{$lang['profit']}</strong></td>
  <td><strong>{$expr(number_format($totalIncome - $tillExpenses - $bankExpenses,2))} {$_SESSION['currencyoperator']}</strong></td>
  <td></td>
  <td></td>
  <td></td>
  <td></td>
...
EOD;
 ...
 <tr>
  <td style='text-align: left; border-bottom: 1px dashed #ababab;'>+ {$lang['closeday-membershipfees-bank']}</td>
  <td style='text-align: right; border-bottom: 1px dashed #ababab;'>{$expr(number_format($membershipfeesBank,2))} {$_SESSION['currencyoperator']}</td>
  <td></td>
  <td></td>
  <td></td>
  <td></td>
...
EOD;
		...
		*/
			...
			$selectProducts = "SELECT '1' AS category, g.flowerid AS productid, g.name AS name, p.purchaseid AS purchaseid, p.growType AS growtype, p.inMenu AS inMenu, p.closedAt AS closedAt, salesPrice AS gramPrice, p.realQuantity FROM flower g, purchases p WHERE p.category = 1 AND p.productid = g.flowerid AND (p.closedAt IS NULL OR DATE(p.closingDate) >= DATE('$reportDate')) AND DATE(p.purchaseDate) <= DATE('$reportDate') UNION ALL SELECT '2' AS category, h.extractid AS productid, h.name AS name, p.purchaseid AS purchaseid, '' AS growtype, p.inMenu AS inMenu, p.closedAt AS closedAt, salesPrice AS gramPrice, p.realQuantity FROM extract h, purchases p WHERE p.category = 2 AND p.productid = h.extractid AND (p.closedAt IS NULL OR DATE(p.closingDate) >= DATE('$reportDate')) AND DATE(p.purchaseDate) <= DATE('$reportDate')";
...
EOD;
		...
		$row = $result->fetch();
					...
					$selectStashedInt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 2 AND DATE(movementtime) <= DATE('$reportDate') AND (movementTypeid = 5 OR movementTypeid = 18)";
		try
		{
		...
		$row = $result->fetch();
					...
					$selectUnStashedInt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 1 AND DATE(movementtime) <= DATE('$reportDate') AND (movementTypeid = 12 OR movementTypeid = 17)";
		try
		{
		...
		$row = $result->fetch();
					...
					$selectStashedExt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 2 AND DATE(movementtime) <= DATE('$reportDate') AND (movementTypeid = 6 OR movementTypeid = 20)";
		try
		{
		...
		$row = $result->fetch();
					...
					$selectUnStashedExt = "SELECT SUM(quantity) FROM productmovements WHERE purchaseid = $purchaseid AND type = 1 AND DATE(movementtime) <= DATE('$reportDate') AND (movementTypeid = 2 OR movementTypeid = 19)";
		try
		{
