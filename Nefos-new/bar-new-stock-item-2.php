<?php
	...
	if (isset($_POST['salesPrice'])) {
		...
		$hw_provider = $_POST['hw_provider'];
		$paidNow = $_POST['paidNow'];
		$barCode = $_POST['barCode'];
		
		
		// Query to add new purchase - 11 arguments
		  $query = sprintf("INSERT INTO b_purchases (category, productid, purchaseDate, purchasePrice, salesPrice, purchaseQuantity, adminComment, inMenu, provider, paid, barCode, hw_provider) VALUES ('%d', '%d', '%s', '%f', '%f', '%f', '%s', '%d', '%d', '%f', '%s', '%d');",
		  $category, $productID, $purchaseDate, $purchasePrice, $salesPrice, $purchaseQuantity, $adminComment, $inMenu, $provider, $paidNow, $barCode, $hw_provider);
		try
		{
...
<?php  echo  "SELECT id, name FROM hw_providers WHERE products_offers REGEXP ('($productID)')"; ?>
<td class="biggerFont left">HW <?php echo $lang['provider']; ?>:</td>   
<td class='left'>
	<select name='hw_provider' style="width: 120px;">
	    <option value=''><?php echo $lang['global-choose']; ?></option>
			<?php
				$HWPRquery = "SELECT id, name FROM hw_providers WHERE products_offers REGEXP ('($productID)')";
				try
				{
					$HWPRresult = $pdo3->prepare("$HWPRquery");
					$HWPRresult->execute();
				}
				catch (PDOException $e)
				{
				...
				while ($HWPRtype = $HWPRresult->fetch()) {
					$id = $HWPRtype['id'];
					$name = $HWPRtype['name'];
