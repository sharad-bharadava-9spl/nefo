...
$getClub = "SELECT shortName,logo_path,member_contract,original_path,email from customers WHERE id = '$club_id'";
	...
	$dirPath = "../../ccsnubev2_com/v6/_club/_".$shortName;
	$imgDir = "../../ccsnubev2_com/v6/images/_".$shortName;
    ...
    $intlogo = str_replace("https://ccsnubev2.com", "../../ccsnubev2_com", $clublogo);
    $intlogoorig = str_replace("https://ccsnubev2.com", "../../ccsnubev2_com", $cluboriginallogo);
    //if(!is_dir("../../ccsnubev2_com/v6/_club")){
    //	mkdir("../../ccsnubev2_com/v6/_club",0777);
    //} 
	$newDir = $clubname;
	$sourceDir = "../../ccsnubev2_com/v6";
	...
	if(!empty($clublogo) || $clublogo != ''){
		rename($intlogo, $imgDir."/logo.png");
		rename($intlogoorig, $imgDir."/original_logo.png");
	...
	if(!empty($clubContract) || $clubContract != ''){
		rename("../../ccsnubev2_com/v6/_club/contract.php", $newDir."/contract.php");
	}
	// update launch date
   ...
   $club_domain = $shortName;
   $db_user =  "ccs_".$shortName."u";
   $db_pass = randomPassword(13);
   $dbName = "ccs_".$shortName;
...
*/
     ...
     // update club user
	  $upClubUser = "UPDATE users SET userPass = '$cryptPass' WHERE email = '$demoEmail'";
	   ...
	   $subject = "CCS Club Status";
		...
		$userMessage = <<<EOD
¡Hola!<br />
¡Gracias por elegir Cannabis Club Systems!<br /><br />
A continuación te mostramos el enlace para crear primer usuario administrador en tú nuevo sistema CCS.<br />
http://ccsnubev2.com/?h=$hash<br /><br />
Para asegurar que todo funciona bien, rogamos que debes usar Mozilla Firefox como tu navegador.<br /><br />
Para poner el sistema en marcha dirígete a nuestro centro de soporte donde puedes encontrar tutoriales para primeros pasos.<br />
No olvides como parte de tu suscripción te ofrecemos formación online adicional continua en cualquier momento.<br /><br />
Si tienes cualquier duda o pregunta estamos a tu disposición.<br /><br />
Un saludo,<br />

EOD;
		sendEmail($usermail, $email, $userMessage, "Acceso a Software CCS ($club_domain)");
		sendEmail($adminmail, $maiAdmin, $userMessage, "Acceso a Software CCS ($club_domain)");
...
echo "<script type='text/javascript'>window.location.href = 'select-ship-region.php?club_id=".$club_id."';</script>";
