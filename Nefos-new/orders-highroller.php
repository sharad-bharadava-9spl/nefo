<?php
	...
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter, userConfirmed, fulfilled, paymentoption FROM sales WHERE customer = '{$_SESSION['clubdomain']}' AND userConfirmed = 1 $timeLimit ORDER by saletime DESC $limitVar";
		try
		{
			$results = $pdo4->prepare("$selectSales");
			$results->execute();
		}
	...
	try
	{
		$result = $pdo4->prepare("$findStartDate");
		$result->execute();
	}
	catch (PDOException $e)
	{
...
<br />
	 <table class='default' id='mainTable'>
	  ...
	  <?php
		...
		try
		{
			$result = $pdo4->prepare("$userLookup");
			$result->execute();
		}
		catch (PDOException $e)
		{
	...
	if ($delivered == 0) {
		...
		try
		{
			$onesaleResult = $pdo4->prepare("$selectoneSale");
		...
		try
		{
			$result = $pdo4->prepare("$categoryDetails");
			$result->execute();
		}
		catch (PDOException $e)
		{
	...
	if ($onesale['category'] == 1) {
		...
		try
		{
			$result = $pdo4->prepare("$selectProduct");
			$result->execute();
		}
		catch (PDOException $e)
		{
		...
		$amount = number_format($amount,2);
				...
				echo "
				<td class='right' ><strong>{$units} u</strong></td>
				<td class='right' ><strong>{$amount} &euro;</strong></td>
				<td class='centered relative' href='dispense.php?saleid={$saleid}'>$pay</td>
				<td class='centered relative' >No</td>
				<td class='centered relative' >$clubApproved</td>
