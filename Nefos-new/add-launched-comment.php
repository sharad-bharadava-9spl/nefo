<?php
	...
	if ($_POST['confirmed'] == 'yes') {
		...
		$query = sprintf("INSERT INTO launchcomments (time, period, comment, operator) VALUES ('%s', '%s', '%s', '%d');",
	  	 $registertime, $period, $comment, $_SESSION['user_id']);
		try
		{
		...
		header("Location: launched-summary.php");
		exit();
