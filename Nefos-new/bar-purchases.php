<?php
	...
	$selectOpenPurchases = "SELECT purchaseid, category, productid, purchaseDate, purchasePrice, salesPrice, purchaseQuantity, adminComment, closedAt, estClosing, inMenu, barCode, closingDate FROM b_purchases WHERE category > 48 OR (category = 35 OR category = 34 OR category = 30 OR category = 19) AND closedAt IS NULL ORDER by purchaseDate DESC";
		try
		{
	...
	$selectClosedPurchases = "SELECT purchaseid, category, productid, purchaseDate, purchasePrice, salesPrice, purchaseQuantity, adminComment, closedAt, estClosing, inMenu, closingComment, barCode, closingDate FROM b_purchases WHERE category > 48 OR (category = 35 OR category = 34 OR category = 30 OR category = 19) AND closedAt IS NOT NULL ORDER by purchaseDate DESC";
		try
		{
...
<center>
 <a href="bar-new-stock-item.php" class="cta1">New item</a> <br />
 <img src="images/excel-new.png" style="cursor: pointer;" onclick="tablesToExcel(['mainTable'], ['mainTable'], 'myfile.xls')" value="Export to Excel" />
</center>
	...
	$purchase_row =	sprintf("
  	  <tr>
  	   <td class='clickableRow' href='bar-stock-item.php?purchaseid=%d'>%s</td>
  	   <td class='clickableRow' href='bar-stock-item.php?purchaseid=%d'>%s</td>
  	   <td class='clickableRow' href='bar-stock-item.php?purchaseid=%d'>%s</td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.0f u</td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> &euro; /u</span></td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> &euro; /u</span></td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.2f<span class='smallerfont3'> &euro; /u</span></td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.0f u</td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.0f u</td>
  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'>%0.0f u</td>
  	   <td class='clickableRow' style='text-align: center' href='bar-stock-item.php?purchaseid=%d'>%s</td>
  	   <td class='clickableRow' style='text-align: center' href='bar-stock-item.php?purchaseid=%d'></td>
	   <td class='relative'></td>
	   <td><span class='relativeitem'>%s</span></td>
	   <td class='relative'>%s</td>
	  </tr>",
	  $purchase['purchaseid'], $formattedDate, $purchase['purchaseid'], $purchaseCategory, $purchase['purchaseid'], $name, $purchase['purchaseid'], $purchase['purchaseQuantity'], $purchase['purchaseid'], $purchase['purchasePrice'], $purchase['purchaseid'], $purchase['salesPrice'] - $purchase['purchasePrice'], $purchase['purchaseid'], $purchase['salesPrice'], $purchase['purchaseid'], $estStock, $purchase['purchaseid'], $inStashInt, $purchase['purchaseid'], $inStashExt, $purchase['purchaseid'], $inMenu, $purchase['purchaseid'], $commentRead, $barCodeImg);
		...
		if ($purchase['barCode'] != '') {
			...
			$purchase_row =	sprintf("
		  	  <tr class='closedProduct'>
		  	   ...
		  	   <td></td>
		  	   <td class='clickableRow' style='text-align: center' href='bar-stock-item.php?purchaseid=%d'>%s</td>
		  	   <td class='clickableRow right' href='bar-stock-item.php?purchaseid=%d'><span class=' %s'>%0.02f u</span></td>
			   <td class='relative'>%s</td>
			   <td><span class='relativeitem'>%s</span></td>
			   <td class='relative'>%s</td>
			  </tr>",
