...
EOD;
	...
	pageStart($lang['index-members'], NULL, $deleteSaleScript, "pmembership", NULL, "", $_SESSION['successMessage'], $_SESSION['errorMessage']);
?>

<center><h1>Previous month's possible misuse (tourist accounts)</h1></center>
...
echo "<table id='mainTable'><tr><td style='padding: 5px;'>DB</td><td style='padding: 5px;'>Member ID</td><td style='padding: 5px;'>Dispenses per day</td><td style='padding: 5px;'>Tot. dispenses</td><td style='padding: 5px;'>Tot. members dispensed</td></tr>";
...
foreach ($getAllDbs as $DB) {
	...
	$db_user = $db_name . "u";
		...
		$selectUsersU = "SELECT userid, COUNT(saleid) FROM sales WHERE saletime BETWEEN '$monthBegin' AND '$monthEnd' GROUP BY userid";
		try
		{
		...
		$query = "SELECT COUNT(DISTINCT userid) FROM sales WHERE saletime BETWEEN '$monthBegin' AND '$monthEnd'";
		try
		{
			$resultQ = $pdo4->prepare("$query");
			$resultQ->execute();
		}
		catch (PDOException $e)
		{
		...
		$rowQ = $resultQ->fetch();
			$totalD = $rowQ['COUNT(DISTINCT userid)'];

		$query = "SELECT COUNT(saleid) FROM sales WHERE saletime BETWEEN '$monthBegin' AND '$monthEnd'";
		try
		{
			$resultQ = $pdo4->prepare("$query");
			$resultQ->execute();
		...
		while ($row = $results->fetch()) {
			...
			if ($sales > 299) {
				...
				$sales = round($sales / 30,0);
			
				echo "<tr><td style='padding: 5px;'>$database</td><td style='padding: 5px; text-align: right;'>$userid</td><td style='padding: 5px; text-align: right;'>$sales</td><td style='padding: 5px; text-align: right;'>$totalD2</td><td style='padding: 5px; text-align: right;'>$totalD</td></tr>";
