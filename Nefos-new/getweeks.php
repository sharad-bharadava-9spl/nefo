<?php 
	...
	for ($x = $day; $x <= $newday; $x++) {
		...
		$selectSales = "SELECT SUM(amount), SUM(units), SUM(realQuantity) from sales WHERE userConfirmed = 1 AND YEARWEEK(saletime,1) = YEARWEEK(DATE_ADD((NOW()), INTERVAL -$x WEEK),1)";
		try
		{
