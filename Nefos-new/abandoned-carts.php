<?php
	...
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter, userConfirmed, fulfilled, paymentoption, customer, paid, discount, invoiced, delivered, comments, intcomment, delivereddate, shipping FROM sales WHERE userConfirmed = 0 $timeLimit ORDER by saletime DESC $limitVar";
		try
		{
	...
	// Query to look up sales
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter, userConfirmed, fulfilled, paymentoption, customer, paid, discount, invoiced, delivered, comments, intcomment, delivereddate, shipping FROM sales WHERE userConfirmed = 0 $timeLimit ORDER by saletime ASC $limitVar";
		try
		{
...
EOD;
	pageStart("Abandoned shopping carts", NULL, $deleteSaleScript, "psales", "sales admin", "Abandoned shopping carts", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
?>
        ...
        <form action='' method='POST'>
	     <select id='filter' name='filter' onchange='this.form.submit()' class='defaultinput' style='width: 200px;'>
	      <?php echo $optionList; ?>
		 </select>
        </form>
	...
	if ($onesale['category'] == 1) {
		...
		if ($discountedBase == $totAmt) {
			...
			$discountedBase = "<span style='color: red;'>$discountedBase &euro;</span>";
				...
				<td class='clickableRow right' href='dispense.php?saleid={$saleid}'><strong>{$discountedBase}</strong></td>
