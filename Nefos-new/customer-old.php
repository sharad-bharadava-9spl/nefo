<?php
	...
	$userDetails = "SELECT c.id, c.registeredSince, c.Brand, c.number, c.longName, c.shortName, c.cif, c.street, c.streetnumber, c.flat, c.postcode, c.city, c.state, c.country, c.website, c.email, c.facebook, c.twitter, c.instagram, c.googleplus, c.status, c.type, c.lawyer, c.URL, c.source, c.billingType, c.phone, s.statusName, c.membermodule, c.contact, c.language, c.clubtype, c.size, c.opened, c.alias, c.launchdate, c.startdate FROM customers c, customerstatus s WHERE c.status = s.id AND c.id = $user_id";
		...
		if ($directdebit == 1) {
			$dd = "Not interested";
		} else if ($directdebit == 2) {
			$dd = "Interested";
		} else if ($directdebit == 3) {
			$dd = "SEPA sent";
		} else if ($directdebit == 4) {
			$dd = "SEPA received";
		} else if ($directdebit == 5) {
			$dd = "Activated";
		} else if ($directdebit == 6) {
			$dd = "Deactivated";
		} else {
			$dd = "";
		}
	...
	if ($source == '0' || $source == '') {
		...
		$rowCount = $pdo2->query("$selectRows")->fetchColumn();
	...
	$selectSales = "SELECT saleid, saletime, userid, amount, '' AS amountpaid, '' AS quantity, unitsTot AS units, adminComment, creditBefore, creditAfter, 'bar' as Type, '' AS donatedTo FROM b_sales WHERE userid = $user_id ORDER by saletime DESC";
		...
		// Did client subvmit contact update?
		$query = "SELECT registeredSince, approved FROM customers_tmp WHERE number = '$number'";
		try
		{
			$result = $pdo3->prepare("$query");
			$result->execute();
		...
		if ($approved == 1) {
			$updatedOrNot = "<span style='background-color: yellow; color: #333; padding: 5px; border-radius: 10px; font-size: 12px; margin-left: 10px;'>Updated: $registeredSince</span>";
		} else {
			$updatedOrNot = "";
		}
...
function delete_customer(id) {
	if (confirm("Are you sure? There's no going back!")) {
				window.location = "uTil/delete-customer.php?id=" + id;
				}
}
EOD;
...
<?php
	echo "	
  <a href='bar-new-sale-2.php?user_id=$user_id' class='cta'>New HW Sale</a><a href='new-call.php?user_id=$user_id' class='cta'>New Call</a>";
...
?>
 ...
 <span class="secondbuttons">
  ...
  <a href="cutoff.php" class="cta">&laquo; Cutoff panel</a><br />
  ...
  <a href="calls.php?userid=<?php echo $user_id;?>" class="cta">Calls</a>
  <a href="feedback.php?userid=<?php echo $user_id;?>" class="cta">Feedback</a>
  <a href="javascript:delete_customer(<?php echo $user_id;?>)" class="cta" style="background-color: red;">Delete</a>
 </span>
</center>

<div class="overview">
 <span class="profilepicholder"><img class="profilepic" src="../images/_<?php echo $domain; ?>/logo.png" /></span>
 <span class="profilefirst"><?php echo $number . " - " . $shortName ?> <a href="customer-contract.php?domain=<?php echo $domain; ?>"><img src="images/contract.png" style='margin-bottom: -3px; margin-left: 5px;'/></a> <?php echo $updatedOrNot; ?></span>
 <br />
...
<?php
	...
	$query = "SELECT affiliation FROM customers WHERE id = $user_id";
	try
	{
		$result = $pdo3->prepare("$query");
		$result->execute();
	...
	if ($affid > 0) {
		...
		$row = $result->fetch();
			$affname = $row['name'];
			$affiliation = "Affiliation: <a href='affiliation.php?affid=$affid' class='yellow'>$affname</a><br />";
	...
	echo "$longName<br />Alias: $alias<br />$affiliation<span class='yellow'>$statusName $membermodule</span>$warningOutput";
	...
	$selectRealActives = "SELECT creditOrDirect FROM systemsettings";
		...
		echo "<br />Direct debit:</strong> $dd";
 ...
 <div id="statistics">
  ...
  <table class="default memberStats">
   ...
   <tr>
    ...
    <td><?php echo date("d-m-Y", strtotime($lastLogin)); ?></td>
   </tr>
   <tr>
    <td class="first">Last dispense:</td>
    <td><?php echo date("d-m-Y", strtotime($lastSale)); ?></td>
   </tr>
   <tr>
   ...
   <tr>
    ...
    <td><?php echo date("d-m-Y", strtotime($lastActivity)); ?></td>
   </tr>
   <tr>
    <td class="first">Last log entry:</td>
    <td><?php echo date("d-m-Y", strtotime($lastLog)); ?></td>
   </tr>
  </table>
