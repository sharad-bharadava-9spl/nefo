<?php
	...
	// $period = date('Y-m-02');
	$cycleDate = date('Y-08-02');
	$cycleDateView = date('02-08-Y');
	//$period = date('Y-m');
	$period = '202008';
...
*/
		...
		// Query comments
		$query = "SELECT time, comment, operator FROM cutoffcomments WHERE customer = '$customer' ORDER BY time DESC";
		try
		{
			$result = $pdo2->prepare("$query");
			$result->execute();
...
EOD;
		...
		$query = "SELECT time FROM cutoffcomments WHERE customer = '$customer' ORDER BY time DESC LIMIT 1";
		try
		{
			$result = $pdo3->prepare("$query");
			$result->execute();
		...
		if ($invs == 0) {
			...
			$tablerow .= "<tr><td><a href='customer.php?user_id=$customerID'>$customer</a></td><td><a href='customer.php?user_id=$customerID'>$shortName</a></td><td class='centered'>$invs</td><td class='right'>$totAmount &euro;</td><td>$warningText</td><td class='centered' style='padding: 0;'><a href='uTil/pl.php?customer=$customerID'>$pl</a></td><td>$promise</td><td>$cutoffdate</td><td class='noExl'>$lastComment</td><td class='centered noExl'>$commentShow</td></tr>";
...
<center>
<a href='invoices-summary.php' class='cta1'>&laquo; Cutoff panel &laquo;</a>
</center>
...
<div id='mainbox-new-club' style='width: initial;'>
 ...
 <div class='boxcontent'>
  <center>
  This report shows clubs from the current cutoff cycle who have paid their invoices.<br /><br />
