<?php
	...
	$query = "SELECT invno, invdate, customer, action, cutoffdate, promise, paid, amount FROM invoices2 WHERE paid = '' AND justificante <> '' AND verified = 1 ORDER BY customer ASC";
	try
	{
	...
	while ($row = $results->fetch()) {
		...
		//$query = "SELECT COUNT(invno), SUM(amount) FROM invoices2 WHERE paid = '' AND brand = 'SW' AND DATE(invdate) < DATE('$cycleDate') AND customer = '$customer'";
		$query = "SELECT COUNT(invno), SUM(amount) FROM invoices2 WHERE paid = '' AND brand = 'SW' AND period = '$period' AND customer = '$customer'";
...
/*		$query = "SELECT invno, invdate, amount FROM invoices2 WHERE paid = '' AND brand = 'SW' AND DATE(invdate) < DATE('$cycleDate') AND customer = '$customer'";
		try
		{
...
EOD;
		...
		// Query justificantes
		$query = "SELECT justificante, verified FROM invoices2 WHERE invno = '$invno'";
		try
		{
			$result = $pdo->prepare("$query");
			$result->execute();
		...
		if ($lastComment == '01-01-1970') {
			...
			$tablerow .= "<tr><td><a href='customer.php?user_id=$customerID'>$customer</a></td><td><a href='customer.php?user_id=$customerID'>$shortName</a></td><td>$warningText</td><td class='centered' style='padding: 0;'><a href='uTil/pl.php?customer=$customerID'>$pl</a></td><td>$invno</td><td>$invdate</td><td>$age</td><td class='right'>$amount &euro;</td><td class='centered noExl'>$commentShow</td><td class='centered noExl'>$justificante</td></tr>";
	...
	$validationScript = <<<EOD
	    ...
	    $(document).ready(function() {
			...
			$("#xllink2").click(function(){

			  $("#mainTable2").table2excel({
			    // exclude CSS class
			    exclude: ".noExl,.dropdown-filter-dropdown",
			    name: "Unallocated payments",
			    filename: "Unallocated" //do not include extension
...
EOD;
	...
	pageStart("Unallocated payments", NULL, $validationScript, "pmembership", NULL, "Unallocated payments", $_SESSION['successMessage'], $_SESSION['errorMessage']);
	
?>
<link rel="stylesheet" href="css/excel-bootstrap-table-filter-style.css">
<center>
<div id='mainbox' style='width: 1150px;'>
   ...
   <table class='default' id='mainTable'>
    <thead>
     <tr>
      ...
      <th>R / L</th>
      <th>Inv #</th>
      <th dateformat="DD-MM-YYYY" isType="date" class="filter">Inv date</th>
      <th>Age</th>
      <th>Amount</th>
...
<div id='mainbox' style='width: 1150px;'>
 ...
 <div class='boxcontent'>
 	<center>
		 <table class='default' id='cloneTable2'>
	      <tr class='nonhover'>
	       <td colspan='13' style='border-bottom: 0;'>
	         <a href="#" id="xllink2" onClick="$('#mainTable2').tableExport({type:'excel',escape:'false'});"><img src="images/excel-new.png" style='margin: 0 0 -5px 8px;'/></a>
	       </td>
	      </tr>
	     </table>
		<br />
		<?php
				// Query to look up users
				 $selectPayment = "SELECT * FROM invoice_payments2  WHERE allocate_payment ='no' order by id DESC";
					try
					{
						$pay_results = $pdo->prepare("$selectPayment");
						$pay_results->execute();
					}
					catch (PDOException $e)
					{
		...
		<table class='default' id='mainTable2'>
		  <thead>	
		   <tr style='cursor: pointer;'>
		    ...
		    <th>Bank ID</th>
		    <th>Settled Date</th>
		    <th>Bank Lodgement Date</th>
		    <th>Payment Type</th>
		    <th>Invoices</th>
		    <th>Comment</th>
		    <th dateformat="DD-MM-YYYY HH:mm:ss" isType="date" class="filter">Payment Created</th>
		    <th dateformat="DD-MM-YYYY HH:mm:ss" isType="date" class="filter">Payment Updated</th>
		    <th>Action</th>
		   </tr>
		  </thead>
		  <tbody>
		  	<?php
				while ($payment = $pay_results->fetch()) {
					...
					echo sprintf("
				  	    ...
				  	    <td>%s</td>
				  	    <td>%s</td>
				  	    <td><a href='edit-payment.php?id=%d'><img src='images/edit.png' height='15' title='Edit payment'></a></td>
						</tr>",
				  	 $id, $amount, $bank_id_name, $settled_date, $bank_lodgement_date, $payment_type_name, $payment_type_code, $invoices, $commentRead, $created_at, $updated_at, $id
				  	);
				  }
				?>
		  </tbody>
