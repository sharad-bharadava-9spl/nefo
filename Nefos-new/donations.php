...
while ($donation = mysql_fetch_array($result2)) {
	...
	$dTime = date("d-m-Y", strtotime($donation['donationTime']));
	$dTimeSQL = date("Y-m-d", strtotime('-1 day', strtotime($donation['donationTime'])));
	...
	if ($dTime != $currDate) {
		...
		if ($y > 0) {
			...
			$donationTotal = "SELECT SUM(amount) FROM donations WHERE DATE(donationTime) = DATE('$nDate')";
	 ...
	 <table class='default' id='mainTable'>
	  <thead>
	   <tr style='cursor: pointer;'>
	    ...
	    <th>{$lang['donation-creditbefore']}</th>
	    <th>{$lang['donation-creditafter']}</th>
	    <th>{$lang['operator']}</th>
...
EOD;
	  		...
	  		$currDate =  date("d-m-Y", strtotime($donation['donationTime']));
