<?php
	...
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter, userConfirmed, fulfilled, paymentoption, customer, paid, discount, invoiced, delivered, comments, intcomment, delivereddate, shipping FROM sales WHERE userConfirmed = 1 AND cancelled = 0 AND paymentoption <> 9 AND paymentoption <> 1 $timeLimit ORDER by saletime DESC $limitVar";
		try
		{
	...
	// Query to look up sales
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter, userConfirmed, fulfilled, paymentoption, customer, paid, discount, invoiced, delivered, comments, intcomment, delivereddate, shipping FROM sales WHERE userConfirmed = 1 AND cancelled = 0 AND paymentoption <> 9 AND paymentoption <> 1 $timeLimit ORDER by saletime ASC $limitVar";
		try
		{
...
?>
         ...
         <a href="#" id="xllink" onClick="$('#mainTable2').tableExport({type:'excel',escape:'false'});"><img src="images/excel-new.png" style='margin: 0 0 -5px 8px;'/></a>
        <form action='' method='POST'>
	     <select id='filter' name='filter' class='defaultinput' onchange='this.form.submit()' style='width: 200px;' >
	      <?php echo $optionList; ?>
		 </select>
        </form>
<br />
	 <table class='default' id='mainTable'>
	  ...
	  <?php
		...
		// Lookup club info
		$query = "SELECT customer, db_pwd FROM db_access WHERE domain = '$customer'";
		try
		{
	...
	if ($delivered == 0) {
		...
		echo "<tr>
  	   <td class='clickableRow' href='dispense.php?saleid={$saleid}'>$saleid</td>
	...
	if ($onesale['category'] == 1) {
		...
		if ($discountedBase == $totAmt) {
			...
			$discountedBase = "<span style='color: red;'>$discountedBase &euro;</span>";
				...
				<td class=' right' href='dispense.php?saleid={$saleid}'><strong>{$discountedBase}</strong></td>
				<td class=' right' href='dispense.php?saleid={$saleid}'><strong>{$totAmt} &euro;</strong></td>
				<td class=' right' href='dispense.php?saleid={$saleid}'>{$shipping} &euro;</td>
				<td class=' centered relative' href='dispense.php?saleid={$saleid}'>$pay</td>
				<!--<td class='centered relative'>$inMenu</td>-->
				...
				<td class=' centered relative' href='dispense.php?saleid={$saleid}'>$commentRead2</td>
				</tr>
				";
