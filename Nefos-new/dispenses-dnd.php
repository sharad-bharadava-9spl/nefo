<?php
	...
	$selectSales = "SELECT saleid, saletime, userid, amount, amountpaid, quantity, units, adminComment, creditBefore, creditAfter FROM sales $timeLimit ORDER by saletime DESC $limitVar";
...
EOD;
	pageStart($lang['title-dispenses'], NULL, $deleteSaleScript, "psales", "sales admin", "Indefinibles", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
while ($sale = mysql_fetch_array($result)) {
	...
	if ($sale['adminComment'] != '') {
		...
		echo "
  	   <td><input type='checkbox' style='width: 12px;' /></td>
