	...
	authorizeUser($accessLevel);
		...
		$objPHPExcel->getProperties()->setCreator("Lokesh Nayak")
			...
			if (!empty($_GET['untilDate']) && $_GET['untilDate'] != '') {
				...
				$timeLimit = "AND DATE(a.invdate) BETWEEN DATE('$fromDate') AND DATE('$untilDate')";
				$limitVar = "";
		  ...
		  if($_GET['count'] == 0){
				// excel count query
				$excelCountQuery  =   "SELECT * from invoices a  LEFT JOIN invoice_payments b ON a.payment = b.id WHERE DATE(a.invdate) > '2019-12-31' $timeLimit AND a.deleteFlag =0 ORDER BY a.invdate DESC";
...
/*			$payment_no = $payment_type = $bank_id = $settled_date = $bank_lodgement_date = $settled_amount='';
	    ...
	    $countItem++;
	    $redirectURL = 'invoices-report-lokesh.php?fromDate='.$_GET['fromDate'].'&untilDate='.$_GET['untilDate'].'&count='.$countItem.'&totalCount='.$count.'&redirect=1';
	
		header('Refresh: 0; invoices-report-lokesh.php?fromDate='.$_GET['fromDate'].'&untilDate='.$_GET['untilDate'].'&count='.$countItem.'&totalCount='.$count.'&redirect=1');
		exit();
		ob_end_flush();
}
$f=$fileExcel;   
