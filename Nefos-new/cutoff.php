<?php
	...
	// promisetype: 0 = old, uncatregorized, 1 = from SW, 2 = direct link (SMS), 3 = 24-hour extension, 4 = CCS
	...
	$query = "SELECT invno, invdate, customer, action, cutoffdate, promise, paid, amount, promisetype FROM invoices WHERE period = '$period' AND brand = 'SW' AND paid = '' ORDER BY customer ASC";
	try
	{
	...
	while ($row = $results->fetch()) {
		...
		if ($promisetype == 1) {
			...
			$ptype = 'SMS';
		} else if ($promisetype == 3) {
			$ptype = '24-hour';
		} else if ($promisetype == 4) {
			$ptype = 'CCS';
		} else {
			$ptype = '';
		}
		...
		$query = "SELECT id, shortName, pl, directdebit_iban, directdebit_blocked FROM customers WHERE number = '$customer'";
		try
		{
		...
		$rowC = $result->fetch();
			...
			$customerID = $rowC['id'];
			$pl = $rowC['pl'];
			$directdebit_iban = $rowC['directdebit_iban'];
			$directdebit_blocked = $rowC['directdebit_blocked'];
			
			if ($directdebit_iban == '') {
				$dd = '';
			} else {
				
				if ($directdebit_blocked == NULL) {
					$dd = "<span style='color: green;'>Active</span>";
				} else {
					$dd = "<span style='color: red;'>Blocked " . date("d/m/Y", strtotime($directdebit_blocked)) . "</span>";
				}
		...
		$query = "SELECT COUNT(invno), SUM(amount), SUM(delta) FROM invoices WHERE paid = '' AND brand = 'SW' AND customer = '$customer'";
		...
		$row = $result->fetch();
			...
			$remainingAmount = number_format($row['SUM(delta)'],2);
...
*/
		...
		if (!$data) {
			...
			foreach ($data as $rowC) {
				...
				$commenttime = date("d/m/Y H:i", strtotime($rowC['time']."+$offsetSec seconds"));
...
<a href='#' id='showComment$invno'><img src='images/comments.png' width='15' /></a>
...
<span style='font-size: 22px; color: #606f5a; font-weight: 600;'>Comments for <a href='customer.php?user_id=$customerID' target='_blank'> #$customer $shortName</a></span><br /><br />
...
EOD;
		...
		if ($justificante == '') {
			...
			if ($verified == 0) {
				...
				$justificante = "<a href='verify-justificante.php?invno=$invno'><img src='images/image.png' width='15' target='_blank' /></a><span style='display:none'>2</span>";
				
			} else if ($verified == 2) {
				
				$justificante = "<a href='verify-justificante.php?invno=$invno'><img src='images/image-red.png' width='15' target='_blank' /></a><span style='display:none'>3</span>";
		...
		if ($warningText != '<strong>SUNSET</strong>') {
		
			if (isset($_GET['nocutoff'])) {
				
				if ($warning != 3) {
			
					$tablerow .= "<tr><td><a href='customer.php?user_id=$customerID'>$customer</a></td><td><a href='customer.php?user_id=$customerID'>$shortName</a></td><td class='centered'>$dd</td><td class='centered'>$invs</td><td class='right'>$totAmount &euro;</td><td class='right'>$remainingAmount &euro;</td><td>$warningText</td><td class='centered' style='padding: 0;'><a href='uTil/pl.php?customer=$customerID'>$pl</a></td><td>$invno</td><td>$invdate</td><td>$age</td><td class='right'>$amount &euro;</td><td>$promise</td><td class='centered'>$ptype</td><td>$cutoffdate</td><td class='noExl'>$lastComment</td><td class='centered noExl'>$commentShow</td><td class='centered noExl'>$justificante</td></tr>";
...
<a href='cutoff-202201.php' class='cta1'>Jan 2022</a>
<a href='cutoff-202202.php' class='cta1'>Feb 2022</a>
<a href='cutoff-202203.php' class='cta1'>Mar 2022</a>
<a href='cutoff-202204.php' class='cta1'>Apr 2022</a>
<a href='cutoff-202205.php' class='cta1'>May 2022</a>
<a href='cutoff-202206.php' class='cta1'>Jun 2022</a>
<a href='cutoff-202207.php' class='cta1'>Jul 2022</a>
<a href='cutoff-202208.php' class='cta1'>Aug 2022</a>
<a href='cutoff-202209.php' class='cta1'>Sep 2022</a>
<a href='cutoff-202210.php' class='cta1'>Oct 2022</a>
</center>
<div id='mainbox-new-club' style='width: initial;'>
 ...
 <div class='boxcontent'>
  <center>
  Cutoff date: <strong>04-11-2022</strong> (final warnings go up on 03-11-2022).<br /><br />
<?php

	if (isset($_GET['nocutoff'])) {
		echo "<a href='?' style='color: #00a48c;'>Click here to include clubs that have been cut</a>.";
	} else {
		echo "<a href='?nocutoff' style='color: #00a48c;'>Click here to exclude clubs that have been cut</a>.";
	}
   ...
   <table class='default' id='mainTable'>
    <thead>
     <tr>
      ...
      <th>Direct Debit</th>
      <th>Invoices</th>
      <th class='centered'>Total<br />Amount</th>
      <th class='centered'>Pending<br />Amount</th>
      <th>Status</th>
      <th>R / L</th>
      <th>Inv #</th>
      <th>Inv date</th>
      <th>Age</th>
      <th class='centered'>Overdue<br />Amount</th>
      <th>Promise</th>
      <th>Type</th>
      <th>Cut off</th>
      <th>Last comment</th>
