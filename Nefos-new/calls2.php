...
<br />
     ...
     // get contact
       ...
       foreach($comment_val as $comment){ 
		...
		if ($call['complete'] == 0) {
			$complete = "<a href='#' onClick='complete($callId)'>{$lang['global-no']}</a>";
		} else if ($call['complete'] == 1) {
			$complete = "<a href='uTil/complete.php?callid={$call['id']}&menu=yes&pos={$_SESSION['scrollpos']}'><img src='images/complete.png' width='16' /></a>";
		}
...
<script type="text/javascript">
	 function loadExcel(){
       ...
       setTimeout(function () {
		...
		function complete(callid) {
			
			// Get position
			var curpos = window.pageYOffset;
			
			// Launch util function
			//window.location.href = 'uTil/complete.php?callid={$call['id']}&menu=no&pos=';
		//<a href='{$_SESSION['scrollpos']}'>
		alert(callid);
		}
</script>