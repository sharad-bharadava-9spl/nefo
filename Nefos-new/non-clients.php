<?php
	...
	if (isset($_GET['y'])) {
		...
		$selectUsers = "SELECT c.id, c.brand, c.registeredSince, c.number, c.shortName, c.city, c.country, c.status, c.type, c.private, s.statusName, c.contract, c.language FROM customers c, customerstatus s WHERE c.status = s.id AND MONTH(launchdate) = $month AND YEAR(launchdate) = $year AND (c.status = 1 OR c.status = 2 OR c.status = 3 OR c.status = 4 OR c.status = 10 OR c.status = 12 OR c.status = 14 OR c.status = 15) ORDER by c.id ASC";
		
	} else {
	
		$selectUsers = "SELECT c.id, c.brand, c.registeredSince, c.number, c.shortName, c.city, c.country, c.status, c.type, c.private, s.statusName, c.contract, c.language FROM customers c, customerstatus s WHERE c.status = s.id AND (c.status = 1 OR c.status = 2 OR c.status = 3 OR c.status = 4 OR c.status = 10 OR c.status = 12 OR c.status = 14 OR c.status = 15) ORDER by c.id ASC";
...
EOD;
	...
	pageStart("Non-clients", NULL, $memberScript, "pmembership", NULL, "NON-CLIENTS", $_SESSION['successMessage'], $_SESSION['errorMessage']);
?>

<center><a href='clients.php' class='cta1'>Clients</a></center>
