<?php
	...
	if (isset($_GET['expenseid'])) {
		...
		echo "<form action='expenses-mkl.php?expenseid=$expenseid&saveimg' method='post' enctype='multipart/form-data'>";
		
	} else if (isset($_GET['closeday'])) {
		
		echo "<form action='new-expense-1-mkl.php?closeday=true' method='post' enctype='multipart/form-data'>";
		
	} else if (isset($_GET['closeshift'])) {
		
		echo "<form action='new-expense-1-mkl.php?closeshift=true' method='post' enctype='multipart/form-data'>";
		
	} else if (isset($_GET['closeshiftandday'])) {
		
		echo "<form action='new-expense-1-mkl.php?closeshiftandday=true' method='post' enctype='multipart/form-data'>";
		
	} else {
		
		echo "<form action='new-expense-1-mkl.php' method='post' enctype='multipart/form-data'>";
