	...
	foreach($_POST['sales'] as $sale) {
		...
		if ($gramsTot > 0) {
			...
			if ($grams > 0 && $grams2 > 0) {
	    		...
	    		// Query to add new sale to salesdetails table - ? arguments
			  	$query = sprintf("INSERT INTO salesdetails (saleid, category, productid, quantity, amount, purchaseid) VALUES ('%d', '%d', '%d', '%f', '%f', '%d');",
	    		...
	    		// Query to add new sale to salesdetails table - ? arguments
			  	$query = sprintf("INSERT INTO f_salesdetails (saleid, category, productid, quantity, amount, purchaseid) VALUES ('%d', '%d', '%d', '%f', '%f', '%d');",
			  	$saleid, $category, $productid, $grams2, '0', $purchaseid);
...
while ($flower = mysql_fetch_array($resultFlower)) {
	...
	if ($flower['description'] != '') {
		...
		$commentRead = "
		                <img src='images/info2.png' style='inline-block; margin-top: 5px; ' id='comment$i' /><div id='helpBox$i' class='helpBox'><strong>{$lang['extracts-description']}:</strong><br />{$flower['description']}</div>
		                <script>
		                  	$('#comment$i').on({
						 		'mouseover' : function() {
	...
	if ($flower['medicaldescription'] != '') {
		...
		$commentReadM = "
		                <img src='images/med.png' style='inline-block; margin-top: 5px; ' id='commentM$i' /><div id='helpBoxM$i' class='helpBox'><strong>{$lang['extracts-medicaldesc']}:</strong><br />{$flower['medicaldescription']}</div>
		                <script>
		                  	$('#commentM$i').on({
						 		'mouseover' : function() {
   ...
   function compute2() {
	...
	<div class='displaybox'>
	 ...
	 <center>
	  ...
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $flower['purchaseid'] . '.' . $flower['photoExt'], $name, $growtype, $flower['flowertype'], $percentageDisplay, $i, $i, $price, $i, $i, $savedGrams[$i], $i, $i, $savedEuro[$i], $i, $i, $savedGrams2[$i], $i, $i, $i, $i, $name, $i, $flower['purchaseid'], $i, $i, $flower['flowerid']
	  );
...
<script>
   ...
   function compute2() {
	   ...
	   <tr>
  	  ...
  	  <input type='hidden' name='sales[%d][productid]' value='%d' />
",
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $name, $growtype, $flower['flowertype'], $price, $i, $i, $price, $i, $i, $savedGrams[$i], $i, $i, $savedEuro[$i], $i, $i, $savedGrams2[$i], $i, $i, $i, $i, $name, $i, $flower['purchaseid'], $i, $i, $flower['flowerid']
	  );
   ...
   function compute2() {
	...
	<div class='displaybox'>
	 ...
	 <center>
	  ...
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $extract['purchaseid'] . '.' . $extract['photoExt'], $extract['name'], $extract['extract'], $i, $i, $price, $i, $i, $savedGrams[$i], $i, $i, $savedEuro[$i], $i, $i, $savedGrams2[$i], $i, $i, $i, $i, $name, $i, $extract['purchaseid'], $i, $i, $extract['extractid']
	  );
...
<script>
   ...
   function compute2() {
	   ...
	   <tr>
  	  ...
  	  <input type='hidden' name='sales[%d][productid]' value='%d' />
",
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $extract['name'], $extract['extract'], $price, $i, $i, $price, $i, $i, $savedGrams[$i], $i, $i, $savedEuro[$i], $i, $i, $savedGrams2[$i], $i, $i, $i, $i, $name, $i, $extract['purchaseid'], $i, $i, $extract['extractid']
	  );
