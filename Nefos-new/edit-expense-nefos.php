m<?php
	...
	if (isset($_POST['expense'])) {
		...
		$updateExpense = sprintf("UPDATE expenses_nefos SET userid = '%d', expense = '%s', moneysource = '%d', other = '%s', amount = '%f', shop = '%s', comment = '%s', receipt = '%d', invoice = '%d', expensecategory = '%d', moneysource = '%d', registertime = '%s', vat = '%f', vatamt = '%f', brand = '%d', type = '%d', status = '%d', country = '%s', subcat = '%d', personal = '%d', refundable = '%d', refunded = '%d', account = '%d' WHERE expenseid = '%d';",
$userid,
$expense,
$moneysource,
$other,
...
$expenseid
		...
		// Query to update expense
		$updateExpense = sprintf("UPDATE expenses_nefos SET userid = '%d', expense = '%s', moneysource = '%d', other = '%s', amount = '%f', shop = '%s', comment = '%s', receipt = '%d', invoice = '%d', expensecategory = '%d', moneysource = '%d', registertime = '%s', vat = '%f', vatamt = '%f', brand = '%d', paymentdate = '%s', type = '%d', status = '%d', country = '%s', subcat = '%d', personal = '%d', refundable = '%d', refunded = '%d', account = '%d' WHERE expenseid = '%d';",
$userid,
$expense,
$moneysource,
$other,
...
$expenseid
		...
		header("Location: expense-nefos.php?expenseid={$expenseid}");
		exit();
	}
	/***** FORM SUBMIT END *****/
...
EOD;
	...
	$selectExpenses = "SELECT expenseid, registertime, userid, expensetype, expense, moneysource, other, amount, shop, comment, receipt, expensecategory, invoice, vat, brand, paymentdate, type, status, country, subcat, personal, refundable, refunded, account FROM expenses_nefos WHERE expenseid = $expenseid";
		try
		{
...
<span class="fakelabel" style="margin-top: 5px;">Brand:</span>
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="radio" name="brand" value="0" id="accept2" <?php if ($brand == 0) { echo " checked"; }?>/>
...
&nbsp;&nbsp;&nbsp;		
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="radio" name="brand" value="1" id="accept3" <?php if ($brand == 1) { echo " checked"; }?>/>
	  <div class="fakebox"></div>
	 </label>
	</div>
...
<?php
	...
	echo "<option value='$moneysource'>$source</option>";
  	...
  	if ($moneysource != 2) { echo "<option value='2'>Nefos BBVA Bank transfer</option>"; }
  	if ($moneysource != 3) { echo "<option value='3'>Nefos BBVA debit card</option>"; }
  	if ($moneysource != 4) { echo "<option value='4'>Nefos BBVA Ahab credit card</option>"; }
  	if ($moneysource != 5) { echo "<option value='5'>Nefos BBVA Andy credit card</option>"; }
  	if ($moneysource != 6) { echo "<option value='6'>Nefos BBVA Direct Debit</option>"; }
  	if ($moneysource != 12) { echo "<option value='12'>Nefos BOI Direct Debit</option>"; }
  	if ($moneysource != 13) { echo "<option value='13'>Nefos BOI debit card</option>"; }
...
<span class="fakelabel" style="margin-top: 5px;">Account:</span>
	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox2"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;BBVA
	  <input type="radio" name="account" value="0" id="accept2"<?php if ($account == 0) { echo " checked"; }?> />
	  <div class="fakebox"></div>
	 </label>
	</div>
&nbsp;&nbsp;&nbsp;	
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="radio" name="account" value="1" id="accept3" <?php if ($account == 1) { echo " checked"; }?>/>
	  <div class="fakebox"></div>
	 </label>
	</div>
  <br />
