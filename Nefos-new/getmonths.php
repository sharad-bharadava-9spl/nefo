<?php 
	...
	for ($x = $day; $x <= $newday; $x++) {
		...
		$selectSales = "SELECT SUM(amount), SUM(units), SUM(realQuantity) from sales WHERE userConfirmed = 1 AND MONTH(saletime) = MONTH(DATE_ADD((NOW()), INTERVAL -$x MONTH)) AND YEAR(saletime) = YEAR(DATE_ADD((NOW()), INTERVAL -$x MONTH))";
		try
		{
