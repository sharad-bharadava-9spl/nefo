<?php
	...
	if ($_POST['productAdd'] == 'complete') {
		...
		// Query to add new purchase movement - 6 arguments
		  $query = sprintf("INSERT INTO b_productmovements (movementtime, type, purchaseid, quantity, movementTypeid, comment, provider, price, paid, priceg) VALUES ('%s', '%d', '%d', '%f', '%d', '%s', '%d', '%f', '%f', '%f');",
		  $movementtime, '1', $purchaseid, $quantity, $movementTypeid, $comment, $provider, $price, $paid, $priceg);
		try
		{
...
?>
     ...
     <span id="providerBox">
     <span style='display: inline-block; width: 80px;'><?php echo $lang['price']; ?> /u:</span> <input type="number" lang="nb" name="priceg" id="priceg" class="fourDigit" step="0.01" />&euro;<br />
     <span style='display: inline-block; width: 80px;'><?php echo $lang['price']; ?> total:</span> <input type="number" lang="nb" name="price" id="price" class="fourDigit" step="0.01" />€<br />
