<?php
	...
	$openingLookup = "SELECT shiftOpenedNo FROM recshiftclose WHERE closingid = '$closingid'";
		try
		{
	...
	if ($dayOpenedNo > 0) {
		...
		$query = sprintf("UPDATE recshiftopen SET openingtime = '%s', oneCent = '%d', twoCent = '%d', fiveCent = '%d', tenCent = '%d', twentyCent = '%d', fiftyCent = '%d', oneEuro = '%d', twoEuro = '%d', fiveEuro = '%d', tenEuro = '%d', twentyEuro = '%d', fiftyEuro = '%d', hundredEuro = '%d', coinsTot = '%f', notesTot = '%f', tillBalance = '%f', bankBalance = '%f', moneyOwed = '%f', owedPlusTill = '%f', tillDelta = '%f', tillComment = '%s' WHERE openingid = '%d';",
		...
		$timeNow = date('Y-m-d H:i:s');
			...
			$updateClosing = sprintf("UPDATE recshiftclose SET shiftOpened = 2, shiftOpenedAt = '%s', shiftOpenedBy = '%d', shiftOpenedNo = '%d' WHERE closingid = '%d';",
			$timeNow,
			$_SESSION['user_id'],
			$openingid,
			$closingid
		...
		$query = sprintf("INSERT INTO recshiftopen (openingtime, oneCent, twoCent, fiveCent, tenCent, twentyCent, fiftyCent, oneEuro, twoEuro, fiveEuro, tenEuro, twentyEuro, fiftyEuro, hundredEuro, coinsTot, notesTot, tillBalance, bankBalance, moneyOwed, owedPlusTill, tillDelta, tillComment, openedby) VALUES ('%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%s', '%d');",
		  $openingtime, $openCount['oneCent'], $openCount['twoCent'], $openCount['fiveCent'], $openCount['tenCent'], $openCount['twentyCent'], $openCount['fiftyCent'], $openCount['oneEuro'], $openCount['twoEuro'], $openCount['fiveEuro'], $openCount['tenEuro'], $openCount['twentyEuro'], $openCount['fiftyEuro'], $openCount['hundredEuro'], $openCount['coinsTot'], $openCount['notesTot'], $openCount['tillTot'], $bankBalance, $amountOwed, $owedPlusTill, $tillDelta, $openCount['tillComment'], $_SESSION['user_id']);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
