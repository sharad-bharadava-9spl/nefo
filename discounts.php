<?php
	...
	$rowCount = $pdo3->query("$selectUsers")->fetchColumn();
	 ...
	 $selectUsers = "SELECT u.user_id, u.memberno, u.first_name, u.last_name, u.registeredSince, u.dni, u.gender, u.day, u.month, u.year, u.doorAccess, u.paidUntil, u.userGroup, ug.groupName, ug.groupDesc, u.form1, u.form2, u.credit, u.usageType, u.creditEligible, u.dniscan, u.dniext1, u.starCat, u.discount, u.usergroup2, u.discountBar FROM users u, usergroups ug WHERE u.userGroup = ug.userGroup AND memberno <> '0' AND u.userGroup < 7 ORDER by u.memberno ASC LIMIT 1000";
	 ...
	 <table id='cloneTable'>
      <tr class='nonhover'>
       <td colspan='13' style='border-bottom: 0;'>
         <a href="javascript:void(0)" onClick="loadExcel();"><img src="images/excel-new.png" style='margin: 0 0 -5px 8px;'/></a>
       </td>
      </tr>
     </table>
<br />
	 <table class='default' id='mainTable'>
	  <thead>	
	   <tr style='cursor: pointer;'>
	    ...
	    <th><?php echo $lang['member-usergroup']; ?></th>
...
while ($user = $results->fetch()) {
	...
	echo sprintf("
  	  <tr>
  	   ...
  	   ",
	  $user['user_id'], $userStar, $user['user_id'], $user['memberno'], $user['user_id'], $user['first_name'], $user['user_id'], $user['last_name'], $user['user_id'], $usageType, $user['user_id'], $user['discount'], $user['user_id'], $catDiscount, $user['user_id'], $indDiscount,$user['user_id'], $usergroupDiscount, $user['user_id'], $user['discountBar'], $user['user_id'], $catDiscountBar, $user['user_id'], $indDiscountBar);
...
echo <<<EOD
   <tr id="loadMore">
  <td class="centered" colspan="12" style='border-bottom: 0;'><center>(1000 / $rowCount {$lang['global-members']} {$lang['loaded']})<br /><a href="#" onclick="event.preventDefault(); loadMoreDays()" style='font-size: 14px; color: #f07923;'>{$lang['load-more']}  </a></center></td>
 </tr>
EOD;
...
function loadMoreDays(){
	...
	// Add 'Loading' text
	$("#loadMore").remove();
	$("#mainTable").append("<tr id='dayLoading'><td colspan='3' class='centered'>Loading. Please wait...</td></tr>");

	// Take dayID, send it to Ajax
	var dayJSID = parseInt($("#dayID").val());	
	var dayEx = parseInt($("#dayEx").val());	
 ...
 function loadExcel(){
 			...
 			var dayEx = parseInt($("#dayEx").val());	
 			var totRow = parseInt($("#totrows").val());	
       		window.location.href = 'discount-report.php?day_id='+dayEx+'&row_id='+totRow;
       		    setTimeout(function () {
			        $("#load").hide();
			    }, 5000);   
       }
...
<form>
 ...
 <input type="hidden" id="dayEx" value="0" />
