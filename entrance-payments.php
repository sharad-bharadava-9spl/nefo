<?php
	...
	if (isset($_POST['untilDate']) && !empty($_POST['untilDate'])) {
	...
	$selectExpenses = "SELECT paymentid, paymentdate, userid, amountPaid, oldExpiry, newExpiry, paidTo, comment, operator, creditBefore, creditAfter FROM entrancepayments $timeLimit ORDER by paymentdate DESC $limitVar"; 
		try
		{
...
function delete_payment(paymentid) {
	if (confirm("{$lang['payment-deleteconfirm']}")) {
				window.location = "uTil/delete-entrance.php?paymentid=" + paymentid + "&paymentscreen";
				}
}
EOD;
			
	pageStart("Entrance Fees", NULL, $deleteDonationScript, "pmembership", NULL, "Entrance Fees", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
<?php
	...
	$expense_row =	sprintf("
  	  <tr>
  	   ...
  	   <td class='right'>%0.00f {$_SESSION['currencyoperator']}</td>
  	   <td class='left'>%s</td>
  	   <td class='noExl' class='right'><span class='relativeitem'>$commentRead</span></td>
  	   %s
...
?>
	    ...
	    <div  class="actionbox-npr" id = "dialog-3" title = "<?php echo $lang['memberfees']; ?>">
			
			<div class='boxcomtemt'>
				<p>Export excel report between date range</p><br>
...
<script type="text/javascript">
	$( "#dialog-3" ).dialog({
	 ...
	 $("#fullList").click(function(){
	    ...
	    var url = 'entrance-payments-report.php?fromDate='+fromDate+'&untilDate='+untilDate+'&count=0&totalCount=0';
	    window.open(url, "Member Payments Report","height=300,width=300,modal=yes,alwaysRaised=yes");
	    setTimeout(function () {
	        $("#load").hide();
	    }, 2000);    
	 });
