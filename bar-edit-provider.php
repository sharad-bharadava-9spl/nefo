<?php
	...
	if (isset($_POST['name'])) {
		...
		$updateCat = sprintf("UPDATE b_providers SET name = '%s', comment = '%s', providernumber = '%d' WHERE id = '%d';",
			$name,
			$comment,
			$memberno,
			$providerid
...
EOD;
	...
	$categoryDetails = "SELECT name, comment, providernumber FROM b_providers WHERE id = $providerid";
		try
		{
