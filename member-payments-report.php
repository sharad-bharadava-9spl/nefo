...
/*	$selectExpenses = "SELECT paymentid, paymentdate, userid, amountPaid, oldExpiry, newExpiry, paidTo, comment, operator, creditBefore, creditAfter FROM memberpayments $timeLimit ORDER by paymentdate DESC";
		...
		catch (PDOException $e)
		{
				...
				$fileExcel = $domain."-member-payments.xlsx";
				
		    if($_GET['count'] == 0){
				// excel count query
				$excelCountQuery  =   "SELECT paymentid, paymentdate, userid, amountPaid, oldExpiry, newExpiry, paidTo, comment, operator, creditBefore, creditAfter FROM memberpayments $timeLimit ORDER by paymentdate DESC";
...
if($_GET['count'] <= $_GET['totalCount']){
   	...
   	$query  =   "SELECT paymentid, paymentdate, userid, amountPaid, oldExpiry, newExpiry, paidTo, comment, operator, creditBefore, creditAfter FROM memberpayments $timeLimit ORDER by paymentdate DESC Limit ".$page_size." OFFSET ".$offset_var;
   	//$query = "select id from shipment Limit ".$page_size." OFFSET ".$offset_var;
 ...
 $startIndex = 2;
	...
	// Look up user details for showing profile on the Sales page
		...
		header('Refresh: 0; member-payments-report.php?fromDate='.$_GET['fromDate'].'&untilDate='.$_GET['untilDate'].'&count='.$countItem.'&totalCount='.$count.'&redirect=1');
		exit();
		ob_end_flush();
