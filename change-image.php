<?php
    ...
    require_once 'crop-functions.php';
		...
		$imgname = "images/_$domain/purchases/" . $productid . ".jpg";
		...
		$query = "UPDATE purchases SET photoExt = '{$_SESSION['photoextension']}' WHERE purchaseid = $purchaseid";
		try
		{
		...
		$_SESSION['success_url'] = "open-purchases.php";
		$_SESSION['successMessage'] = $lang['picture-updatesuccess'];
	...
	// If not, it means a photo was uploaded. Let's verify it.
		...
		if ($_FILES[$image_fieldname]['error'] != 0) {
			...
			header("Location: ?purchaseid=$purchaseid");
			exit();
		}
		   ...
		   <div class="boxheader">
		    Upload photo   </div>
			 <div class="boxcontent">
			 	<?php   if ($_SESSION['cropOrNot'] == '1') { ?>
					<form name="photo" enctype="multipart/form-data" action="?purchaseid=<?php echo $purchaseid; ?>" method="post">
			...
			<div class="twoboxes">
				<div class="boxheader">
					...
					<form onsubmit='oneClick.disabled = true; return true;' id='myform' method='post' action='change-image-photo.php'>
