<?php
	...
	if ($_SESSION['domain'] == 'thebulldog' || $_SESSION['domain'] == 'cloud' || $_SESSION['domain'] == 'ccstest') {
			...
			$selectSales = "SELECT s.saleid, s.saletime, s.userid, s.amount, s.amountpaid, s.quantity, s.units, s.adminComment, s.creditBefore, s.creditAfter, s.discount, s.direct, s.puesto, s.dispensedfrom, s.paypal, s.fulfilled, d.time FROM sales s, delivery d WHERE 1 AND d.saleid = s.saleid AND s.saleid > 182649 AND (s.puesto = 11 OR s.puesto = 22) $timeLimit ORDER by d.time DESC $limitVar";
