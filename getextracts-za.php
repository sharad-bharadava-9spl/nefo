	...
	if ($prodFijo != 0 || $prodFijo != '') {
		...
		if ($extract['medDiscount'] > 0) {
			...
			$price = round($extract['salesPrice'] * $dispDiscount,2);
        		...
        		$discountType = 2;	
			$price = round(($extract['salesPrice']) * $medicalDiscountCalc,2);
		} else {
        		$dispDiscount1 = number_format((($medicalDiscount * 100) / $extract['salesPrice']),2);
        		$discountType = 2;	
			$price = round(($extract['salesPrice']) - $medicalDiscount,2);
		}
            ...
            if(isset($rowDU['discount_percentage']) && $rowDU['discount_percentage'] > 0){
                ...
                if(isset($rowDU['discount_price']) && $rowDU['discount_price'] > 0){
                    ...
                    $discountType = 4;
                    $price = round(($extract['salesPrice']) - $rowDU['discount_price'],2);
                }
            }
	} else {
		
		$price = round($extract['salesPrice'],2);
		...
		$catCurrentDiscount = (100 - $catDiscount) / 100;
            ...
            if ($prodFijoHD != 0 || $prodFijoHD != '') {
                ...
                if ($prodFijoHD1 > $dispDiscount1){
                    ...
                    //$price = round($extract['salesPrice'] * $prodDiscountHD,2);
                    $dispDiscount1 = number_format((($prodFijoHD * 100) / $extract['salesPrice']),2);
                     $discountType = 3;
                    $price = round($prodFijoHD,2);
                }                
   ...
   function compute3() {
        ...
        function applyVolumeDiscount(){
            ...
            if (b != 0 || b != '') {
                ...
                $.ajax({
                    type:'get',
                    url:'get-volume-discount-za.php?id='+purId+'&qty='+b,
                    datatype:'text',
	    ...
	    function applyVolumeDiscountAmount(){
	        ...
	        if (b != 0 || b != '') {
	            ...
	            $.ajax({
	                type:'get',
	                url:'get-volume-discount-amount-za.php?id='+purId+'&amount='+b,
	                datatype:'text',
	                success:function(data)
	                {
	                    if(data){
	...
	if ($prodFijo != 0 || $prodFijo != '') {
		...
		if ($extract['medDiscount'] > 0) {
            ...
            if(isset($rowDU['discount_percentage']) && $rowDU['discount_percentage'] > 0){
                ...
                $discountType = 4;
                $price = round($extract['salesPrice'] - $dispDiscountP,2);
	} else {
