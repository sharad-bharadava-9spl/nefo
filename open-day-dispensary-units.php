...
EOD;
	...
	pageStart($lang['title-openday'], NULL, $confirmLeave, "pcloseday", "step1 dev-align-center", $lang['openday-dis2-one'], $_SESSION['successMessage'], $_SESSION['errorMessage']);

	if ($_SESSION['firstOpening'] != 'true') {
		echo "<a href='uTil/auto-open-day-dispensary-units.php?cid=$closingid' class='cta1' style='width: 400px;' id='skipCount'>{$lang['dont-count']}</a>";
	} else {
		echo "<a href='uTil/auto-open-day-dispensary-units-first.php' class='cta1' style='width: 400px;' id='skipCount'>{$lang['use-purchase-values']}</a>";
	}
...
<?php
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND closedAt IS NULL";
		...
		if ($catType == 0) {
			...
			echo "<h5>$catName</h5><div class='productboxwrap'>";
		...
		while ($product = $resultProducts->fetch()) {
				...
				$flower_row = sprintf("
					<div class='actionbox-np2 unit-box'>
				 <div class='mainboxheader'>%s ({$product['purchaseid']})</div><br />
				  <div class='boxcontent unit-content'>
				<input type='number' lang='nb' name='dayopenProduct[%d][weight]' class='fourDigit defaultinput' placeholder='u' step='0.01' required /><br />
			  	   ...
			  	   <input type='hidden' name='dayopenProduct[%d][purchaseid]' value='%d' /></div></div>",
...
?>
 ...
 <button name='oneClick' type="submit" class="customButton cta1"><?php echo $lang['global-nextstep']; ?> &raquo;</button>
</form>
