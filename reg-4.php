...
EOD;
		...
		$row = $result->fetch();
			...
			// Look up notes
			$selectRows = "SELECT COUNT(noteid) FROM usernotes WHERE userid = $user_id ORDER by notetime DESC";
			...
			try
			{
				$resultsN = $pdo3->prepare("$getNotes");
				$resultsN->execute();
			}
			catch (PDOException $e)
			{
...
EOD;
  ...
  <center><a href='reg-5.php?aval=$origAval' class='cta'>Continuar</a></center><br />";
