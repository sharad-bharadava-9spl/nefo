<?php
	...
	// Usage tracking
	$query = "INSERT INTO features (domain, user_id, userGroup, feature) VALUES ('{$_SESSION['domain']}', '{$_SESSION['user_id']}', '{$_SESSION['userGroup']}', 3)";
	try
	{
...
<tbody>
 <tr>
  ...
  <td class='centered' style='text-transform: uppercase; font-weight: 600;'>Ingresos ({$_SESSION['currencyoperator']})</td>
  <td class='centered' style='text-transform: uppercase; font-weight: 600;'>Gastos ({$_SESSION['currencyoperator']})</td>
  <td class='centered' style='text-transform: uppercase; font-weight: 600;'>+ ({$_SESSION['currencyoperator']})</td>
 </tr>
...
function loadChart(table_id, container){
    ...
    series2.update({visible: false, color: '#acbca3'});
...
function loadMoreMonths(){
 ...
 function loadExcel(){
 			...
 			var weekJSID = parseInt($("#weekID").val());
 	        var monthJSID = parseInt($("#monthID").val());	 
       		window.location.href = 'profit-loss-report.php?day_id='+dayJSID+'&week_id='+weekJSID+'&month_id='+monthJSID;
       		    setTimeout(function () {
			        $("#load").hide();
			    }, 5000);   
       }
