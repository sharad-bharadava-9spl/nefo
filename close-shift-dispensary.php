<?php
	...
	if ($_SESSION['type'] == 'opening') {
		...
		$checkOpening = "SELECT disShiftClosed AS disClosed, disShiftClosedBy AS disClosedBy, shiftClosedNo FROM opening WHERE openingid = $openingid";
		
	} else {

		$checkOpening = "SELECT disClosed, disClosedBy, shiftClosedNo FROM shiftopen WHERE openingid = $openingid";
	...
	if ($disClosed == '2' && (!isset($_GET['redo']))) {
		pageStart($lang['close-shift'], NULL, $validationScript, "pcloseday", "step2 dev-align-center", $lang['close-shift'] . " - ERROR", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
EOD;
	...
	if ($dayClosedNo > 0) {
		$query = "DELETE from shiftclosedetails WHERE categoryType = 0 AND closingid = $dayClosedNo";
		try
		{
	...
	if ($_SESSION['type'] == 'opening') {
		...
		$updateOpening = sprintf("UPDATE opening SET disShiftClosed = '1', disShiftClosedBy = '%d' WHERE openingid = '%d';",
			$responsible,
			$openingid
		);
...
EOD;
	...
	pageStart($lang['close-shift'], NULL, $confirmLeave, "pcloseday", "dev-align-center", $lang['closeday-dis-one'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
?>
	...
	<form onsubmit='oneClick.disabled = true; return true;' id="skipWeightForm" action="close-shift-dispensary-1.php" method="POST">
...
<?php
	...
	$selectFlower = "SELECT g.flowerid, g.breed2, g.name, p.productid, p.purchaseid, p.purchaseQuantity, p.growType, p.closedAt, p.inMenu, p.tupperWeight FROM flower g, purchases p WHERE p.category = 1 AND p.productid = g.flowerid AND ((p.closedAt IS NULL) OR (p.closingDate BETWEEN '$openingtime' AND '$closingtime')) ORDER BY g.name;";
		try
		{
		...
		$i = 0;
		echo "<span style='display: none;'><h5>{$lang['global-flowerscaps']}</h5><div class='productboxwrap'>";
  ...
  // AND NOW THE H TABLE:
  	$selectExtract = "SELECT h.extractid, h.name, p.productid, p.purchaseid, p.purchaseQuantity, p.closedAt, p.inMenu, p.tupperWeight FROM extract h, purchases p WHERE p.category = 2 AND p.productid = h.extractid AND ((p.closedAt IS NULL) OR (p.closingDate BETWEEN '$openingtime' AND '$closingtime')) ORDER BY h.name;";
		try
		{
...
?>
	...
	<form onsubmit='oneClick.disabled = true; return true;' id="registerForm" action="close-shift-dispensary-1.php" method="POST">
...
<?php
	...
	$selectFlower = "SELECT g.flowerid, g.breed2, g.name, p.productid, p.purchaseid, p.purchaseQuantity, p.growType, p.closedAt, p.inMenu, p.tupperWeight FROM flower g, purchases p WHERE p.category = 1 AND p.productid = g.flowerid AND ((p.closedAt IS NULL) OR (p.closingDate BETWEEN '$openingtime' AND '$closingtime')) ORDER BY g.name;";
		...
		echo "<h5>{$lang['global-flowerscaps']}<div class='fakeboxholder' style='float: right;''> <label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}<input type='checkbox' style='width: 10px;' id='auto1'> <div class='fakebox'></div></label></div></h5><div class='productboxwrap'>";
	...
	<input type='number' lang='nb' name='daycloseProduct[%d][weight]' class='fourDigit defaultinput prequired1' placeholder='g' step='0.01' id='required%d' required /><br />
	 <div class='fakeboxholder'><label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}<input type='checkbox' name='daycloseProduct[%d][auto]'' id='paut0%d' class='pcat1' value='1'> <div class='fakebox'></div> </label></div><br /><br />
	<!--<strong>{$lang['global-shake']}:</strong><br /><br />
    <input type='radio' name='daycloseProduct[%d][shake]' value='0' style='margin-left: 5px; width: 12px;' checked>0%%</input>&nbsp;&nbsp;
    <input type='radio' name='daycloseProduct[%d][shake]' value='25' style='margin-left: 5px; width: 12px;'>25%%</input><br />&nbsp;&nbsp;
    <input type='radio' name='daycloseProduct[%d][shake]' value='50' style='margin-left: 5px; width: 12px;'>50%%</input>&nbsp;&nbsp;
    <input type='radio' name='daycloseProduct[%d][shake]' value='75' style='margin-left: 5px; width: 12px;'>75%%</input>&nbsp;&nbsp;-->
  	   <input type='hidden' name='daycloseProduct[%d][name]' value='%s' />
  	   <input type='hidden' name='daycloseProduct[%d][category]' value='1' />
  	   <input type='hidden' name='daycloseProduct[%d][productid]' value='%d' />
  ...
  // AND NOW THE H TABLE:
  	$selectExtract = "SELECT h.extractid, h.name, p.productid, p.purchaseid, p.purchaseQuantity, p.closedAt, p.inMenu, p.tupperWeight FROM extract h, purchases p WHERE p.category = 2 AND p.productid = h.extractid AND ((p.closedAt IS NULL) OR (p.closingDate BETWEEN '$openingtime' AND '$closingtime')) ORDER BY h.name;";
		...
		echo "</div><h5>{$lang['global-extractscaps']}<div class='fakeboxholder' style='float: right;''> <label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}<input type='checkbox' style='width: 10px;'' id='auto2'> <div class='fakebox'></div></label></div></h5><div class='productboxwrap'>";
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND closedAt IS NULL";
		...
		if ($type == 1) {
			...
			echo "</div><h5>$name<div class='fakeboxholder' style='float: right;''> <label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}<input type='checkbox' style='width: 10px;'' id='auto$category'> <div class='fakebox'></div></label></div></h5><div class='productboxwrap'>";
