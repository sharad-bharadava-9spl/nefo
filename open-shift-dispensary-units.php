...
EOD;
	...
	echo "<a href='uTil/auto-open-shift-dispensary-units.php?cid=$closingid' class='cta1' style='width: 400px;'>{$lang['use-yday-values']}</a>";
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND closedAt IS NULL";
		...
		while ($product = $resultProducts->fetch()) {
				...
				$flower_row = sprintf("
				 ...
				 <div class='mainboxheader'>%s ({$product['purchaseid']})</div>
...
?>
 ...
 <button class='cta1' name='oneClick' type="submit" class="customButton"><?php echo $lang['global-nextstep']; ?> &raquo;</button>
</form>
