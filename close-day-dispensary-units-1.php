<?php
	...
	// If the page re-submitted, let's save Closing values! Also set Opening to 2.
	if (isset($_GET['saveDispensary'])) {
		...
		catch (PDOException $e)
		{
				...
				$deleteCloseOther = "DELETE from closingother WHERE categoryType = 1 AND closingid = '$closingid'";
		try
		{
		...
		catch (PDOException $e)
		{
				...
				if ($_SESSION['openAndClose'] == 2) {
					...
					$updateOpening = sprintf("UPDATE closing SET dis2Opened = 2, dis2OpenedAt = '%s' WHERE closingid = '%d';",
						$realClosingtime,
						$openingid
						);
		try
		...
		catch (PDOException $e)
		{
				...
				$query = sprintf("INSERT INTO closing (closingtime, currentClosing) VALUES ('%s', 1);",
				$realClosingtime);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		...
		$closingid = $pdo3->lastInsertId();
				...
				if ($_SESSION['openAndClose'] == 2) {
					...
					$updateOpening = sprintf("UPDATE closing SET dis2Opened = 2, dayOpenedNo = '%d', dis2OpenedAt = '%s' WHERE closingid = '%d';",
						$closingid,
						$realClosingtime,
						$openingid
						);
		...
		$row = $result->fetch();
					...
					$query = sprintf("INSERT INTO closingdetails (closingid, category, categoryType, productid, purchaseid, weightToday, addedToday, soldToday, takeoutsToday, weight, weightEst, weightDelta, specificComment, intStash, extStash, totalWeight, inMenu) VALUES ('%d', '%d', '%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%s', '%f', '%f', '%f', '%d');",
				  			 $closingid, $category, 1, $productid, $purchaseid, $weightToday, $addedToday, $soldToday, $takeoutsToday, $weight, $estWeight, $weightDelta, $prodclosecomment, $inStashInt, $inStashExt, $weight + $inStashInt + $inStashExt, $inMenu);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
		...
		catch (PDOException $e)
		{
				...
				if ($_SESSION['openAndClose'] == 2) {
					...
					$updateOpening = sprintf("UPDATE closing SET dis2Closed = 2, dis2ClosedAt = '%s' WHERE closingid = '%d';",
						$realClosingtime,
						$closingid
						);
		try
		...
		$row = $result->fetch();
				...
				// Here do DB insert for each cat, echo first to see if it works properly
				$query = sprintf("INSERT INTO closingother (closingid, category, categoryType, stockDelta, soldtoday, unitsSold, prodOpening, prodAdded, prodRemoved, prodEstStock, prodStock, intStash, extStash) VALUES ('%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%f');",
				  $closingid, $catID, 1, ${'totDelta'.$cat}, 0, 0, ${'prodOpening'.$cat}, ${'prodAdded'.$cat}, ${'prodRemoved'.$cat}, ${'prodEstStock'.$cat}, ${'totWeight'.$cat}, $inStashInt, $inStashExt);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
		...
		$row = $result->fetch();
			    	...
			    	// Query to add to closingdetails table - 12 arguments
					$query = sprintf("INSERT INTO closingdetails (closingid, category, categoryType, productid, purchaseid, weightToday, addedToday, soldToday, takeoutsToday, weight, weightEst, weightDelta, specificComment, intStash, extStash, totalWeight, inMenu) VALUES ('%d', '%d', '%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%s', '%f', '%f', '%f', '%d');",
				  			 $closingid, $category, 1, $productid, $purchaseid, $weightToday, $addedToday, $soldToday, $takeoutsToday, $weight, $estWeight, 0, $prodclosecomment, $inStashInt, $inStashExt, $weight + $inStashInt + $inStashExt, $inMenu);
		try
		{
			$result = $pdo3->prepare("$query")->execute();
		}
...
EOD;
		...
		pageStart($lang['title-closeday'], NULL, $confirmLeave, "pcloseday", "step6 dev-align-center", $lang['closeday-dis2-two'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
		...
		foreach($_POST['daycloseProduct'] as $prodClose) {
			...
			if (${'header'.$category} != 'set') {
				echo "<br /><h5 class='title'>$catName</h5>";
				${'header'.$category} = 'set';
			}
...
<script>
   ...
   function compute() {
		...
		<div class='actionbox-np2'>
		 ...
		 <textarea class='defaultinput' placeholder= '{$lang['global-comment']}' name='confirmedClose[%d][prodclosecomment]'></textarea>
		</div>
	  ...
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $prodClose['name'], $purchaseid, $i, $weightToday, $i, $addedToday, $i, $soldToday, $i, $takeoutsToday, $i, $i, $estWeight, $i, $i, $estWeight, $i, $i, $name, $i, $category, $i, $productid, $i, $purchaseid, $i, $productStatus, $i, $inMenu
	  );
...
<script>
   ...
   function compute() {
		...
		<div class='actionbox-np2'>
		 ...
		 <tr>
		  <td>{$lang['countnow']}
		  	<input type='number' lang='nb' name='confirmedClose[%d][weight]' id='fullWeight%d' class='fourDigit purchaseNumber' value='%0.02f' %s step='0.01' /></td>
		 </tr>
		  <tr>
  	   ...
  	   <input type='hidden' name='confirmedClose[%d][productStatus]' value='%s' />
  	   <input type='hidden' name='confirmedClose[%d][inMenu]' value='%d' />",
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $prodClose['name'], $purchaseid, $i, $weightToday, $i, $addedToday, $i, $soldToday, $i, $takeoutsToday, $i, $i, $estWeight, $i, $i, $weight, $disableOrNot, $i, $i, $deltaColour, $weightDelta, $i, $i, $name, $i, $category, $i, $productid, $i, $purchaseid, $i, $productStatus, $i, $inMenu
	  );
...
<script>
   ...
   function compute() {
		...
		<div class='actionbox-np2'>
  	   ...
  	   <input type='hidden' name='confirmedClose[%d][productStatus]' value='%s' />
  	   <input type='hidden' name='confirmedClose[%d][inMenu]' value='%d' />",
	  $i, $i, $i, $i, $i, $i, $i, $i, $i, $i, $prodClose['name'], $purchaseid, $i, $i, $weight, $disableOrNot, $i, $i, $name, $i, $category, $i, $productid, $i, $purchaseid, $i, $productStatus, $i, $inMenu
	  );
