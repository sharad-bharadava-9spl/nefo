<?php
	...
	pageStart($lang['admin-signups'], NULL, NULL, "pdispensary", "product admin", $lang['admin-signups'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
		...
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE(NOW())";
		...
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -1 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus1 = $row['COUNT(memberno)'];
			$quantityTodayMinus1 = $row['SUM(quantity)'];
			
		// Look up daily sales -2
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -2 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus2 = $row['COUNT(memberno)'];
			$quantityTodayMinus2 = $row['SUM(quantity)'];
			
		// Look up daily sales -3
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -3 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus3 = $row['COUNT(memberno)'];
			$quantityTodayMinus3 = $row['SUM(quantity)'];
			
		// Look up daily sales -4
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -4 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus4 = $row['COUNT(memberno)'];
			$quantityTodayMinus4 = $row['SUM(quantity)'];
			
		// Look up daily sales -5
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -5 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus5 = $row['COUNT(memberno)'];
			$quantityTodayMinus5 = $row['SUM(quantity)'];
			
		// Look up daily sales -6
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -6 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus6 = $row['COUNT(memberno)'];
			$quantityTodayMinus6 = $row['SUM(quantity)'];

		// Look up daily sales -7
		$selectSales = "SELECT COUNT(memberno) from users WHERE DATE(registeredSince) = DATE_ADD(DATE(NOW()), INTERVAL -7 DAY)";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesTodayMinus7 = $row['COUNT(memberno)'];
		...
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(NOW(),1) AND YEAR(registeredSince) = YEAR(NOW()) ";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeek = $row['COUNT(memberno)'];
			$quantityWeek = $row['SUM(quantity)'];
			
		// Look up weekly sales -1
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -1 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -1 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus1 = $row['COUNT(memberno)'];
			$quantityWeekMinus1 = $row['SUM(quantity)'];
			
		// Look up weekly sales -2
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -2 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -2 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus2 = $row['COUNT(memberno)'];
			$quantityWeekMinus2 = $row['SUM(quantity)'];
			
		// Look up weekly sales -3
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -3 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -3 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus3 = $row['COUNT(memberno)'];
			$quantityWeekMinus3 = $row['SUM(quantity)'];
			
		// Look up weekly sales -4
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -4 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -4 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus4 = $row['COUNT(memberno)'];
			$quantityWeekMinus4 = $row['SUM(quantity)'];
			
		// Look up weekly sales -5
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -5 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -5 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus5 = $row['COUNT(memberno)'];
			$quantityWeekMinus5 = $row['SUM(quantity)'];
			
		// Look up weekly sales -6
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -6 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -6 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus6 = $row['COUNT(memberno)'];
			$quantityWeekMinus6 = $row['SUM(quantity)'];

		// Look up weekly sales -7
		$selectSales = "SELECT COUNT(memberno) from users WHERE WEEK(registeredSince,1) = WEEK(DATE_ADD((NOW()), INTERVAL -7 WEEK),1) AND YEAR(registeredSince) = YEAR(DATE_ADD((NOW()), INTERVAL -7 WEEK))";

		$result = mysql_query($selectSales)
		or handleError($lang['error-loadsales'],"Error loading sale from db: " . mysql_error());
		
		$row = mysql_fetch_array($result);
			$salesWeekMinus7 = $row['COUNT(memberno)'];
