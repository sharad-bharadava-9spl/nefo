<?php
	...
	/* LOGIC:
		...
		If first closing, do not save details: quantity,
		
		If NOT first closing, DO save details. Also save dispensedreal, flowerreal, extract etc.-
	...
	*/
	...
	if ($_SESSION['openAndClose'] == 2) {
		...
		if (isset($_GET['saveReception'])) {
			...
			$selectMembers = "SELECT COUNT(memberno) from users WHERE (userGroup BETWEEN '1' AND '4') OR (userGroup = 5 AND DATE(paidUntil) >= DATE('$closingtime'))";
			...
			if ($dayClosedNo > 0) {
				...
				if ($_SESSION['noCompare'] != 'true') {
					...
					$query = sprintf("UPDATE closing SET openingtime = '%s', closingtime = '%s', shiftEnd = '%s', oneCent = '%d', twoCent = '%d', fiveCent = '%d', tenCent = '%d', twentyCent = '%d', fiftyCent = '%d', oneEuro = '%d', twoEuro = '%d', fiveEuro = '%d', tenEuro = '%d', twentyEuro = '%d', fiftyEuro = '%d', hundredEuro = '%d', coinsTot = '%f', notesTot = '%f', cashintill = '%f', tillComment = '%s', closingbalance = '%f', bankBalance = '%f', moneytaken = '%f', takenduringday = '%f', closedby = '%d', totalMembers = '%d', activeMembers = '%d', totCredit = '%f' WHERE closingid = '%d';",		  
...
<form onsubmit='oneClick.disabled = true; return true;' id="registerForm" action="?saveReception" method="POST"><br />
 ...
 <input type="hidden" name="salesTodayPaid" value="<?php echo $salesTodayPaid; ?>" />
  ...
  <div class="halfblock">
   <h3><?php echo $lang['closeday-bankbalancecaps']; ?></h3>
   <table>
    <tr>
	 <td><?php echo $lang['closeday-openingtoday']; ?>:</td>
