	...
	if (isset($_POST['dispense'])) {
		...
		mysql_query($query)
			...
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO sales (saletime, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f');",
		  	$saletime, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $gramsTOT);
		...
		$saleid = mysql_insert_id();
			...
			// Query to add new sale to Sales table - 6 arguments
		  	$query = sprintf("INSERT INTO f_sales (saletime, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f');",
		  	$saletime, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil, $gramsTOT);
		...
		mysql_query($query)
			...
			$imgname = 'images/dispensesigs/' . $saleid . '.png';
...
<form id="registerForm" action="" name="FORM1" method="POST">
...
<input id="button2" name="DoneBtn" type="button" value="Finalizar" onclick="javascript:onDone()" style="margin-left: 10px; width: 80px;" /><br />
   <a href="#" id="openDispenseDate" style="vertical-align: bottom;" class="yellow">+ Fecha manuaal</a><br />
...
<?php
	...
	/*
		...
		quantity = grams + grams2
		realQ = realGrams + grams2
	...
	*/
	if (isset($_POST['dispense'])) {
		...
		$salesCalc = $_POST['sales'];
		
		foreach($salesCalc as $salesC) {
			
			$grams = $salesC['grams'];
			$grams2 = $salesC['grams2'];
			$realGrams = $salesC['realGrams'];
		
			$normalQ = $normalQ + $grams + $grams2;
			$realQ = $realQ + $realGrams + $grams2;
		...
		$euroTOT = $_POST['euroTOT'];
		$eurcalcTOT = $_POST['eurcalcTOT'];
		$gramsTOT = $_POST['gramsTOT']; // 2
		$realQuantity = $_POST['realgramsTOT']; // 1,02
		$unitsTOT = $_POST['unitsTOT'];
		$paidTOT = $_POST['paidTOT'];
		$adminComment = $_POST['adminComment'];
		$newcredit = $_POST['newcredit'];
		...
		mysql_query($query)
			...
			// Query to add new sale to Sales table - 6 arguments
		  	...
		  	$saletime, $user_id, $eurcalcTOT, $eurcalcTOT, $normalQ, $realQ, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $paidUntil);
