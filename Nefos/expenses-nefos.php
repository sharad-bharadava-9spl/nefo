<?php
	...
	if (isset($_POST['mydata'])) {
		...
		$imgname = 'images/expenses-nefos/' . $expenseid . '.jpg';
		...
		$updateExt = "UPDATE expenses_nefos SET photoext = 'jpg' WHERE expenseid = $expenseid";
		try
		{
		...
		if ($_FILES[$image_fieldname]['error'] != 0) {
			...
			header("Location: new-receipt-upload-nefos.php?expenseid=$expenseid");
			exit();
		}
		...
		$upload_filename = "images/expenses-nefos/" . $expenseid . "." . $extension;
		...
		$updateExt = "UPDATE expenses_nefos SET photoext = '$extension' WHERE expenseid = $expenseid";
		try
		{
			$result = $pdo3->prepare("$updateExt")->execute();
		}
	...
	$selectExpenses = "SELECT expenseid, registertime, userid, expensetype, expense, moneysource, other, amount, shop, comment, receipt, invoice, expensecategory, photoext, vat, vatamt, brand, paymentdate, type, status, country, subcat, personal, refundable, refunded, account FROM expenses_nefos $timeLimit ORDER by registertime DESC $limitVar";
		try
		{
...
function delete_expense(expenseid) {
	if (confirm("{$lang['expense-deleteconfirm']}")) {
				window.location = "uTil/delete-expense-nefos.php?expenseid=" + expenseid + "&source=expenses";
				}
}
EOD;
...
<center><a href="new-expense-nefos.php" class="cta"><?php echo $lang['expense-newexpense']; ?></a> <a href="expenses-summary-nefos.php" class="cta"><?php echo $lang['summary']; ?></a></center>
...
while ($expense = $results->fetch()) {
	...
	$file = 'images/expenses-nefos/' . $expenseid . '.' . $photoext;

	if (file_exists($file)) {
		$invScan = "<a href='$file'><img src='images/receipt.png' /></a>";
	} else {
		$invScan = "<a href='new-expense-nefos.php?expenseid=$expenseid'><img src='images/receipt-na.png' /></a>";
	}
	...
	} else if ($moneysource == 11) {
		$source = "Nefos BOI Bank transfer";
	...
	$expense_row =	sprintf("
  	  <tr>
  	   <td class='clickableRow left' href='expense.php?expenseid=%d'>$brandName</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   ...
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow left' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td style='text-align: right;' class='clickableRow' href='expense-nefos.php?expenseid=%d'>%0.2f <span class='smallerfont'>&euro;</span></td>
  	   <td style='text-align: right;' class='clickableRow' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow %s' href='expense-nefos.php?expenseid=%d'>%s</td>
  	   <td class='clickableRow %s' href='expense-nefos.php?expenseid=%d'>%s</td>
	   <td>%s</td>
	   <td>%s</td>
  	   <td class='noExl'>%s </td>
	   <td class='relative noExl'>$commentRead</td>
  	   <td class='noExl' style='text-align: center;'><a href='edit-expense-nefos.php?expenseid=%d'><img src='images/edit.png' height='15' title='{$lang['expense-edit']}' /></a>&nbsp;&nbsp;<a href='javascript:delete_expense(%d)'><img src='images/delete.png' height='15' title='{$lang['expense-delete']}' /></a></td>
	  </tr>",
	  $expense['expenseid'], $expense['expenseid'], $formattedDate, $expense['expenseid'], $formattedDate2, $expense['expenseid'], $formattedTime, $expense['expenseid'], $expenseCat, $expense['expenseid'], $subcat, $expense['expenseid'], $expense['expense'], $expense['expenseid'], $VATtype, $expense['expenseid'], $expense['shop'], $expense['expenseid'], $VATstatus, $expense['expenseid'], $expense['country'], $expense['expenseid'], $member, $expense['expenseid'], $expense['amount'], $expense['expenseid'], $vatAmount, $expense['expenseid'], $source, $expense['expenseid'], $account, $recClass, $expense['expenseid'], $receipt, $invClass, $expense['expenseid'], $invoice, $personal, $refundable, $invScan, $expense['expenseid'], $expense['expenseid']
	  );
	  echo $expense_row;
  }
?>
