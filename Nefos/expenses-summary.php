<?php
	...
	$selectExpenseTot = "SELECT SUM(amount), SUM(vatamt) FROM expenses $timeLimit";

	$result1 = mysql_query($selectExpenseTot)
	...
	// Query to look up expenses
	$selectExpenseTot = "SELECT e.expensecategory, SUM(e.amount), SUM(e.vatamt) FROM expenses e, expensecategories c WHERE e.expensecategory = c.categoryid $timeLimit2 GROUP BY expensecategory ORDER BY c.nameen ASC";

	$result2 = mysql_query($selectExpenseTot)
		or handleError($lang['error-expensesload'],"Error loading expense from db: " . mysql_error());
...
while ($expenseTot = mysql_fetch_array($result2)) {
