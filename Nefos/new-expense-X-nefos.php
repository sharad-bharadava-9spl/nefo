<?php
	...
	if (isset($_POST['expense'])) {
		...
		$userid = $_POST['userSelect'];
  	    $expense = mysql_real_escape_string(str_replace('%', '&#37;', $_POST['expense']));
		$expenseCat = $_POST['expenseCat'];
		$shop = mysql_real_escape_string(str_replace('%', '&#37;', $_POST['shop']));
		$amount = $_POST['amount'];
		$moneysource = $_POST['moneysource'];
		$other = $_POST['other'];
		$receipt = $_POST['receipt'];
		$invoice = $_POST['invoice'];
		$comment = mysql_real_escape_string(str_replace('%', '&#37;', $_POST['comment']));
		$day = $_POST['day'];
		$month = $_POST['month'];
		$year = $_POST['year'];
		$day2 = $_POST['day2'];
		...
		$type = $_POST['type'];
		$status = $_POST['status'];
		$country = mysql_real_escape_string(str_replace('%', '&#37;', $_POST['country']));
		...
		if ($paymentdate == '') {
			...
			// Query to add new sale to Sales table - 6 arguments
			  $query = sprintf("INSERT INTO expenses_nefos (registertime, userid, expense, moneysource, other, amount, shop, comment, receipt, expensecategory, invoice, photoext, vat, vatamt, brand, type, status, country) VALUES ('%s', '%d', '%s', '%d', '%s', '%f', '%s', '%s', '%d', '%d', '%d', '%s', '%f', '%f', '%d', '%d', '%d', '%s');",
			  $registertime, $userid, $expense, $moneysource, $other, $amount, $shop, $comment, $receipt, $expenseCat, $invoice, $photoext, $vat, $vatamt, $brand, $type, $status, $country);
			  
		} else {

			// Query to add new sale to Sales table - 6 arguments
			  $query = sprintf("INSERT INTO expenses_nefos (registertime, userid, expense, moneysource, other, amount, shop, comment, receipt, expensecategory, invoice, photoext, vat, vatamt, brand, paymentdate, type, status, country) VALUES ('%s', '%d', '%s', '%d', '%s', '%f', '%s', '%s', '%d', '%d', '%d', '%s', '%f', '%f', '%d', '%s', '%d', '%d', '%s');",
			  $registertime, $userid, $expense, $moneysource, $other, $amount, $shop, $comment, $receipt, $expenseCat, $invoice, $photoext, $vat, $vatamt, $brand, $paymentdate, $type, $status, $country);
		  
		}
				  
		mysql_query($query)
			or handleError($lang['error-savedata'] . mysql_error(),"Error inserting expense: " . mysql_error());
...
EOD;
	...
	if (!isset($_GET['skipFoto'])) {
		...
		if (isset($_POST['mydata'])) {
			...
			$imgname = 'images/expenses-nefos/' . $tempID . '.jpg';
			...
			if ($_FILES[$image_fieldname]['error'] != 0) {
				...
				header("Location: new-receipt-upload-nefos.php");
				exit();
			}
			...
			$upload_filename = "images/expenses-nefos/" . $tempID . "." . $extension;
	...
	pageStart($lang['title-newexpense'], NULL, $validationScript, "pexpenses", "admin", $lang['global-expenses'] . " NEFOS", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
<?php
		...
		if ($_SESSION['lang'] == 'es') {
			$selectExpenseCat = "SELECT categoryid, namees, descriptiones FROM expensecategories WHERE categoryid < 46 ORDER BY nameen ASC";
			$catResult = mysql_query($selectExpenseCat)
				or handleError($lang['error-expensesload'],"Error loading expense from db: " . mysql_error());
		} else {
			$selectExpenseCat = "SELECT categoryid, nameen, descriptionen FROM expensecategories WHERE categoryid < 46 ORDER BY nameen ASC";
			$catResult = mysql_query($selectExpenseCat)
				or handleError($lang['error-expensesload'],"Error loading expense from db: " . mysql_error());
		}
...
<span>
 <input type="radio" name="brand" value="0" style="margin-left: 5px;">Nefos</input>
 <input type="radio" name="brand" value="1">CCS</input>
 </span>
<br />
<br />
...
<span>
 <input type="radio" name="type" value="1" style="margin-left: 3px;">Goods</input>
 <input type="radio" name="type" value="2" style="margin-left: 3px;">Services</input>
</span>
<br />
<span class="fakelabel" style="margin-top: 5px;">Status:</span>
<span>
 <input type="radio" name="status" value="1" style="margin-left: 3px;">Business</input>
 <input type="radio" name="status" value="2" style="margin-left: 3px;">Private</input>
</span>
<br /><br />

<span class="fakelabel" style="margin-top: 2px; margin-bottom: 2px;">Invoice date:</span><div style="display: inline-block;"><span id="dateshow">&nbsp;<?php echo date('d-m-Y'); ?> <a href="#" class="smallerfont yellow" id="clickChange">[<?php echo $lang['change']; ?>]</a></span>
