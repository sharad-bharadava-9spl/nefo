	...
	if(isset($_GET['did'])){
		...
		$selectPathVideo = "SELECT * from help_videos WHERE id= $id";
			...
			$getRow = $select_results->fetch();
				...
				unlink($remove_video_path_en);
				unlink($remove_video_path_es);
				unlink($remove_video_path_ca);
				unlink($remove_video_path_fr);
				unlink($remove_video_path_nl);
				unlink($remove_video_path_it);
				unlink($remove_preview_path);
