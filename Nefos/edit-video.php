		...
		$row = $results->fetch();
			...
			//$video_status = $row['video_status'];
			   ...
			   //  query to look up issues
				$selectTags = "SELECT * FROM video_tags WHERE id NOT IN($tags) order by id ASC"; 
					try
					{
				...
				/*while($rowTag = $result_tag->fetch()){
		   		 ...
		   		 $selectTagsId = "SELECT * from video_tags WHERE id IN ($tags)";
		   				try
						{
		 ...
		 <tr>
		 	...
		 	<td>
	 			<!-- <input type="text" name="tag" id="tagselect" value="<?php //echo $tag_names ?>"  class="multi_tags" placeholder="add multiple tags" required="">
			  	...
			  	<?php  while($tagIdRow = $tag_id_results->fetch()){ 

			  			  $tag_id = $tagIdRow['id'];
			  		      $tag_en = $tagIdRow['tag'];
			  		      $tag_es = $tagIdRow['tag_es'];
			  		      $tag_ca = $tagIdRow['tag_ca'];
			  		      $tag_fr = $tagIdRow['tag_fr'];
			  		      $tag_nl = $tagIdRow['tag_nl'];
			  		      $tag_it = $tagIdRow['tag_it'];
			  		       ...
			  		       $tag_box = $tag_es." (Spanish)<br>".$tag_ca." (Catalan)<br>".$tag_fr." (French)<br>".$tag_nl." (Dutch)<br>".$tag_it." (Italian)";
											...
											$tagInfo = "
											                ...
											                ";
			  	 ?>
			  		<input type="checkbox" class="specialInput" name="tag_id[]" value="<?php echo $tagIdRow['id'] ?>" checked required=""> <?php echo $tagIdRow['tag']; ?>&nbsp;<span style="position: relative;"><?php echo $tagInfo; ?></span>&nbsp;
			   <?php } ?>
			   <?php  while($rowTag = $result_tag->fetch()){
			   			  $tag_id1 = $rowTag['id'];
			  		      $tag_en1 = $rowTag['tag'];
			  		      $tag_es1 = $rowTag['tag_es'];
			  		      $tag_ca1 = $rowTag['tag_ca'];
			  		      $tag_fr1 = $rowTag['tag_fr'];
			  		      $tag_nl1 = $rowTag['tag_nl'];
			  		      $tag_it1 = $rowTag['tag_it'];
			  		       ...
			  		       $tag_box1 = $tag_es1." (Spanish)<br>".$tag_ca1." (Catalan)<br>".$tag_fr1." (French)<br>".$tag_nl1." (Dutch)<br>".$tag_it1." (Italian)";

		  					
											$tagInfo1 = "
											                <img src='images/info2.png' id='comment$tag_id1' /><div id='helpBox$tag_id1' class='helpBox'><strong>Other Languages:</strong><br>{$tag_box1}</div>
											                <script>
											                  	$('#comment$tag_id1').on({
															 		'mouseover' : function() {
																	 	$('#helpBox$tag_id1').css('display', 'block');
															  		},
															  		'mouseout' : function() {
																	 	$('#helpBox$tag_id1').css('display', 'none');
																  	}
															  	});
															</script>
											                ";
			     ...
			     ?>
			  		<input type="checkbox" class="specialInput" name="tag_id[]" value="<?php echo $rowTag['id'] ?>" required=""> <?php echo $rowTag['tag']; ?>&nbsp;<span style="position: relative;"><?php echo $tagInfo1; ?></span>&nbsp;
			   <?php } ?>
		 	</td>
		 </tr> 
		 <tr>
		 	...
		 	<td><input type="file" id="video_input_en" name='video_file_en' onchange="setFileInfo(this, 'video_duration_time_en');" accept="video/mp4"><br><br>
		 		<input type="hidden" name="last_video_path_en" value="<?php echo $video_path_en; ?>">
		 		<input type="hidden" name="video_duration_en" id="video_duration_time_en" value="<?php echo $video_duration_en ?>">
		 		<video width="400" controls>
				 <source src="<?php echo $video_path_en; ?>" type="video/mp4">
