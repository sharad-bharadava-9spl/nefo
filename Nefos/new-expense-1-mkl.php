	...
	if (isset($_POST['expense'])) {
		...
		if ($paymentdate == '') {
			...
			// Query to add new sale to Sales table - 6 arguments
			  $query = sprintf("INSERT INTO expenses_mklnew (registertime, userid, expense, moneysource, other, amount, shop, comment, receipt, expensecategory, invoice, photoext, vat, vatamt, brand, type, status, country, subcat, personal, refundable, hwsw) VALUES ('%s', '%d', '%s', '%d', '%s', '%f', '%s', '%s', '%d', '%d', '%d', '%s', '%f', '%f', '%d', '%d', '%d', '%s', '%d', '%d', '%d', '%s');",
			  $registertime, $userid, $expense, $moneysource, $other, $amount, $shop, $comment, $receipt, $expenseCat, $invoice, $photoext, $vat, $vatamt, $brand, $type, $status, $country, $expenseCatSub, $personal, $refundable, $hwsw);
			  
		} else {

			// Query to add new sale to Sales table - 6 arguments
			  $query = sprintf("INSERT INTO expenses_mklnew (registertime, userid, expense, moneysource, other, amount, shop, comment, receipt, expensecategory, invoice, photoext, vat, vatamt, brand, paymentdate, type, status, country, subcat, personal, refundable, hwsw) VALUES ('%s', '%d', '%s', '%d', '%s', '%f', '%s', '%s', '%d', '%d', '%d', '%s', '%f', '%f', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%d', '%s');",
			  $registertime, $userid, $expense, $moneysource, $other, $amount, $shop, $comment, $receipt, $expenseCat, $invoice, $photoext, $vat, $vatamt, $brand, $paymentdate, $type, $status, $country, $expenseCatSub, $personal, $refundable, $hwsw);
		...
		$oldfile = 'images/expenses-mkl/' . $tempNo . '.' . $photoext;
		$newfile = 'images/expenses-mkl/' . $expenseid . '.' . $photoext;
...
EOD;
	...
	if (!isset($_GET['skipFoto'])) {
		...
		if (isset($_POST['mydata'])) {
			...
			$imgname = 'images/expenses-mkl/' . $tempID . '.jpg';
			...
			if ($_FILES[$image_fieldname]['error'] != 0) {
				...
				header("Location: new-receipt-upload-mkl.php");
				exit();
			}
			...
			$upload_filename = "images/expenses-mkl/" . $tempID . "." . $extension;
	...
	pageStart($lang['title-newexpense'], NULL, $validationScript, "pexpenses", "admin", $lang['global-expenses'] . " mkl", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
<span class="fakelabel" style="margin-top: 5px;">HW or SW:</span>
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="radio" name="hwsw" value="HW" id="accept2" />
...
&nbsp;&nbsp;&nbsp;		
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="radio" name="hwsw" value="SW" id="accept3" />
	  <div class="fakebox"></div>
	 </label>
	</div>
...
<span class="fakelabel" style="margin-top: 5px;"><?php echo $lang['expense-source']; ?>:</span>
  ...
  <option value="7">MKL BBVA SW debit card</option>
  <option value="8">MKL BBVA HW debit card</option>
  <option value="9">MKL BBVA SW credit card</option>
  <option value="10">MKL BBVA HW credit card</option>
