<?php
	...
	if (isset($_POST['amount'])) {
		...
		// Query to add to Donations table
		 $query = sprintf("UPDATE b_providerpayments SET amount = '%f', comment = '%s' WHERE paymentid = '%d';",
	...
	$providerDetails = "SELECT name, providernumber, credit FROM b_providers WHERE id = $providerid";
	try
	{
	...
	$providerDetails = "SELECT amount, comment FROM b_providerpayments WHERE paymentid = $paymentid";
	try
	{
		$result = $pdo3->prepare("$providerDetails");
		$result->execute();
