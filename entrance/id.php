<?php 
	...
	if (isset($_GET['user_id'])) {
		...
		$userDetails = "SELECT u.user_id, u.memberno, u.registeredSince, u.first_name, u.last_name, u.day, u.month, u.year, u.usageType, u.cardid, u.paidUntil, ug.userGroup, ug.groupName, u.credit, u.banComment, u.photoext, u.bajaDate FROM users u, usergroups ug WHERE u.userGroup = ug.userGroup AND u.user_id = '$user_id'";
...
exit();	
			...
			// If user is already signed in, we try to sign him out
				...
				if ($minutesOfVisit < 10) {
					...
					$_SESSION['errorMessage'] = $lang['global-member'] . " " . $lang['left-at'] . " " . $visitTimeReadable. " (" . number_format($minutesOfVisit,0) . " minutos).<br /><strong>No han pasado 15 minutos desde tu entrada!</strong>";
...
EOD;
			...
			<center><span id="count" style="color: #5aa242; font-size: 18px; font-weight: 600;">5</span></center><br />
<div class="overview">
...
<?php 
	...
	if ($userGroup == 5 || $userGroup == 6) {  // show Member w/ expiry
