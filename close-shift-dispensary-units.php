...
<?php
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND ((closedAt IS NULL) OR (closingDate BETWEEN '$openingtime' AND '$closingtime'))";
		...
		if ($catType == 0) {
			...
			echo "<h3 class='title'>$catName</h3><div class='actionbox-np2'>";
		...
		while ($flower = $resultFlower->fetch()) {
				...
				if ($flower['closedAt'] != NULL) {
					...
					$flower_row = sprintf("
		<div class='actionbox-np2'>
	 <div class='mainboxheader'>>%s (%s)</div>
	 <div class='boxcontent'>
	<input type='number' lang='nb' name='daycloseProduct[%d][weight]' class='fourDigit defaultinput' placeholder='g' step='0.01' value='0' disabled required style='color: red;' /><br />
  	   ...
  	   <input type='hidden' name='daycloseProduct[%d][inMenu]' value='%d' /></div></div>",
...
<?php
	...
	$selectCats = "SELECT DISTINCT category FROM purchases WHERE category > 2 AND ((closedAt IS NULL) OR (closingDate BETWEEN '$openingtime' AND '$closingtime'))";
		...
		if ($catType == 0) {
			...
			echo "<h5>$catName<div class='fakeboxholder' style='float:right;'> <label class='control'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$lang['calculate-automatically']}	<input type='checkbox'  id='auto$category' /><div class='fakebox'></div></label></div></h5><br><div class='productboxwrap'>";
...
?>
	 ...
	 <input type='hidden' name='closingConfirm' value='yes' />
 	 <br /><button name='oneClick' type="submit" class="customButton cta1"><?php echo $lang['global-nextstep']; ?> &raquo;</button>
