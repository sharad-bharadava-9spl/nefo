<?php
    ...
    $query = "SELECT COUNT(*) FROM users WHERE invited = 1";
	...
	$selectUsers = "SELECT u.user_id, u.memberno, u.first_name, u.paidUntil, u.registeredSince, u.userGroup, u.last_name, u.credit, u.email, u.telephone, SUM(s.amount), u.invited FROM users u, sales s WHERE u.user_id = s.userid AND u.userGroup <> 8 GROUP BY user_id ORDER by SUM(s.amount) DESC LIMIT $offset, $no_of_records_per_page";
	try
	{
...
if ($_SESSION['membershipFees'] == 1) {
   ...
   $user['user_id'], $lastDispense, $user['user_id'], $user['SUM(s.amount)']);
	...
	if ($invited == 1) {
		$invited = "<img src='images/complete.png' width='16' />";
	} else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
		$invited = "";
	} else {
		$invited = "<a href='uTil/invite.php?user_id={$user['user_id']}'><span style='color: red;'>{$lang['global-no']}</span></a>";
	} 
