<?php
	...
	$productid = $_GET['flowerid'];
	
	
	// Look for purchases with this productid, if it exists, throw an error.
	$deleteUser = "SELECT purchaseid FROM purchases WHERE category = 1 AND productid = $productid";
	...
	if ($data) {
		...
		$deleteUser = sprintf("DELETE FROM flower WHERE flowerid = '%d';", $productid);
		try
		{
		...
		$_SESSION['successMessage'] = $lang['flower-deleted'];
