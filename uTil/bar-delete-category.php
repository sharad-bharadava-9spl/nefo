<?php
	...
	$deleteUser = "SELECT purchaseid FROM b_purchases WHERE category = $categoryid";
	...
	if ($data) {
		...
		$selectCat = "SELECT icon FROM b_categories WHERE id= $categoryid";
		...
		$iconRow = $icon_result->fetch();
			...
			if(trim($icon) != ''){
				$icon_path = "../images/_$domain/bar-category/".$icon;
				unlink($icon_path);
			}
		// Build the delete statement
		$deleteUser = "DELETE FROM b_categories WHERE id = $categoryid";
		try
		{
