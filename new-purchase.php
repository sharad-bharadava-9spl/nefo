...
<?php
	...
	if ($_SESSION['domain'] != 'dabulance') {
		$prodImage = "<img src='images/icon-leaf-g.png' class='midalign' height='48' /><br />";
	} else {
		$prodImage = "";
	}
		...
		echo "
<div class='actionbox-noheight'>
 <h2>$prodImage$name ($type)<span class='usergrouptext'>$rowCount</span></h2>
 <div style='background-color: #c3c8c1; height: 2px;'></div><br />
  <form id='registerForm' action='new-purchase-2.php' method='POST'>
   ...
   <select class='fakeInput defaultinput' name='prePurchase'>
	...
	$selectProduct = "SELECT productid, name, breed2 FROM products WHERE category = $categoryid ORDER by name ASC";
	try
	{
