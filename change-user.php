<?php
	...
	$domain = $_SESSION['domain'];
			...
			catch (PDOException $e)
			{
				...
				$result = $result->fetch();
					...
					catch (PDOException $e)
					{
						...
						$_SESSION['successMessage'] = $lang['operator-changed'];
		...
		if ($_POST['action'] == 'submit') {
			...
			$query = sprintf("SELECT first_name, user_id, memberno, email, userGroup, workStation, domain FROM users WHERE email = '%s' AND $password_match_column = '%s';",
			$email, $password_match);
			try
			{
				$result = $pdo3->prepare("$query");
				$result->execute();
 ...
 <table style="display: inline-block; vertical-align: top; text-align: left;">
  ...
  <tr>
   <td>   <form id="registerForm" action="" method="POST">
     ...
     <input type="hidden" name='photoExt' value='$photoExt'>
