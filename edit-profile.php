...
$user_id
	...
	$userDetails = "SELECT photoext, dniext1, dniext2 FROM users WHERE user_id = $user_id";
		...
		$member_exist = object_exist($google_bucket, $google_root_folder.$imgname);

		if ($member_exist) { 
			   //unlink ($imgname);
				delete_object($google_bucket, $google_root_folder.$imgname);
		 }
		
		$imgname2 = "images/_$domain/ID/" . $user_id . "-front." . $dniext1;
		//if (file_exists($imgname2)) { unlink ($imgname2); }

			$object_exist = object_exist($google_bucket, $google_root_folder.$imgname2);
		if($object_exist){
			delete_object($google_bucket, $google_root_folder.$imgname2);
		}
		
		$imgname3 = "images/_$domain/ID/" . $user_id . "-back." . $dniext2;
		//if (file_exists($imgname3)) { unlink ($imgname3); }
		
		$object_exist2 = object_exist($google_bucket, $google_root_folder.$imgname3);
		if($object_exist2){	
			delete_object($google_bucket, $google_root_folder.$imgname3);
		}
		
		$imgname4 = "images/_$domain/sigs/" . $user_id . ".png";
		$object_exist4 = object_exist($google_bucket, $google_root_folder.$imgname4);
		//if (file_exists($imgname4)) { unlink ($imgname4); }
		if($object_exist4){	
			delete_object($google_bucket, $google_root_folder.$imgname4);
		}
...
if (($_SESSION['exentoset'] == 0 && $_SESSION['userGroup'] == 1) || $_SESSION['exentoset'] == 1) {
	...
	if (($_POST['oldUserGroup'] == 9) && ($_POST['userGroup'] == 5)) {
		...
		if ($_SESSION['normalNumbers'] == 1) {
			...
			$query = "SELECT memberno FROM users WHERE memberno = $memberNumber AND user_id <> $user_id";
			try
			{
				$result = $pdo3->prepare("$query");
				$result->execute();
	...
	// Calculate Workstation access + contributor access
	if ($_SESSION['userGroup'] == 1) {
		...
		foreach($_POST['contributor'] as $contr) {
	  	...
	  	$contributor = $_POST['contributorTot'];
  	}
...
EOD;
    ...
    $(document).ready(function() {
	    ...
	    $('#userGroup').change(function(){
			var val2 = $(this).val();
		    if(val2 == 3) {
		        $("#contributorBox").fadeIn('slow');
	    	} else {
		        $("#contributorBox").fadeOut('slow');
	    	}
	    });
...
EOD;
	...
	$userDetails = "SELECT u.user_id, u.memberno, u.registeredSince, u.first_name, u.last_name, u.email, u.day, u.month, u.year, u.nationality, u.gender, u.dni, u.street, u.streetnumber, u.flat, u.postcode, u.city, u.country, u.telephone, u.mconsumption, u.usageType, u.signupsource, u.cardid, u.cardid2, u.cardid3, u.photoid, u.docid, u.doorAccess, u.friend, u.paidUntil, u.adminComment, ug.userGroup, ug.groupName, ug.groupDesc, u.form1, u.form2, u.creditEligible, u.dniscan, u.discount, u.discountBar, u.photoext, u.workStation, u.starCat, u.interview, u.maxCredit, u.exento, u.usergroup2, u.contributor, u.map, u.calendar, u.menu FROM users u, usergroups ug WHERE u.userGroup = ug.userGroup AND u.user_id = '{$user_id}'";
...
if ($userGroup == 6) {
	...
	$member_exist1 = object_exist($google_bucket, $google_root_folder.$memberPhoto);
...
<?php } ?>
    ...
    <input type="hidden" name="contributorTot" value="<?php echo $contributor; ?>" />
...
<?php if ($_SESSION['domain'] == 'dabulance') { ?>
	<div id="accessBox">
	 ...
	 <label class="control">
	  <span id="errorBox4"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Map
	  <input type="checkbox" name='map' value='1' <?php if ($map == 1) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox5"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Calendar
	  <input type="checkbox" name='calendar' value='1' <?php if ($calendar == 1) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox6"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Menu
	  <input type="checkbox" name='menu' value='1' <?php if ($menu == 1) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
 ...
 if ($_SESSION['puestosOrNot'] == 1) { ?>
	...
	<div id="expiryBox">
	 ...
	 <label class="control">
	  ...
	  <input type="checkbox" name='workStation[]' value='1' <?php if ($workStation == 1 || $workStation == 6 || $workStation == 11 || $workStation == 16 || $workStation == 101 || $workStation == 106 || $workStation == 111 || $workStation == 116) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="checkbox" name='workStation[]' value='5' <?php if ($workStation == 5 || $workStation == 6 || $workStation == 15 || $workStation == 16 || $workStation == 105 || $workStation == 106 || $workStation == 115 || $workStation == 116) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  ...
	  <input type="checkbox" name='workStation[]' value='10' <?php if ($workStation == 10 || $workStation == 11 || $workStation == 15 || $workStation == 16 ) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
<?php }
	...
	<div id="contributorBox">
 	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox4"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Vendor
	  <input type="checkbox" name='contributor[]' value='v' <?php if (strpos($contributor, 'v') !== false) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox5"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Services
	  <input type="checkbox" name='contributor[]' value='s' <?php if (strpos($contributor, 's') !== false) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div><br />
	<div class='fakeboxholder'>	
	 <label class="control">
	  <span id="errorBox6"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Artist
	  <input type="checkbox" name='contributor[]' value='a' <?php if (strpos($contributor, 'a') !== false) { echo "checked"; } ?> />
	  <div class="fakebox"></div>
	 </label>
	</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
