<?php
	...
	if (!isset($_SESSION['clubdomain'])) {
		
		pageStart($lang['title-login'], NULL, $validationScript, "pindex", "loggedOut", NULL, $_SESSION['successMessage'], $lang['no-access']);
		exit();
	...
	if (isset($_POST['dispense'])) {
		...
		// Query to add new sale to Sales table - 6 arguments
	  	$query = sprintf("INSERT INTO sales (saletime, userid, amount, amountPaid, quantity, units, adminComment, settled, settledTo, creditBefore, creditAfter, expiry, realQuantity, discount, userConfirmed, fulfilled, orderHash, customer) VALUES ('%s', '%d', '%f', '%f', '%f', '%f', '%s', '%s', '%d', '%f', '%f', '%s', '%f', '%f', 0, 0, '%s', '%s');",
	  	$saletime, $user_id, $eurcalcTOT, $eurcalcTOT, $gramsTOT, $unitsTOT, $adminComment, $saletime, '1', $creditLookup, $newCreditCalc, $saletime, $gramsTOT, $totDiscountInput, $orderHash, $_SESSION['clubdomain']);
...
EOD;
	...
	pageStart("CCS HighRoller shop", NULL, $validationScript, "pdispense", "newsale", "", $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
<?php
	...
	// Lookup club info
	$query = "SELECT customer FROM db_access WHERE domain = '{$_SESSION['clubdomain']}'";
	try
	{
	...
	echo "<span class='memberinfo'><span class='membername'>$shortName</span>";
...
<?php
	...
	$selectCats = "SELECT id, name from categories ORDER by sortorder ASC, id ASC";
		try
		{
...
<script>
	...
	$memberPhoto = 'images/purchases/' . $purchaseid . ".$photoExt";
