...
$startIndex1 = 3;                      
	...
	for ($a = 0; $a < $day_num; $a++) {
		...
		$row = $result->fetch();
		    ...
		    $objPHPExcel->getActiveSheet()
		                ->setCellValue('B'.$startIndex1, $salesToday);
	...
	for ($a = 0; $a < $week_num; $a++) {
		...
		$row = $result->fetch();
			$salesToday = $row['COUNT(memberno)'];
			    ...
			    $objPHPExcel->getActiveSheet()
			                ->setCellValue('B'.$startIndex2, $salesToday);
	...
	for ($a = 0; $a < $month_num; $a++) {
		...
		$row = $result->fetch();
			$salesToday = $row['COUNT(memberno)'];
			    ...
			    $objPHPExcel->getActiveSheet()
			                ->setCellValue('B'.$startIndex3, $salesToday);
            ...
            header('Content-Disposition: attachment;filename=member-signups.xlsx');
