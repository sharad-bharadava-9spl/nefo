<?php
	...
	$selectPurchases = "SELECT '' AS id, 'purchase' AS type, purchaseDate, purchaseid, category, productid, purchaseQuantity, realQuantity, purchasePrice, paid, adminComment AS comment FROM purchases WHERE provider = $providerid UNION ALL SELECT paymentid AS id, 'payment' AS type, paymentTime AS purchaseDate, '' AS purchaseid, '' AS category, '' AS productid, '' AS purchaseQuantity, '' AS realQuantity, '' AS purchasePrice, amount AS paid, comment FROM providerpayments WHERE providerid = $providerid UNION ALL SELECT '' AS id, 'reload' AS type, movementtime AS purchaseDate, purchaseid, '' AS category, '' AS productid, quantity AS purchaseQuantity, quantity AS realQuantity, price AS purchasePrice, paid, comment FROM productmovements WHERE provider = $providerid ORDER BY purchaseDate DESC";
	try
	{
	...
	$deleteUserScript = <<<EOD
		...
		function delete_payment(id, providerid) {
			if (confirm("{$lang['payment-deleteconfirm']}")) {
						window.location = "uTil/delete-provider-payment.php?paymentid=" + id + "&providerid=" + providerid;
						}
		}
EOD;
...
<div id="mainbox-no-width">
 ...
 <table style="border-spacing: 10px; border-collapse: separate;">
  <tr>
   <td class='dispensetd'><?php echo $lang['purchases']; ?>:<div style='inline-block; float: right;'><?php echo round($totalPurchased,2); ?> <?php echo $_SESSION['currencyoperator'] ?></div></td>
  </tr>
  <tr>
   <td class='dispensetd'><?php echo $lang['payments']; ?>:<div style='inline-block; float: right;'><?php echo round($totalPaid,2); ?> <?php echo $_SESSION['currencyoperator'] ?></div></td>
  </tr>
  <tr>
   <td class='dispensetd' <?php echo $colourStyle; ?>><?php echo $lang['global-credit']; ?>:<div style='inline-block; float: right;'><?php echo round($totCredit,2); ?> <?php echo $_SESSION['currencyoperator'] ?></div></td>
  </tr>
 </table>
</div>
</div>
		...
		if ($type == 'purchase') {
			...
			if ($type == 1) {
				...
				<tr>
		  	    ...
		  	    <td class='right clickableRow' href='purchase.php?purchaseid=$purchaseid'>%0.02f g.</td>
		  	    <td class='right clickableRow' href='purchase.php?purchaseid=$purchaseid'>%0.02f {$_SESSION['currencyoperator']}</td>
		  	    <td class='right clickableRow' href='purchase.php?purchaseid=$purchaseid'>%0.02f {$_SESSION['currencyoperator']}</td>
	   			<td class='clickableRow' href='purchase.php?purchaseid=$purchaseid'><span class='relativeitem'>$commentRead</span></td>
	   			<td></td>
		  	    </tr>",
			...
			<tr class='green'>
	  	    ...
	  	    <td class='right'>%0.02f {$_SESSION['currencyoperator']}</td>
	   		...
	   		<td><a href='edit-provider-payment.php?paymentid=$id&providerid=$providerid'><img src='images/edit.png' height='15' title='Edit' /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:delete_payment($id, $providerid)'><img src='images/delete.png' height='15' /></a></td>
	  	    </tr>",
	  	    $formattedDate, $paid);
