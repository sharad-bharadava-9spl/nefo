<?php
	...
	if (isset($_POST['untilDate'])) {
		...
		// Look up todays BAR dispenses
		$selectSalesFlower = "SELECT SUM(unitsTot), SUM(amount) from b_sales WHERE saletime BETWEEN '$openingTime' AND '$closingTime'";
		try
		{
			$result = $pdo3->prepare("$selectSalesFlower");
			$result->execute();
		...
		while ($pur = $results->fetch()) {
			...
			$totalPrice = round($purchasePrice * $purchaseQuantity,2);
			$totalPriceCat[$category]['totalPrice'] = $totalPriceCat[$category]['totalPrice'] + $totalPrice;
 ...
 <tr>
  ...
  <td class='left'><a href='purchase.php?purchaseid=$purchaseid' style='color: #444;'>$name</a></td>
...
EOD;
		...
		// Look up dispensary purchases PER CATEGORY
		$selectPurchases = "SELECT category, SUM(purchasePrice), SUM(purchaseQuantity) FROM purchases WHERE DATE(purchaseDate) BETWEEN DATE('$openingTime') AND DATE('$closingTime') GROUP BY category";
		try
		{
			$results = $pdo3->prepare("$selectPurchases");
			$results->execute();
...
EOD;
		while ($pur = $results->fetch()) {
			...
			$category = $pur['category'];
			$purchasePrice = $pur['SUM(purchasePrice)'];
			$purchaseQuantity = $pur['SUM(purchaseQuantity)'];
			
			$totalPrice = $totalPriceCat[$category]['totalPrice'];
 ...
 <tr>
  ...
  <td class='left'><strong>$catName</strong></td>
  <td class='left'></td>
  <td><strong>$totalPrice {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>$purchaseQuantity g.</strong></td>
  <td></td>
 </tr>
...
EOD;
 ...
 <tr>
  ...
  <td><strong>$totalPrice {$_SESSION['currencyoperator']}</strong></td>
  <td></td>
  <td><strong>$purchaseQuantity u.</strong></td>
 </tr>
...
EOD;
		...
		while ($pur = $results->fetch()) {
			...
			$totalPriceCatReload[$category]['totalPrice'] = $totalPriceCatReload[$category]['totalPrice'] + $totalPrice;
			$totalPriceCatReload[$category]['paid'] = $totalPriceCatReload[$category]['paid'] + $paid;
			$totalPriceCatReload[$category]['purchaseQuantity'] = $totalPriceCatReload[$category]['purchaseQuantity'] + $purchaseQuantity;
...
EOD;
		...
		// Dispensary reloads BY CATEGORY
		// Query to look for category
		$categoryDetailsCN = "SELECT id, name, type FROM categories";
		try
		{
		...
		if ($totalPriceCatReload[$id]['totalPrice'] > 0 || $totalPriceCatReload[$id]['purchaseQuantity'] > 0) {
 ...
 <tr>
  ...
  <td class='left'><strong>$catName</strong></td>
  <td class='left'></td>
  <td><strong>{$totalPriceCatReload[$id]['totalPrice']} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$totalPriceCatReload[$id]['paid']} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$totalPriceCatReload[$id]['purchaseQuantity']} g.</strong></td>
  <td></td>
 </tr>
...
EOD;
 ...
 <tr>
  ...
  <td><strong>{$totalPriceCatReload[$id]['paid']} {$_SESSION['currencyoperator']}</strong></td>
  <td></td>
  <td><strong>{$totalPriceCatReload[$id]['purchaseQuantity']} u.</strong></td>
 </tr>
...
EOD;
		...
		$rowp = $result->fetch();
			...
			$totalPriceCatReloadB[$category]['totalPrice'] = $totalPriceCatReloadB[$category]['totalPrice'] + $totalPrice;
			$totalPriceCatReloadB[$category]['paid'] = $totalPriceCatReloadB[$category]['paid'] + $paid;
			$totalPriceCatReloadB[$category]['purchaseQuantity'] = $totalPriceCatReloadB[$category]['purchaseQuantity'] + $purchaseQuantity;
...
EOD;
		...
		// Bar reloads BY CATEGORY
		// Query to look for category
		$categoryDetailsCN = "SELECT id, name FROM b_categories";
		try
		{
			$results = $pdo3->prepare("$categoryDetailsCN");
			$results->execute();
		...
		if ($totalPriceCatReloadB[$id]['totalPrice'] > 0 || $totalPriceCatReloadB[$id]['purchaseQuantity'] > 0) {
 ...
 <tr>
  ...
  <td class='left'><strong>$catName</strong></td>
  <td class='left'></td>
  <td><strong>{$totalPriceCatReloadB[$id]['totalPrice']} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$totalPriceCatReloadB[$id]['paid']} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$totalPriceCatReloadB[$id]['purchaseQuantity']} u.</strong></td>
 </tr>
...
EOD;
		...
		while ($pur = $results->fetch()) {
			...
			$totalPrice = round($purchasePrice * $purchaseQuantity,2);
			$totalPriceCatB[$category]['totalPrice'] = $totalPriceCatB[$category]['totalPrice'] + $totalPrice;
...
EOD;
		...
		// Look up dispensary purchases PER CATEGORY
		$selectPurchases = "SELECT category, SUM(purchasePrice), SUM(purchaseQuantity) FROM b_purchases WHERE DATE(purchaseDate) BETWEEN DATE('$openingTime') AND DATE('$closingTime') GROUP BY category";
		try
		{
			$results = $pdo3->prepare("$selectPurchases");
			$results->execute();
...
EOD;
		while ($pur = $results->fetch()) {
			...
			$totalPrice = $totalPriceCatB[$category]['totalPrice'];
...
<table style='color: #$ftcolor; text-align: right; font-family: Tahoma, Verdana, sans-serif; font-size: 14px;' id='t1'>
 ...
 <tr>
  ...
  <td></td>
  <td></td>
  <td><strong>{$expr(number_format($expTot,2))} {$_SESSION['currencyoperator']}</strong></td>
 </tr>
 <tr>
  <td>&nbsp;</td>
  <td></td>
...
echo <<<EOD
 <tr>
  ...
  <td><strong>{$expr(number_format($donationscash + $membershipFeescash + $salescash + $barsalescash,2))} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$expr(number_format($donationsbank + $membershipFeesbank + $salesbank + $barsalesbank,2))} {$_SESSION['currencyoperator']}</strong></td>
  <td><strong>{$expr(number_format($donations + $membershipFees + $salescash + $salesbank + $barsalescash + $barsalesbank,2))} {$_SESSION['currencyoperator']}</strong></td>
 </tr>
 <tr>
  <td>&nbsp;</td>
  <td></td>
 ...
 <tr>
  ...
  <td></td>
  <td></td>
  <td><strong>{$expr(number_format($donations + $membershipFees + $salescash + $barsalescash + $salesbank + $barsalesbank - $expTot,2))} {$_SESSION['currencyoperator']}</strong></td>
 </tr>
 <tr>
  <td>&nbsp;</td>
  <td></td>
...
<table style='color: #$ftcolor; text-align: right; font-family: Tahoma, Verdana, sans-serif; font-size: 14px;' id='t6'>
 <tr>
  <td style='text-align: left;'>{$lang['closeday-dispensed']} {$_SESSION['currencyoperator']}</td>
  <td></td>
  <td></td>
  <td>{$expr(number_format($sales,2))} {$_SESSION['currencyoperator']}</td>
 </tr>
 <tr>
...
EOD;
		...
		$totalPriceCatSales[$category]['salesTot'] = $totalPriceCatSales[$category]['salesTot'] + $salesTot;
		$totalPriceCatSales[$category]['quantitySoldTot'] = $totalPriceCatSales[$category]['quantitySoldTot'] + $quantitySoldTot;
		$totalPriceCatSales[$category]['realquantitySoldTot'] = $totalPriceCatSales[$category]['realquantitySoldTot'] + $realquantitySoldTot;
 ...
 <tr>
  ...
  <td style='text-align: right;'>$salesTot {$_SESSION['currencyoperator']}</td>
  <td style='text-align: right;'>$quantitySoldTot g.</td>
  <td style='text-align: right;'>$realquantitySoldTot g.</td>
...
EOD;
 ...
 <tr>
  ...
  <td style='text-align: right;'></td>
  <td style='text-align: right;'></td>
  <td style='text-align: right;'>$realquantitySoldTot u.</td>
 </tr>
...
EOD;
		...
		if ($totalPriceCatSales[$id]['salesTot'] > 0 || $totalPriceCatSales[$id]['quantitySoldTot'] > 0) {
 ...
 <tr>
  <td style='text-align: left;'><strong>{$catName}</strong></td>
  <td style='text-align: left;'></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSales[$id]['salesTot']} {$_SESSION['currencyoperator']}</strong></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSales[$id]['quantitySoldTot']} g.</strong></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSales[$id]['realquantitySoldTot']} g.</strong></td>
  <td style='text-align: right;'></td>
 </tr>
...
EOD;
 ...
 <tr>
  ...
  <td style='text-align: right;'></td>
  <td style='text-align: right;'></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSales[$id]['realquantitySoldTot']} u.</strong></td>
 </tr>
...
EOD;
	...
	echo <<<EOD
<br /><br /><h1>{$lang['bar-sales-today']}</h1>
...
<table style='color: #$ftcolor; text-align: right; font-family: Tahoma, Verdana, sans-serif; font-size: 14px;' id='t6'>
 <tr>
  ...
  <td></td>
  <td></td>
  <td>{$expr(number_format($barsales,2))} {$_SESSION['currencyoperator']}</td>
 </tr>
 <tr>
  <td style='text-align: left;'>{$lang['closeday-dispensed']} u.</td>
  <td></td>
...
EOD;
	...
	// Look up dispensed last month
	$selectSalesFlower = "SELECT DISTINCT(d.purchaseid), d.productid, d.category FROM b_salesdetails d, b_sales s WHERE s.saleid = d.saleid AND s.saletime BETWEEN '$openingTime' AND '$closingTime' ORDER BY d.category ASC";
		try
		{
			$resultsX = $pdo3->prepare("$selectSalesFlower");
			$resultsX->execute();
		}
		catch (PDOException $e)
		{
...
EOD;
		...
		while ($pur = $resultsX->fetch()) {
		
		
		$purchaseid = $pur['purchaseid'];
		$productid = $pur['productid'];
		
		$selectSalesPurchase = "SELECT SUM(d.amount), SUM(d.quantity), MAX(d.category) from b_sales s, b_salesdetails d WHERE s.saleid = d.saleid AND s.saletime BETWEEN '$openingTime' AND '$closingTime' AND d.purchaseid = $purchaseid";
		try
		{
			$result = $pdo3->prepare("$selectSalesPurchase");
			$result->execute();
		...
		$totalPriceCatSalesB[$category]['salesTot'] = $totalPriceCatSalesB[$category]['salesTot'] + $salesTot;
		$totalPriceCatSalesB[$category]['quantitySoldTot'] = $totalPriceCatSalesB[$category]['quantitySoldTot'] + $quantitySoldTot;


			
			$selectCAT = "SELECT name FROM b_categories WHERE id = $category";
		try
		{
			$result = $pdo3->prepare("$selectCAT");
			$result->execute();
		...
		$catR = $result->fetch();
			...
			$selectProdName = "SELECT name FROM b_products WHERE productid = $productid";
		try
		{
			$result = $pdo3->prepare("$selectProdName");
			$result->execute();
 ...
 <tr>
  ...
  <td style='text-align: left;'>$prodName</td>
  <td style='text-align: right;'>$salesTot {$_SESSION['currencyoperator']}</td>
  <td style='text-align: right;'>$quantitySoldTot u.</td>
 </tr>
...
EOD;
		...
		if ($totalPriceCatSalesB[$id]['salesTot'] > 0 || $totalPriceCatSalesB[$id]['quantitySoldTot'] > 0) {
 ...
 <tr>
  <td style='text-align: left;'><strong>{$catName}</strong></td>
  <td style='text-align: left;'></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSalesB[$id]['salesTot']} {$_SESSION['currencyoperator']}</strong></td>
  <td style='text-align: right;'><strong>{$totalPriceCatSalesB[$id]['quantitySoldTot']} u.</strong></td>
 </tr>
