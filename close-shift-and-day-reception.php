<?php
	...
	if ($recClosed == '2' && (!isset($_GET['redo'])) && (!isset($_GET['addexpense']))) {
		pageStart($lang['close-shift-and-day'], NULL, $validationScript, "pcloseday", "step2 dev-align-center", $lang['closeday-rec-one'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
...
EOD;
		...
		<div class="boxcontent">
		 <table class='purchasetable'>
		  <tr>
		   <td class='biggerFont'>{$lang['day-opened']}
		     <strong>{$dayOpeningtimeView}</strong></td>
		    </tr>
		    <tr>
		   <td class='biggerFont'>{$lang['shift-opened']}
		   <strong>{$shiftOpeningtimeView}</strong></td>
		   </tr>
		   <tr>
		   <td class='biggerFont'>{$lang['day-closed']}
		  <strong> {$closingtimeView}</strong></td>
		  </tr>
		  <tr>
		   <td class='biggerFont'>{$lang['shift-duration']}
		   <strong>{$shiftDuration}</strong></td>
		    </tr>
		    <tr>
		   <td sclass='biggerFont'>{$lang['day-duration']}
		    <strong>{$shiftDurationDay}</strong></td>
		  </tr>
		 </table>
		 </div>
		</div></center>
...
EOD;
	...
	pageStart($lang['close-shift-and-day'], NULL, $deleteExpenseScript, "pcloseday", "dev-align-center", $lang['closeday-rec-one'], $_SESSION['successMessage'], $_SESSION['errorMessage']);
	
	echo $_SESSION['pageHeader'];
	
?>

<center><a href="new-expense.php?closeshiftandday" class="cta1"><?php echo $lang['closeday-addexpense']; ?></a></center>
...
<form onsubmit='oneClick.disabled = true; return true;' id="registerForm" action="close-shift-and-day-reception-1.php" method="POST"><br />
 ...
 <center><button name='oneClick' class="cta4" type="submit"><?php echo $lang['closeday-confirmexpenses']; ?></button></center>
</form>
