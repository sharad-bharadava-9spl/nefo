...
if ($_SESSION['creditOrDirect'] == 1) {
 ...
 <tr>
  ...
  <td>{$expr(number_format($donationsToday,2, '.', ''))}</span></td>
  <td>{$expr(number_format($feesToday,2, '.', ''))}</span></td>
  <td>{$expr(number_format($totalToday,2, '.', ''))}</span></td>
 </tr>
EOD;		
} else {
		$sales_row .= <<<EOD
 <tr>
  ...
  <td>{$expr(number_format($donationsToday,2, '.', ''))}</span></td>
  <td>{$expr(number_format($feesToday,2, '.', ''))}</span></td>
  <td>{$expr(number_format($salesTodayBarCash,2, '.', ''))}</span></td>
  <td>{$expr(number_format($salesTodayCash,2, '.', ''))}</span></td>
  <td>{$expr(number_format($totalToday,2, '.', ''))}</span></td>
 </tr>
EOD;
}
