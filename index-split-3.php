<?php
	...
	$_SESSION['recClose'] = $closingReadable;
	$_SESSION['recCloseSQL'] = $closingSQL;
			...
			if ($openAndClose > 2) {
    			...
    			$query = "SELECT openingtime FROM shiftopen ORDER BY openingtime DESC";
				try
				{
					$result = $pdo3->prepare("$query");
					$result->execute();
				...
				if ($data) {
					...
					foreach ($data as $row) {
		    			...
		    			$query = "SELECT closingid, closingtime FROM shiftclose WHERE closingtime > '$openingtimeSQL'";
						try
						{
						...
						$output .= <<<EOD
   <tr>
    <td class='$clickable' href='index-split-4.php?ot=$openingtime&ots=$openingtimeSQL' style='padding: 10px; padding-right: 50px;'>$openingtime</td>
    <td class='$clickable' href='index-split-4.php?ot=$openingtime&ots=$openingtimeSQL' style='padding: 10px; color: #aaa;'>$closingtime</td>
   </tr>  
EOD;
					...
					echo "<h1>" . $lang['step'] . " 3: " . $lang['choose-dis-opening'] . "</h1>";
